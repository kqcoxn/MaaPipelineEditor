...
## 更新摘要
**变更内容**
- 新增JSON导出缩进配置选项，支持自定义JSON格式化缩进
- 更新配置分类映射，将jsonIndent归类为管道配置
- 扩展管道配置界面，添加JSON导出缩进设置控件
- 更新导出逻辑，支持动态缩进配置
- 增强文件保存功能，支持自定义JSON缩进
- **更新版本信息**：beta迭代号从3增加到4，体现持续开发进度跟踪
...
**更新** 新增JSON导出缩进配置功能，允许用户自定义导出JSON文件的缩进格式，提升配置文件的可读性和一致性。**版本更新** 当前版本为1.3.0_beta_4，beta迭代号已从3升级到4，体现了持续的开发进度跟踪。
...
subgraph "通信协议层"
F[ConfigProtocol.ts<br/>WebSocket 协议]
G[wsStore.ts<br/>WebSocket 状态]
end
subgraph "后端配置系统"
H[config.go<br/>Go 配置结构]
I[paths.go<br/>路径管理]
J[default.json<br/>默认配置]
end
subgraph "应用集成"
K[App.tsx<br/>应用入口]
L[各组件<br/>配置使用]
...
subgraph "JSON格式化"
M[exporter.ts<br/>导出器]
N[fileStore.ts<br/>文件存储]
O[file_service.go<br/>文件服务]
P[nodeJsonValidator.ts<br/>JSON验证器]
end
A --> B
A --> C
A --> D
...
**更新** 新增jsonIndent配置项，归类为管道配置，用于控制JSON导出时的缩进格式。
...
**更新** 新增jsonIndent配置项的处理逻辑，允许用户直接设置JSON导出缩进值。
...
管道配置界面新增了JSON导出缩进设置功能：

```mermaid
graph LR
A[PipelineConfigSection] --> B[节点属性导出形式]
A --> C[默认端点位置]
A --> D[导出默认识别/动作]
A --> E[Pipeline 导出版本]
A --> F[忽略字段校验]
...
**更新** 新增JSON导出缩进配置项，提供直观的数值输入控件，支持0-16空格范围设置。
...
- **范围限制**：支持0-16个空格的缩进设置
- **默认值**：默认为4个空格，符合常见JSON格式标准
- **实时生效**：修改后立即影响后续的JSON导出格式
- **界面提示**：提供详细的使用说明和示例
...
导出器系统集成了JSON缩进配置，确保导出的JSON文件格式一致：
...
**更新** 导出器现在从配置存储中获取jsonIndent值，支持动态缩进配置。
...
文件存储系统集成了JSON缩进配置，确保文件保存时使用正确的格式：
...
H[exporter.ts] --> A
I[fileStore.ts] --> A
J[nodeJsonValidator.ts] --> A
end
subgraph "后端依赖"
K[config.go] --> L[viper]
K --> M[spf13/viper]
N[paths.go] --> O[os/path/filepath]
K --> N
P[file_service.go] --> Q[encoding/json]
P --> R[tailscale/hujson]
end
subgraph "通信依赖"
S[ConfigProtocol.ts] --> T[WebSocket]
U[wsStore.ts] --> T
S --> U
end
subgraph "应用集成"
...
**更新** 新增JSON格式化相关依赖，包括nodeJsonValidator.ts和file_service.go中的JSON处理功能。
...
- **主版本号**：1.3.0
- **次版本号**：语义化版本控制
- **Beta迭代号**：从3升级到4，体现持续开发进度
- **MaaFW版本**：5.9.0
- **协议版本**：0.7.4

**图表来源**
- [configStore.ts:5-11](file://src/stores/configStore.ts#L5-L11)
...
Actions->>Actions : CURRENT=4, PREVIOUS=3
Actions->>Actions : 比较版本差异
Actions->>Preview : 部署新版本
Preview-->>Dev : 预览页面更新
```

**更新** GitHub Actions工作流现在监控beta迭代号从3到4的变更，确保每次beta迭代都触发部署流程。
...
6. **版本管理完善**：多层版本标识和自动化部署流程确保了开发进度的有效跟踪

**更新** 最新的JSON缩进配置功能和版本更新进一步增强了系统的灵活性和用户体验。当前版本1.3.0_beta_4体现了持续的开发进度，beta迭代号从3升级到4，配合GitHub Actions的自动化部署流程，确保了每次迭代都能及时反映到预览环境中。
