...
## 更新摘要
**所做更改**
- 更新了设备管理器和控制器管理器中的输入方法描述，移除了PostThreadMessage相关说明
- 在MFW集成机制章节中反映了输入方法清理的维护更新
- 更新了相关章节的来源标注
...
MCTRL["internal/mfw/controller_manager.go"]
MADPT["internal/mfw/adapter.go"]
...
- [LocalBridge/internal/mfw/controller_manager.go:20-994](file://LocalBridge/internal/mfw/controller_manager.go#L20-L994)
- [LocalBridge/internal/mfw/adapter.go:23-824](file://LocalBridge/internal/mfw/adapter.go#L23-L824)
...
**更新** 设备管理器中的输入方法列表已清理，移除了PostThreadMessage相关条目，保持输入方法列表的简洁性和实用性。
...
+CreateAdbController(adbPath, address, screencapMethod, inputMethod, config, agentPath) string
+CreateWin32Controller(hwnd, screencapMethod, inputMethod) string
...
class Adapter {
+ConnectADB(adbPath, address, screencapMethods, inputMethods, config, agentPath) error
+ConnectWin32(hwnd, screencapMethod, inputMethod) error
+LoadResource(path) error
+LoadResources(paths) error
+InitTasker() error
+RunTask(entry, override) *maa.TaskJob
+StopTask() error
}
Service --> DeviceManager : "持有"
Service --> ControllerManager : "持有"
Service --> Adapter : "持有"
```
...
**章节来源**
- [LocalBridge/internal/mfw/service.go:37-138](file://LocalBridge/internal/mfw/service.go#L37-L138)
- [LocalBridge/internal/mfw/device_manager.go:27-95](file://LocalBridge/internal/mfw/device_manager.go#L27-L95)
- [LocalBridge/internal/mfw/controller_manager.go:25-53](file://LocalBridge/internal/mfw/controller_manager.go#L25-L53)
- [LocalBridge/internal/mfw/adapter.go:63-167](file://LocalBridge/internal/mfw/adapter.go#L63-L167)
...
Maa --> MCTRL["internal/mfw/controller_manager.go"]
Maa --> MADPT["internal/mfw/adapter.go"]
LB --> CFG
LB --> WS
