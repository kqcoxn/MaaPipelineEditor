...
subgraph "前端验证"
FV["文件存储<br/>fileStore.ts"]
NU["节点工具<br/>nodeUtils.ts"]
ES["错误存储<br/>errorStore.ts"]
NH["节点名助手<br/>nodeNameHelper.ts"]
end
FH --> FS
FS --> SC
FS --> WK
...
- [fileStore.ts:631-642](file://src/stores/fileStore.ts#L631-L642)
...
## 架构总览
系统采用"协议处理器 -> 服务层 -> 扫描/监控"的分层设计，配合事件总线与 WebSocket 实现实时推送。启动流程中，主程序加载配置、创建服务、注册处理器、启动 WebSocket 服务器并等待信号退出。**新增的前端验证层在保存前进行节点名重复检查，提升用户体验。**
...
participant FE as "前端验证<br/>fileStore.ts"
participant NV as "节点验证<br/>nodeUtils.ts"
CLI->>CFG : 加载配置
...
**新增功能**：在保存文件前进行重复节点名验证，防止无效管道文件被保存。

- **节点名重复检查**：遍历所有节点，统计每个节点名出现的次数，识别重复项。
- **前缀处理**：根据配置和节点类型自动添加或移除前缀，确保检查准确性。
- **错误状态管理**：使用错误存储管理重复节点名错误，提供统一的错误处理接口。
- **用户反馈**：保存失败时向用户显示详细的重复节点名信息，指导修复。

```mermaid
flowchart TD
Start(["保存文件请求"]) --> CheckErrors["检查重复节点名错误"]
CheckErrors --> HasErrors{"存在重复节点名?"}
HasErrors --> |是| ShowError["显示错误信息并阻止保存"]
HasErrors --> |否| ProcessSave["处理保存请求"]
ShowError --> End(["结束"])
ProcessSave --> SendRequest["发送保存请求到后端"]
SendRequest --> WaitAck["等待后端确认"]
WaitAck --> AckSuccess{"确认成功?"}
AckSuccess --> |是| Success["保存成功"]
AckSuccess --> |否| ShowError
Success --> End
```
...
- [fileStore.ts:631-642](file://src/stores/fileStore.ts#L631-L642)
- [nodeUtils.ts:249-275](file://src/stores/flow/utils/nodeUtils.ts#L249-L275)
- [errorStore.ts:1-38](file://src/stores/errorStore.ts#L1-38)
...
- **新增：错误模型**：ErrorTypeEnum 定义错误类型，包括节点名重复错误。
...
- 循环依赖：未发现循环依赖，模块职责清晰。
- **新增：前端验证依赖**：文件存储依赖节点工具进行重复检查，节点工具依赖错误存储管理状态。
...
- **新增：前端验证性能**：重复节点名检查在保存前进行，避免无效保存请求占用后端资源。
...
## 结论
LocalBridge 文件管理系统通过清晰的分层设计与完善的事件机制，实现了高效、安全、可扩展的本地文件管理能力。扫描与监控结合防抖与忽略策略，确保在复杂文件系统中的稳定性；协议层与 WebSocket 的配合提供了实时反馈；严格的路径验证与安全检查保障了系统安全。**新增的前端重复节点名验证机制进一步提升了用户体验，防止无效管道文件被保存，确保文件完整性。**
