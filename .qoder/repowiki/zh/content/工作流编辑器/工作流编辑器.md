...
## 更新摘要
**变更内容**
- 工具模块重构：数据处理工具从 src/utils/ 移动到 src/utils/data/，UI工具从 src/utils/ 移动到 src/utils/ui/，节点工具从 src/utils/ 移动到 src/utils/node/
- 新增智能粘贴功能，支持节点关系保持和自动定位逻辑
- 搜索面板AI搜索能力增强，支持跨文件节点搜索和自动定位
- 工具面板动态模板计数功能，根据画布高度自适应显示模板数量
- 字段面板工具AI配置验证，提供智能字段填充和验证机制
- 边走线模式配置优化，支持曲线、直角和避让三种连接样式
- 智能连接到空白处功能，支持从节点拖拽连接到画布空白区域时自动弹出节点添加面板
- 跨文件导航能力，支持节点间跳转和文件间导航
...
**更新** 本版本进行了重要的工具模块重构，将原有的统一工具目录结构优化为按功能分类的模块化组织方式。同时新增了多项增强功能：智能粘贴功能支持节点关系保持和自动定位、搜索面板AI搜索能力增强支持跨文件节点搜索、工具面板动态模板计数功能、字段面板工具AI配置验证等，为用户提供更智能、更高效的可视化工作流设计体验。

## 项目结构
工作流编辑器基于React与@xyflow/react构建，采用模块化的节点与边实现、Zustand状态管理、ELKJS自动布局与磁吸对齐等能力，形成可扩展、可维护的可视化工作流编辑平台。工具模块现已重构为按功能分类的组织结构。
...
- [AddPanel.tsx:1-107](file://src/components/panels/tools/AddPanel.tsx#L1-L107)
...
## 架构总览
工作流编辑器采用"容器-节点-边-状态-布局"分层架构，容器负责事件与渲染，节点与边各自封装UI与交互，状态通过切片集中管理，布局算法独立于UI层。工具模块现已重构为按功能分类的组织结构。
...
- **新增** 工具模块重构优化
  ...
  - 数据处理、UI操作、节点验证等工具职责明确，便于单独测试和优化。
...
**更新** 新增的多项增强功能和工具模块重构进一步提升了编辑器的智能化水平和代码质量：智能粘贴功能支持节点关系保持和自动定位、搜索面板AI搜索能力增强支持跨文件节点搜索、工具面板动态模板计数功能、字段面板工具AI配置验证、跨文件导航能力、以及按功能分类的工具模块重构等，为用户提供了更智能、更高效、更易维护的可视化工作流设计体验。

通过本文档的节点类型说明、连接机制解析、操作流程梳理与扩展指南，用户可以快速掌握编辑器的使用与二次开发。这些增强功能不仅提高了工作效率，还为复杂工作流的设计提供了强有力的技术支撑。工具模块的重构更是为未来的功能扩展和维护奠定了坚实的基础。
...
- **边走线模式适配**
  ...
  - **新增** 避让模式适配：新节点应考虑在避让模式下的路径规划，确保不会被误避让。
- **智能连接到空白处适配**
...
- **跨文件导航适配**
  - **新增** 新节点类型应在跨文件搜索中正确显示，支持文件路径和节点名称的解析。
  - **新增** 考虑节点的前缀处理，确保跨文件引用的正确性。
- **工具模块重构适配**
  - **新增** 新增工具函数应遵循新的模块结构，按功能分类组织到相应的工具目录中。
  - **新增** 确保工具函数的导入路径更新为新的模块结构，避免导入错误。
  - **新增** 遵循单一职责原则，将相关功能组织到合适的工具模块中。
...
- **职责分离**：数据处理、UI操作、节点验证等工具职责明确，便于单独测试和优化。
