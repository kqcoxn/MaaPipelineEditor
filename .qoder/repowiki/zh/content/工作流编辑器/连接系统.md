...
## 更新摘要
**变更内容**
- 新增避让路由模式功能，支持智能避让路径计算
- 新增 getAvoidanceEdgePath 函数，包含节点碰撞检测、平行边偏移计算、递归避让算法
- 更新边路径模式配置，现在支持 bezier、smoothstep 和 avoid 三种模式
- 增强边路径计算系统，支持动态切换连接样式
- 控制点拖拽功能仅在贝塞尔模式下启用
...
- 配置层：全局配置影响边标签显示、控制点显示、聚焦透明度、边走线模式等
...
## 架构总览
连接系统采用"视图-状态-工具-样式"的分层设计，通过@xyflow/react提供的节点与边渲染能力，结合Zustand进行状态管理，最终以Less样式完成视觉呈现。新增的避让算法工具提供了智能路径规划能力。
...
在复杂网络图场景下，建议结合路径模式与性能优化策略，以获得更佳的用户体验。避让模式特别适合需要智能路径规划的场景，直角模式适合需要规整布局的场景，而贝塞尔曲线模式则更适合追求流畅视觉效果的应用。