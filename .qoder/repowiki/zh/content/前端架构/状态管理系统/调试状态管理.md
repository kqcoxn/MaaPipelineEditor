...
S6["debugOverlayStore.ts<br/>执行覆盖/高亮"]
S7["debugRunProfileStore.ts<br/>运行配置档案"]
S8["debugModalMemoryStore.ts<br/>模态记忆/偏好简化版"]
S9["debugOverrideStore.ts<br/>调试覆盖草稿"]
end
subgraph "UI反馈层"
...
- 执行覆盖与高亮
  - 职责：根据轨迹摘要或回放摘要，维护节点/边的高亮集合与选中执行记录，支持清理覆盖。
  - 关键状态：各类高亮集合、选中执行相关 ID。
  - 典型操作：应用摘要/回放摘要、应用节点执行覆盖、清理覆盖。
  
  **章节来源**
  - [debugOverlayStore.ts:1-112](file://src/stores/debugOverlayStore.ts#L1-L112)

- 调试运行配置档案
  - 职责：维护调试运行配置档案集与当前激活档案，支持创建/删除/更新、构建运行请求、持久化与版本兼容迁移。
  - 关键状态：档案列表、激活档案 ID、当前档案与工件策略。
  - 典型操作：设置/更新档案、设置入口/资源/代理/工件策略、构建运行请求、持久化读写。
  
  **章节来源**
  - [debugRunProfileStore.ts:1-657](file://src/stores/debugRunProfileStore.ts#L1-L657)

- **更新** 调试模态记忆与偏好（简化版）
  - 职责：维护调试模态的用户偏好（面板、运行模式、过滤器、细节模式等），基于 localStorage 持久化。**简化了lastPanel的生命周期管理**。
  - 关键状态：最后面板、最后运行模式、自动生成功能、自动开闭行为、节点执行过滤器、细节/归属模式。
  - 典型操作：设置最后面板/运行模式/过滤器/模式、持久化读写。
  - **更新**：lastPanel现在只在当前页面生命周期内有效，在刷新后总是从概览重新开始。
  
  **章节来源**
  - [debugModalMemoryStore.ts:1-251](file://src/stores/debugModalMemoryStore.ts#L1-L251)

- 调试覆盖草稿
  - 职责：维护调试覆盖草稿内容，支持读取/写入 localStorage。
  - 关键状态：草稿字符串。
  - 典型操作：设置草稿、重置草稿。
...
participant Overlay as "覆盖Store"
...
### 执行覆盖与高亮
- 设计目的
  - 将轨迹摘要映射到画布元素，高亮当前/已访问/成功/失败节点与执行边，支持节点执行覆盖。
- 数据结构
  - 多个 Set：当前节点、识别中节点、访问/成功/失败节点、执行/候选边
  - 选中执行记录/尝试相关 ID
- 生命周期
  - 应用摘要 -> 同步高亮集合 -> 应用节点执行覆盖 -> 清理覆盖

```mermaid
flowchart TD
Apply["应用摘要/回放摘要"] --> Sync["同步高亮集合"]
Sync --> Overlay["应用节点执行覆盖"]
Overlay --> Clear["清理覆盖"]
```

**图表来源**
- [debugOverlayStore.ts:42-77](file://src/stores/debugOverlayStore.ts#L42-L77)
- [debugOverlayStore.ts:79-111](file://src/stores/debugOverlayStore.ts#L79-L111)
...
### 调试运行配置档案
- 设计目的
  - 统一管理调试运行配置（控制器、代理、资源路径、保存策略、MAA 选项），支持多档案与持久化。
- 数据结构
  - 档案预设：包含档案与工件策略
  - 当前激活档案：用于构建运行请求
- 生命周期
  - 读取持久化 -> 归一化/迁移 -> 写入持久化 -> 更新当前状态
  - 构建运行请求：整合资源快照、控制器、目标节点、输入与覆盖
- 关键流程（构建运行请求）
  - 生成快照 -> 解析目标节点 -> 选择控制器 -> 规范化输入 -> 组装请求

```mermaid
flowchart TD
Load["读取持久化"] --> Normalize["归一化/迁移"]
Normalize --> Persist["写入持久化"]
Persist --> Update["更新当前状态"]
Build["构建运行请求"] --> Snapshot["生成资源快照"]
Snapshot --> Resolve["解析目标节点"]
Resolve --> Controller["选择控制器"]
Controller --> Input["规范化输入"]
Input --> Compose["组装请求"]
```

**图表来源**
- [debugRunProfileStore.ts:119-158](file://src/stores/debugRunProfileStore.ts#L119-L158)
- [debugRunProfileStore.ts:205-237](file://src/stores/debugRunProfileStore.ts#L205-L237)
...
### **更新** 调试模态记忆与偏好（简化版）
- 设计目的
  - 将调试模态的用户偏好持久化到 localStorage，保证刷新后体验一致。**lastPanel现在只在当前页面生命周期内有效**。
- 数据结构
  - 包含最后面板、最后运行模式、自动生成功能、自动开闭行为、节点执行过滤器与细节/归属模式
  - **更新**：lastPanel仅在当前页面生命周期内保存，刷新后重置为默认值
- 生命周期
  - 读取 -> 归一化 -> 写入 -> 更新状态

```mermaid
flowchart TD
Read["读取localStorage"] --> Normalize["归一化/校验"]
Normalize --> Write["写入localStorage"]
Write --> Update["更新状态"]
```

**图表来源**
- [debugModalMemoryStore.ts:185-222](file://src/stores/debugModalMemoryStore.ts#L185-L222)
- [debugModalMemoryStore.ts:224-241](file://src/stores/debugModalMemoryStore.ts#L224-L241)
...
```mermaid
flowchart TD
Read["读取草稿"] --> Write["写入草稿"]
Write --> Reset["重置草稿"]
```

**图表来源**
- [debugOverrideStore.ts:12-28](file://src/stores/debugOverrideStore.ts#L12-L28)
