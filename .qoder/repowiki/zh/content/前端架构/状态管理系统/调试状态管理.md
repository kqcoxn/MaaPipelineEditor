...
subgraph "状态容器"
S1["debugSessionStore.ts<br/>调试会话/能力/资源检查"]
...
S8["debugModalMemoryStore.ts<br/>模态记忆/偏好"]
...
- 调试会话与能力
  - 职责：维护调试模态的打开状态、活动面板、选中节点；承载当前会话快照、运行请求、协议错误；管理能力清单与资源预检/健康检查状态。
  - 关键状态：模态开关、活动面板、会话快照、运行状态、能力清单、资源检查状态。
  - 典型操作：打开/关闭模态、设置会话快照、记录运行开始/停止请求、设置代理测试结果、设置协议错误、设置/清除能力清单与资源检查状态。
...
- 模态记忆与偏好
  - 职责：维护调试模态的用户偏好（面板、运行模式、过滤器、细节模式等），基于 localStorage 持久化。
  - 关键状态：最后面板、最后运行模式、自动生成功能、自动开闭行为、节点执行过滤器、细节/归属模式。
  - 典 typic 操作：设置最后面板/运行模式/过滤器/模式、持久化读写。
...
### 调试会话与资源健康
- 设计目的
  - 作为调试模态的中枢，协调能力探测、资源预检/健康检查与运行控制。
- 数据结构
...
### 模态记忆与偏好
- 设计目的
  - 将调试模态的用户偏好持久化到 localStorage，保证刷新后体验一致。
- 数据结构
...
- 组件内聚与耦合
  ...
  - 会话 store 与资源健康模块通过诊断与错误进行弱耦合联动
...
## 结论
该调试状态管理体系以 zustand 为核心，围绕事件流与派生视图构建，具备良好的扩展性与可维护性。通过明确的 store 职责划分与统一类型定义，实现了从事件采集、摘要生成、覆盖高亮到结果展示的完整闭环。配合持久化与回放能力，满足了日常调试与问题复盘的需求。
