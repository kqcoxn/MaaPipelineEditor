...
## 更新摘要
**变更内容**
- 版本更新记录从1.5.0下调至1.4.3，更新日志标记为'fix'而非major release
- 反映边缘避让bug修复，体现系统在自动布局和字段处理方面的重大改进
- 版本1.4.2特别强调了子字段为空时占位配置项的新增，以及多项性能优化的改进
- 版本1.4.1则重点介绍了WlRoots Wayland合成器控制、macOS原生应用控制功能，以及WaitFreezes系列Focus字段支持
...
该系统的核心功能包括：
- 展示完整的版本历史记录，涵盖从 0.5.2 到 1.4.3 的所有版本更新
- 实时置顶公告显示，突出显示重要通知和公告信息
- 多维度更新分类（新功能、问题修复、体验优化、其他更新）
- Markdown 格式支持（链接和加粗文本）
- 响应式设计和良好的用户体验

**更新** 最新版本（1.4.3）作为当前的占位记录，体现了系统在边缘避让bug修复方面的重大改进。版本 1.4.2 则特别强调了子字段为空时占位配置项的新增，以及多项性能优化的改进。版本 1.4.1 则重点介绍了 WlRoots Wayland 合成器控制、macOS 原生应用控制功能，以及 WaitFreezes 系列 Focus 字段支持。这些功能显著提升了用户在不同操作系统环境下的设备连接和控制体验。版本 1.4.0 则特别强调了 AI 流程探索模式、Anchor 节点高亮索引与跨文件跳转功能，以及右键节点面板可粘贴复制项的便捷操作。版本 1.3.1 则特别强调了两个重要功能的新增：自定义排序功能和连接空白处时的节点面板唤醒功能。自定义排序功能同步影响字段面板与节点渲染，确保字段顺序的一致性；连接空白处功能允许用户在拖拽连接到画布空白处时直接唤起新增节点面板，提升工作效率。版本 1.3.0 则特别强调了状态更新机制的改进，解决了删除重复节点名后状态未及时更新的问题，并引入了重要的性能优化：模态框内不再监听撤销等全局快捷键，显著提升了应用的响应性能。
...
- [updateLogs.ts:1-746](file://src/data/updateLogs.ts#L1-L746)
...
**更新** 最新版本（1.4.3）的占位记录体现了系统在边缘避让bug修复方面的重大改进，包含以下重要特性：

| 版本号 | 发布日期 | 类型 | 主要特性 |
|--------|----------|------|----------|
| 1.4.3 | 2026-4 | fix | **修复分组内边避让模式行为异常的问题**<br/>**优化配置面板使用体验，独立文件配置面板**<br/>**AI 服务兼容 OpenAI、Claude、Gemini、第三方四种模式** |
| 1.4.2 | 2026-4-22 | perf | **👾 新增"子字段为空时占位"配置项**<br/>**优化同位置面板互斥交互体验**<br/>**优化右侧组件层级体验**<br/>**修复导入可嵌套二值坐标列表类字段时，若不嵌套会导致解析为字符串列表的问题**<br/>**修复字段面板过长时会出现双滚动条的问题**<br/>**修复最新开发版提示版本过旧的问题** |
...
**章节来源**
- [updateLogs.ts:49-746](file://src/data/updateLogs.ts#L49-L746)

### 边缘避让bug修复实现

#### 功能概述
版本 1.4.3 修复了分组内边避让模式行为异常的问题，该功能通过以下组件实现：

```mermaid
graph TB
subgraph "边缘避让修复架构"
A[avoidanceUtils.ts] --> B[calculateAvoidancePath 函数]
C[calculateAvoidancePathRecursive 递归函数] --> D[findBlockingNode 检测函数]
E[calculateSelfLoopPath 自循环路径] --> F[calculateAvoidancePath 主入口]
G[EdgePanel 边面板] --> H[边-边避让配置]
I[Flow 组件] --> J[边渲染]
K[GroupNode 分组节点] --> L[分组内避让]
end
```

**图表来源**
- [avoidanceUtils.ts:379-576](file://src/core/avoidanceUtils.ts#L379-L576)
- [avoidanceUtils.ts:581-694](file://src/core/avoidanceUtils.ts#L581-L694)
- [avoidanceUtils.ts:701-746](file://src/core/avoidanceUtils.ts#L701-L746)

#### 边避让路径计算优化
系统通过改进的递归算法处理复杂的避让路径计算：
...
**图表来源**
- [avoidanceUtils.ts:397-415](file://src/core/avoidanceUtils.ts#L397-L415)
- [avoidanceUtils.ts:495-505](file://src/core/avoidanceUtils.ts#L495-L505)
- [avoidanceUtils.ts:561-576](file://src/core/avoidanceUtils.ts#L561-L576)
...
**图表来源**
- [avoidanceUtils.ts:712-717](file://src/core/avoidanceUtils.ts#L712-L717)
- [avoidanceUtils.ts:581-694](file://src/core/avoidanceUtils.ts#L581-L694)
...
**图表来源**
- [avoidanceUtils.ts:714-717](file://src/core/avoidanceUtils.ts#L714-L717)
- [avoidanceUtils.ts:725-729](file://src/core/avoidanceUtils.ts#L725-L729)
...
**更新** 版本 1.4.3 修复了边缘避让bug并优化了相关性能：

#### 递归算法优化
- **深度限制**：通过 maxRecursionDepth 限制递归深度
- **阻挡计数**：通过 blockedCount 防止重复绕行同一节点
- **路径缓存**：避免重复计算相同路径

#### 路径计算优化
- **距离阈值**：通过 directLineMaxDistance 优化直线连接决策
- **绕行策略**：通过 minDist 选择最短绕行路径
- **自循环优化**：通过 calculateSelfLoopPath 优化自循环路径计算

#### 内存管理优化
- **对象复用**：通过 excludeIds Set 避免重复对象创建
- **数组优化**：通过 Point[] 数组优化路径存储
- **配置缓存**：通过 DEFAULT_AVOIDANCE_CONFIG 缓存默认配置
...
#### 边缘避让异常
**症状**：连接线避让节点时路径异常或性能问题
**可能原因**：
- calculateAvoidancePathRecursive 递归深度过大
- blockedCount 计数异常
- 路径计算逻辑错误

**解决步骤**：
1. 检查 maxRecursionDepth 配置是否合理
2. 验证 blockedCount 对象的计数逻辑
3. 确认 findBlockingNode 函数的检测逻辑
4. 检查 calculateAvoidancePath 主入口的参数传递
...
- **新增** 自动布局功能支持对选中节点局部排版，提升工作流管理效率
- **新增** 边缘避让bug修复，优化了复杂路径计算的性能和准确性

**更新** 最新版本（1.4.3）作为当前的占位记录，体现了系统在边缘避让bug修复方面的重大改进。版本 1.4.2 则特别体现了系统在字段处理和性能优化方面的重大改进。通过引入子字段为空时占位配置项，用户可以更灵活地控制字段的显示和处理方式。同时，多项性能优化的实施显著提升了系统的响应速度和用户体验。版本 1.4.1 的发布日期已修正为'2026-4-8'，确保了版本发布的准确时间记录。这一修正虽然看似简单，但对于版本管理的准确性至关重要，有助于用户和开发者更好地追踪软件的发展历程。版本 1.4.1 同样体现了系统在跨平台设备控制和节点关注功能方面的重大突破。通过引入 WlRoots Wayland 合成器控制、macOS 原生应用控制功能，以及 WaitFreezes 系列 Focus 字段支持，系统现在能够支持更多操作系统环境下的设备连接和更精细的节点控制。WaitFreezes 系列 Focus 字段支持是本次功能增强的重要里程碑，它为用户提供了更强大的节点关注能力和回调消息控制机制，使得用户可以更精确地监控和控制节点的执行过程。同时，设备连接面板的智能平台检测功能确保用户只会看到当前平台可用的连接选项，大大简化了用户的操作流程。这些功能的实现展示了现代前端开发在跨平台兼容性和用户体验方面的深度思考和技术实力。
...
**更新** 边缘避让bug修复的引入标志着系统在路径计算和性能优化方面的重大进步。通过改进的递归算法、深度限制和路径缓存机制，系统现在能够更准确地处理复杂的避让路径计算，修复了分组内边避让模式行为异常的问题。这一改进不仅提升了系统的稳定性，还显著改善了用户体验，特别是在处理复杂工作流时的性能表现。
...
**更新** 自动布局功能支持对选中节点局部排版的引入，为用户提供了更灵活的工作流管理方式。这一功能允许用户对特定节点群组进行局部自动布局，提升了复杂工作流的可维护性和可读性。结合"子字段为空时占位"配置项，用户可以更精确地控制字段的显示和处理方式，进一步提升了系统的灵活性和易用性。

**更新** AI 服务兼容性的增强体现了系统在人工智能集成方面的持续发展。通过支持 OpenAI、Claude、Gemini、第三方四种模式，系统为用户提供了更广泛的 AI 服务选择，满足了不同用户的需求和偏好。这一改进不仅提升了系统的实用性，也为未来的 AI 功能扩展奠定了基础。