...
## 更新摘要
**变更内容**
- 修复了二进制坐标列表字段的类型匹配问题，将条件从检查长度4改为接受长度4和长度2的数组
- 改进了字段解析的准确性，支持更灵活的坐标格式
- 更新了位置数组验证逻辑，增强了对不同坐标格式的支持
...
**更新** 本版本重点修复了二进制坐标列表字段的类型匹配问题，改进了对长度4和长度2数组的支持，提升了字段解析的准确性。
...
**更新** 位置数组类型（PositionList）现在支持长度为2和4的整型数组，增强了对不同坐标格式的兼容性。
...
**更新** 位置数组类型匹配现在支持长度为2和4的整型数组，增强了对不同坐标格式的兼容性。
...
BoolOK --> |否| Fail
Switch --> |位置数组| PosCheck["检查长度4或长度2"]
PosCheck --> PosOK{"长度2或4且为整数?"}
PosOK --> |是| RetPos["返回位置数组"]
PosOK --> |否| PosFallback["回退到标签字符串"]
...
**图表来源**
- [typeMatchers.ts:156-183](file://src/core/parser/typeMatchers.ts#L156-L183)
...
位置数组验证逻辑现在能够正确处理以下格式：
- 长度为4的数组：[x, y, w, h]（完整坐标）
- 长度为2的数组：[x, y]（简化坐标）
- 单个值：true（表示启用）
- 字符串：标签字符串
...
```mermaid
flowchart TD
Start(["位置数组输入"]) --> CheckType{"输入类型?"}
CheckType --> |数组| CheckLen["检查数组长度"]
CheckType --> |其他| BuildSingle["构建单个位置"]
CheckLen --> Len4{"长度==4?"}
Len4 --> |是| Validate4["验证4个整数"]
Len4 --> |否| Len2{"长度==2?"}
Len2 --> |是| Validate2["验证2个整数"]
Len2 --> |否| Fallback["回退到标签字符串"]
Validate4 --> Valid4["返回[x,y,w,h]"]
Validate2 --> Valid2["返回[x,y]"]
BuildSingle --> SingleCheck["检查单个值"]
SingleCheck --> IsTrue{"值==true?"}
IsTrue --> |是| RetTrue["返回true"]
IsTrue --> |否| RetStr["返回字符串"]
Valid4 --> End(["结束"])
Valid2 --> End
RetTrue --> End
RetStr --> End
...
**更新** 位置数组验证逻辑的改进为自定义验证规则提供了更好的基础，支持更灵活的坐标格式处理。
...
**更新** 位置数组验证的改进减少了不必要的长度检查，提升了验证效率。
...
- **更新** 位置数组验证失败：检查输入是否为长度为2或4的整型数组，或是否为有效的布尔值/字符串。
...
## 结论
该字段验证系统以 Schema 驱动的方式定义字段约束，结合类型匹配器实现统一的同步验证，并通过 UI 编辑器提供即时反馈。waitFreezes 与 focus 等复杂字段通过结构化模式与模式切换保障灵活性与一致性。

**更新** 本版本重点修复了二进制坐标列表字段的类型匹配问题，将条件从检查长度4改为接受长度4和长度2的数组，改进了字段解析的准确性。建议在提交阶段补充更严格的校验与错误收集机制，并通过缓存与批量处理优化性能。
...
- 常见验证场景与解决方案
  ...
  - **更新** 位置数组验证：支持长度为2和4的整型数组，增强坐标格式兼容性。
...
- 扩展与最佳实践
  ...
  - **更新** 位置数组处理：在自定义验证中充分利用改进的位置数组验证逻辑。
