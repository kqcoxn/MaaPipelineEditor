...
## 更新摘要
**变更内容**
- 新增对 MaaFramework 资源包的扫描与管理能力，扩展了对 pipeline、image、model 目录的识别，以及资源包索引的维护机制
- 新增资源服务与资源包协议处理器，负责资源包扫描、图片索引与事件广播
- 新增 MFW 资源管理器，支持资源包加载、获取、卸载与全局卸载
- 更新了项目结构与核心组件，以反映新增的资源扫描与 MFW 集成
- 新增了关于资源包识别规则、图片索引策略与事件传播的详细组件分析
...
- 配置文件自动创建机制
- 新增：MaaFramework 资源包扫描与管理、pipeline/image/model 目录识别、资源包索引维护、图片索引与事件广播
...
- 资源服务与协议处理器
  ...
  - 参考：[LocalBridge/internal/service/resource/resource_service.go](file://LocalBridge/internal/service/resource/resource_service.go#L1-L337)、[LocalBridge/internal/protocol/resource/handler.go](file://LocalBridge/internal/protocol/resource/handler.go#L1-L200)
- MFW 服务与资源管理器
...
- 参考路径
  - [资源服务](file://LocalBridge/internal/service/resource/resource_service.go#L1-L337)
...
- 资源扫描
  ...
  - 参考：[资源服务](file://LocalBridge/internal/service/resource/resource_service.go#L1-L337)
...
## 结论
本地文件管理通过前端 WebSocket 客户端与 LocalBridge 的紧密协作，实现了从文件扫描、列表展示、打开/保存/创建到文件变更通知的完整闭环。后端采用事件驱动与模块化设计，前端通过 Store 与协议处理器实现状态同步与用户体验优化。新增的资源扫描与 MFW 集成进一步扩展了本地文件管理的能力边界，使其能够识别并索引资源包中的 pipeline、image、model 等目录，为后续的图像资源选择与流水线配置提供支撑。
