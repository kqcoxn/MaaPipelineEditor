...
- 前端
  ...
  - **新增：配置存储与JSON缩进设置：src/stores/configStore.ts**
...
FE_ConfigStore["配置存储<br/>src/stores/configStore.ts"]
...
- [src/stores/configStore.ts](file://src/stores/configStore.ts#L1-L268)
...
- **新增：配置存储与JSON缩进设置**
  - 管理全局配置，包括JSON缩进设置（默认4空格）、配置处理模式、文件自动重载等。
  - 参考：[src/stores/configStore.ts](file://src/stores/configStore.ts#L1-L268)
- 本地文件列表面板与新建文件弹窗
...
- 文件服务与扫描/监听
  ...
  - **新增：实现文件扫描深度限制和文件数量限制，防止资源耗尽**
  - **新增：实现文件监控自写检测，防止无限循环**
...
FE->>WS : "发送 /etl/save_file (含缩进配置)"
WS->>LB : "转发消息"
LB->>FS : "SaveFile(filePath, content, indent)"
FS->>FS : "自写检测忽略自身写入事件"
FS-->>LB : "成功"
...
### **新增：配置存储与JSON缩进设置**
- 字段：configs（包含jsonIndent: number，默认4）、status（界面状态）
- 配置项：jsonIndent（JSON缩进空格数，默认4空格）、configHandlingMode（配置处理模式）、fileAutoReload（文件自动重载）
- 行为：提供配置读取、设置、导出功能，支持不同配置分类管理
- **新增功能**：JSON缩进配置，支持自定义缩进空格数（0表示不缩进，其他数值表示空格数）
- 参考路径
  - [配置存储定义](file://src/stores/configStore.ts#L1-L268)
...
### 文件扫描深度限制和文件数量限制
- **实现原理**：扫描器支持最大扫描深度和最大文件数量限制，防止资源耗尽
- **默认配置**：max_depth=10（10层深度限制），max_files=10000（10000个文件限制）
- **性能收益**：避免大规模目录扫描导致的内存和CPU资源耗尽
- **实现细节**：
  - 深度限制：通过计算当前目录相对于根目录的深度来判断是否超出限制
  - 文件数量限制：在遍历过程中统计已发现的文件数量，达到限制时停止遍历
  - 截断信息：返回ScanResult结构，包含Truncated标志和LimitReason说明

**章节来源**
- [LocalBridge/internal/service/file/scanner.go](file://LocalBridge/internal/service/file/scanner.go#L14-L18)
- [LocalBridge/internal/service/file/scanner.go](file://LocalBridge/internal/service/file/scanner.go#L64-L147)
- [LocalBridge/internal/config/config.go](file://LocalBridge/internal/config/config.go#L109-L112)

### 文件监控自写检测
- **实现原理**：文件服务记录最近写入的文件时间戳，在处理文件变化事件时忽略自身写入的事件
- **检测机制**：使用recentlyWrittenFiles映射记录文件写入时间，selfWriteIgnoreWindow设置2秒忽略窗口
- **性能收益**：防止保存操作触发的文件变化事件再次触发保存，避免无限循环
- **实现细节**：
  - 写入前记录文件时间戳
  - 处理文件变化时检查是否在忽略窗口期内
  - 超过窗口期自动清理记录
...
**章节来源**
- [LocalBridge/internal/service/file/file_service.go](file://LocalBridge/internal/service/file/file_service.go#L180-L197)
- [LocalBridge/internal/service/file/file_service.go](file://LocalBridge/internal/service/file/file_service.go#L278-L294)
...
### **新增：JSON缩进配置选项**
- **实现原理**：保存请求支持indent参数，控制JSON序列化的缩进空格数
- **默认配置**：jsonIndent=4（4个空格缩进）
- **配置选项**：
  - indent=0：不进行缩进（紧凑格式）
  - indent>0：使用指定数量的空格进行缩进
  - 默认值：4个空格
- **前端集成**：从configStore获取jsonIndent配置，传递给后端保存请求
- **性能考虑**：较大的缩进值会增加文件体积，但提高可读性

**章节来源**
- [LocalBridge/pkg/models/message.go](file://LocalBridge/pkg/models/message.go#L52-L56)
- [LocalBridge/internal/protocol/file/file_handler.go](file://LocalBridge/internal/protocol/file/file_handler.go#L149-L156)
- [src/stores/configStore.ts](file://src/stores/configStore.ts#L179-L179)
...
FE_ConfigStore["configStore.ts"] --> FE_FileStore
...
- **新增：配置管理优化**
  - 配置存储支持分类管理，避免不必要的配置更新
  - JSON缩进设置支持热更新，无需重启服务
...
- 打开/保存失败
  ...
  - **新增：检查JSON缩进配置是否正确，避免缩进值过大导致序列化失败**
...
- **新增：性能相关问题排查**
  ...
  - **新增：扫描限制问题**：检查max_depth和max_files配置是否过小导致扫描截断
  - **新增：自写检测问题**：确认recentlyWrittenFiles映射是否正确清理
  - **新增：JSON缩进问题**：检查indent参数是否正确传递和处理
...
**本次性能优化主要体现在以下几个方面**：
- **文件扫描深度限制和文件数量限制**：防止大规模目录扫描导致的资源耗尽
- **文件监控自写检测**：防止保存操作触发的无限循环
- **保存功能JSON缩进配置**：支持自定义JSON格式化选项
...
- 配置项
  - server.host/port、file.root/exclude/extensions/max_depth/max_files、log.level/dir/push_to_client、maafw.enabled/lib_dir
  - **新增：文件扫描深度限制（max_depth，默认10）**
  - **新增：文件数量限制（max_files，默认10000）**
  - **新增：JSON缩进配置（jsonIndent，默认4）**
...
- **新增：性能优化配置**
  ...
  - **新增：扫描限制配置**：max_depth和max_files参数，防止资源耗尽
  - **新增：自写检测配置**：selfWriteIgnoreWindow参数，防止无限循环
  - **新增：JSON缩进配置**：jsonIndent参数，控制保存文件的格式化选项