...
**更新** 新增对 WithPseudoMinimize 系列截图方法的支持，包括 FramePool (伪最小化) 和 PrintWindow (伪最小化) 两种新选项，显著提升对最小化应用的截图兼容性。
...
**更新** WithPseudoMinimize 系列截图方法的引入显著提升了对最小化应用的截图兼容性，通过 FramePool (伪最小化) 和 PrintWindow (伪最小化) 两种新选项，用户可以更可靠地捕获最小化状态下的应用画面，这对于自动化测试和屏幕录制场景具有重要意义。这一改进增强了连接面板对各种应用状态的适应能力，为用户提供了更强大的截图工具集。
