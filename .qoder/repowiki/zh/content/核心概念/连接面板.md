...
## 更新摘要
**变更内容**
- 增强边删除功能的顺序处理机制，确保删除边后同源同类型边的顺序一致性
- 改进边管理逻辑，优化删除边时的标签递减处理
- 新增 WithPseudoMinimize 系列截图方法支持，包括 FramePool (伪最小化) 和 PrintWindow (伪最小化) 两种新选项
- 提升对最小化应用的截图兼容性，增强对 Win32 窗口的截图能力
- 优化截图方法配置，支持更多截图技术选项
...
**更新** 增强了边删除功能的顺序处理机制，确保删除边后同源同类型边的顺序一致性。新增对 WithPseudoMinimize 系列截图方法的支持，包括 FramePool (伪最小化) 和 PrintWindow (伪最小化) 两种新选项，显著提升对最小化应用的截图兼容性。
...
- [MFWProtocol.ts:1-574](file://src/services/protocols/MFWProtocol.ts#L1-L574)
- [server.ts:1-374](file://src/services/server.ts#L1-L374)
- [edgeSlice.ts:1-222](file://src/stores/flow/slices/edgeSlice.ts#L1-L222)
- [edgeLinker.ts:1-159](file://src/core/parser/edgeLinker.ts#L1-L159)
...
- [PanelConfigSection.tsx:1-459](file://src/components/panels/config/PanelConfigSection.tsx#L1-L459)
...
- 连接面板（ConnectionPanel）
  ...
  - **新增** WithPseudoMinimize 系列截图方法支持，包括 FramePool (伪最小化) 和 PrintWindow (伪最小化)
  - 提供刷新、连接、断开、切换连接等操作
  - 展示连接状态徽章与错误提示
...
- 边切片（edgeSlice）
  - 管理边集合、边顺序、边属性
  - 在添加边时进行冲突检查与顺序计算
  - **更新** 在删除边时维护同源同类型边的顺序一致性，确保删除后标签连续性
- 边渲染（edges.tsx）
  - 根据源句柄类型渲染不同样式的边与标签
  - 支持边控制点拖拽和路径调整
  - 优化边标签显示性能配置
...
**图表来源**
- [ConnectionPanel.tsx:240-324](file://src/components/panels/main/ConnectionPanel.tsx#L240-L324)
- [MFWProtocol.ts:300-417](file://src/services/protocols/MFWProtocol.ts#L300-L417)
- [server.ts:105-252](file://src/services/server.ts#L105-L252)
- [mfwStore.ts:102-158](file://src/stores/mfwStore.ts#L102-L158)
...
- 新增功能
  ...
  - **新增** WithPseudoMinimize 系列截图方法：通过伪最小化技术提升对最小化应用的截图兼容性
...
Choose --> Type{"设备类型"}
Type --> |ADB| ADBConfig["ADB设备配置<br/>多选截图/输入方法<br/>支持伪最小化截图方法"]
Type --> |Win32| Win32Config["Win32窗口配置<br/>单选截图/输入方法<br/>支持伪最小化截图方法"]
...
**图表来源**
- [ConnectionPanel.tsx:167-324](file://src/components/panels/main/ConnectionPanel.tsx#L167-L324)
- [MFWProtocol.ts:300-417](file://src/services/protocols/MFWProtocol.ts#L300-L417)
- [mfwStore.ts:102-158](file://src/stores/mfwStore.ts#L102-L158)
...
Insert --> Save["保存历史"]
Save --> Exit(["结束"])
Delete(["删除边"]) --> Find["查找被删除边"]
Find --> Decrease["同源同类型边标签递减"]
Decrease --> Update["更新边集合"]
Update --> SaveDel["保存历史"]
SaveDel --> ExitDel(["结束"])
```

**图表来源**
- [edgeSlice.ts:150-209](file://src/stores/flow/slices/edgeSlice.ts#L150-L209)
...
**图表来源**
- [edgeLinker.ts:37-159](file://src/core/parser/edgeLinker.ts#L37-L159)
...
**图表来源**
- [EdgePanel.tsx:130-281](file://src/components/panels/main/EdgePanel.tsx#L130-L281)
- [edgeSlice.ts:101-147](file://src/stores/flow/slices/edgeSlice.ts#L101-L147)
...
- 边顺序计算
  ...
  - **更新** 删除边时仅对同源同类型边进行 label 调整，避免全量重排
  - 边标签显示可通过配置开关优化性能
- 渲染优化
...
- 新增设备类型优化
  ...
  - **新增** WithPseudoMinimize 系列截图方法通过伪最小化技术提升截图效率，减少对应用状态的干扰

**章节来源**
- [ConnectionPanel.tsx:209-238](file://src/components/panels/main/ConnectionPanel.tsx#L209-L238)
- [edgeSlice.ts:150-209](file://src/stores/flow/slices/edgeSlice.ts#L150-L209)
- [edges.tsx:472-496](file://src/components/flow/edges.tsx#L472-L496)
- [PanelConfigSection.tsx:425-453](file://src/components/panels/config/PanelConfigSection.tsx#L425-L453)
...
- 无法连接设备
  ...
  - **新增** 对于最小化应用截图问题，尝试使用 WithPseudoMinimize 系列截图方法
- 设备列表为空
  - 点击"刷新"重新获取设备列表
...
- 性能问题
  ...
  - 检查系统资源使用情况
- **新增** 截图兼容性问题
  - 尝试使用 WithPseudoMinimize 系列截图方法替代标准截图方法
  - 确保应用支持伪最小化技术
  - 检查应用的窗口状态和可见性设置
...
- [edgeSlice.ts:150-209](file://src/stores/flow/slices/edgeSlice.ts#L150-L209)
- [PanelConfigSection.tsx:127-152](file://src/components/panels/config/PanelConfigSection.tsx#L127-L152)

## 结论
连接面板通过清晰的设备选择与方法配置，结合协议层与状态层的协作，实现了稳定的多设备连接体验；同时，流程图中的边管理与渲染确保了连接语义的直观表达与可控性。新增的 PlayCover 和 Gamepad 支持进一步扩展了应用的设备兼容性，而面板配置选项则为用户提供了灵活的性能优化手段。

**更新** 增强的边删除功能确保了删除边后同源同类型边的顺序一致性，通过优化删除前处理逻辑和标签递减机制，避免了删除操作对其他边顺序的影响。WithPseudoMinimize 系列截图方法的引入显著提升了对最小化应用的截图兼容性，通过 FramePool (伪最小化) 和 PrintWindow (伪最小化) 两种新选项，用户可以更可靠地捕获最小化状态下的应用画面，这对于自动化测试和屏幕录制场景具有重要意义。这一改进增强了连接面板对各种应用状态的适应能力，为用户提供了更强大的截图工具集。

整体设计在易用性、扩展性和性能之间取得良好平衡，便于后续增加新的设备类型与连接能力。