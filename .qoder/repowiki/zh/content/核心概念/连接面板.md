# 连接面板

<cite>
**本文引用的文件**
- [ConnectionPanel.tsx](file://src/components/panels/ConnectionPanel.tsx)
- [mfwStore.ts](file://src/stores/mfwStore.ts)
- [MFWProtocol.ts](file://src/services/protocols/MFWProtocol.ts)
- [server.ts](file://src/services/server.ts)
- [edgeSlice.ts](file://src/stores/flow/slices/edgeSlice.ts)
- [edgeLinker.ts](file://src/core/parser/edgeLinker.ts)
- [edges.tsx](file://src/components/flow/edges.tsx)
- [constants.ts](file://src/components/flow/nodes/constants.ts)
- [edgeUtils.ts](file://src/stores/flow/utils/edgeUtils.ts)
- [EdgePanel.tsx](file://src/components/panels/EdgePanel.tsx)
</cite>

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖分析](#依赖分析)
7. [性能考虑](#性能考虑)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)

## 简介
连接面板用于在可视化编辑器中选择并连接“控制器”（ADB 设备或 Win32 窗口），并配置截图与输入方法。它通过协议层与本地服务进行交互，实现设备发现、连接建立、断开以及状态反馈；同时支持在流程图中对“连接”（边）进行顺序与属性配置，确保流程控制语义清晰、可维护性强。

## 项目结构
连接面板位于组件层，配合状态存储、协议层与流程图渲染共同工作：
- 组件层：连接面板负责用户交互与设备选择
- 状态层：MFWStore 维护连接状态、设备列表与错误信息
- 协议层：MFWProtocol 通过 WebSocket 与本地服务通信，下发设备刷新、创建控制器、断开等指令，并接收设备列表与连接结果
- 流程图层：边的添加、删除、顺序调整与样式渲染

```mermaid
graph TB
subgraph "界面层"
CP["连接面板<br/>ConnectionPanel.tsx"]
EP["连接设置面板<br/>EdgePanel.tsx"]
end
subgraph "状态层"
MS["MFWStore<br/>mfwStore.ts"]
end
subgraph "协议层"
MP["MFWProtocol<br/>MFWProtocol.ts"]
WS["LocalWebSocketServer<br/>server.ts"]
end
subgraph "流程图层"
ES["边切片<br/>edgeSlice.ts"]
EL["边链接器<br/>edgeLinker.ts"]
ED["边渲染<br/>edges.tsx"]
EC["句柄类型常量<br/>constants.ts"]
EU["边工具<br/>edgeUtils.ts"]
end
CP --> MS
CP --> MP
MP --> WS
EP --> ES
ES --> ED
ES --> EC
ES --> EU
EL --> ES
```

图表来源
- [ConnectionPanel.tsx](file://src/components/panels/ConnectionPanel.tsx#L1-L681)
- [mfwStore.ts](file://src/stores/mfwStore.ts#L1-L134)
- [MFWProtocol.ts](file://src/services/protocols/MFWProtocol.ts#L1-L399)
- [server.ts](file://src/services/server.ts#L1-L298)
- [edgeSlice.ts](file://src/stores/flow/slices/edgeSlice.ts#L1-L275)
- [edgeLinker.ts](file://src/core/parser/edgeLinker.ts#L1-L159)
- [edges.tsx](file://src/components/flow/edges.tsx#L1-L70)
- [constants.ts](file://src/components/flow/nodes/constants.ts#L1-L14)
- [edgeUtils.ts](file://src/stores/flow/utils/edgeUtils.ts#L1-L45)
- [EdgePanel.tsx](file://src/components/panels/EdgePanel.tsx#L1-L209)

章节来源
- [ConnectionPanel.tsx](file://src/components/panels/ConnectionPanel.tsx#L1-L681)
- [mfwStore.ts](file://src/stores/mfwStore.ts#L1-L134)
- [MFWProtocol.ts](file://src/services/protocols/MFWProtocol.ts#L1-L399)
- [server.ts](file://src/services/server.ts#L1-L298)
- [edgeSlice.ts](file://src/stores/flow/slices/edgeSlice.ts#L1-L275)
- [edgeLinker.ts](file://src/core/parser/edgeLinker.ts#L1-L159)
- [edges.tsx](file://src/components/flow/edges.tsx#L1-L70)
- [constants.ts](file://src/components/flow/nodes/constants.ts#L1-L14)
- [edgeUtils.ts](file://src/stores/flow/utils/edgeUtils.ts#L1-L45)
- [EdgePanel.tsx](file://src/components/panels/EdgePanel.tsx#L1-L209)

## 核心组件
- 连接面板（ConnectionPanel）
  - 提供 ADB 设备与 Win32 窗口两类设备的选择与连接入口
  - 支持截图方法与输入方法的自定义选择
  - 提供刷新、连接、断开、切换连接等操作
  - 展示连接状态徽章与错误提示
- MFWStore（mfwStore）
  - 维护连接状态、控制器类型与 ID、设备信息、设备列表与错误信息
  - 提供更新设备列表、设置连接状态、清除连接等接口
- MFWProtocol（MFWProtocol）
  - 注册设备列表与控制器状态路由
  - 下发刷新设备列表、创建控制器、断开控制器等指令
  - 接收设备列表与连接结果，更新状态存储
- 边切片（edgeSlice）
  - 管理边集合、边顺序、边属性
  - 在添加边时进行冲突检查与顺序计算
  - 在删除边时维护同源同类型边的顺序一致性
- 边链接器（edgeLinker）
  - 解析节点引用字符串，支持锚点与回跳前缀
  - 生成外部节点或锚点节点，创建边并附带属性
- 边渲染（edges.tsx）
  - 根据源句柄类型渲染不同样式的边与标签
- 边工具（edgeUtils）
  - 提供查找边、筛选选中边、计算链接顺序等工具
- 连接设置面板（EdgePanel）
  - 展示当前选中边的源/目标节点、连接类型与顺序
  - 支持修改边顺序与删除边

章节来源
- [ConnectionPanel.tsx](file://src/components/panels/ConnectionPanel.tsx#L1-L681)
- [mfwStore.ts](file://src/stores/mfwStore.ts#L1-L134)
- [MFWProtocol.ts](file://src/services/protocols/MFWProtocol.ts#L1-L399)
- [edgeSlice.ts](file://src/stores/flow/slices/edgeSlice.ts#L1-L275)
- [edgeLinker.ts](file://src/core/parser/edgeLinker.ts#L1-L159)
- [edges.tsx](file://src/components/flow/edges.tsx#L1-L70)
- [edgeUtils.ts](file://src/stores/flow/utils/edgeUtils.ts#L1-L45)
- [EdgePanel.tsx](file://src/components/panels/EdgePanel.tsx#L1-L209)

## 架构总览
连接面板与流程图的交互路径如下：
- 用户在连接面板选择设备并配置方法后，点击“连接设备”，触发协议层发送创建控制器指令
- 协议层接收设备列表与连接结果，更新状态存储
- 用户在流程图中拖拽边时，边切片根据句柄类型与现有边集合进行冲突检查与顺序计算
- 边渲染根据句柄类型与选中状态渲染不同样式与标签

```mermaid
sequenceDiagram
participant U as "用户"
participant CP as "连接面板<br/>ConnectionPanel"
participant MP as "MFW协议<br/>MFWProtocol"
participant WS as "本地WS服务<br/>LocalWebSocketServer"
participant MS as "状态存储<br/>MFWStore"
U->>CP : 选择设备/方法并点击“连接设备”
CP->>MP : createAdbController()/createWin32Controller()
MP->>WS : 发送创建控制器指令
WS-->>MP : 返回控制器创建结果
MP->>MS : 更新连接状态/控制器信息/错误信息
MP-->>CP : 状态变更通知
```

图表来源
- [ConnectionPanel.tsx](file://src/components/panels/ConnectionPanel.tsx#L192-L246)
- [MFWProtocol.ts](file://src/services/protocols/MFWProtocol.ts#L232-L315)
- [server.ts](file://src/services/server.ts#L223-L237)
- [mfwStore.ts](file://src/stores/mfwStore.ts#L88-L133)

## 详细组件分析

### 连接面板（ConnectionPanel）
- 设备选择与方法配置
  - ADB 设备列表与 Win32 窗口列表分别渲染，支持点击选择
  - 截图方法与输入方法下拉框支持“自动选择”或手动指定
  - 当切换设备或首次打开面板时，自动刷新对应设备列表
- 连接控制
  - “连接设备”按钮在满足条件时启用（已选中设备且方法有效）
  - “断开连接”按钮在已连接时可用
  - “连接新设备”在当前设备非已连接设备时可用
- 状态展示
  - 连接状态徽章随状态变化
  - 已连接时展示当前设备信息
  - 出错时展示错误提示

```mermaid
flowchart TD
Start(["打开连接面板"]) --> Init["初始化状态<br/>设置Tab/选中设备"]
Init --> Refresh{"是否已连接？"}
Refresh --> |是| Skip["跳过刷新"]
Refresh --> |否| DoRefresh["触发刷新设备列表"]
DoRefresh --> Wait["等待刷新完成"]
Wait --> Ready["准备就绪"]
Ready --> Choose{"选择设备/方法"}
Choose --> Connect{"点击“连接设备”"}
Connect --> Valid{"方法有效？"}
Valid --> |否| Warn["提示方法无效"]
Valid --> |是| Send["发送创建控制器指令"]
Send --> Status["等待连接结果"]
Status --> Done["更新状态存储"]
Done --> End(["关闭/保持"])
```

图表来源
- [ConnectionPanel.tsx](file://src/components/panels/ConnectionPanel.tsx#L127-L246)
- [MFWProtocol.ts](file://src/services/protocols/MFWProtocol.ts#L232-L315)
- [mfwStore.ts](file://src/stores/mfwStore.ts#L88-L133)

章节来源
- [ConnectionPanel.tsx](file://src/components/panels/ConnectionPanel.tsx#L1-L681)

### 边添加与顺序管理（edgeSlice）
- 冲突检查
  - 对 next、jump_back、on_error 三类句柄在同一源节点上的组合进行互斥检查
  - on_error 自环（源=目标）直接拒绝
- 顺序计算
  - 根据现有边集合与句柄类型计算新边的 label
  - next 与 jump_back 共享同一组顺序
- 删除处理
  - 删除边后，同源同类型边 label 递减，保证连续性

```mermaid
flowchart TD
Enter(["添加边"]) --> Check["冲突检查"]
Check --> Crash{"是否存在冲突？"}
Crash --> |是| Abort["取消添加"]
Crash --> |否| Calc["计算顺序"]
Calc --> Insert["插入新边"]
Insert --> Save["保存历史"]
Save --> Exit(["结束"])
```

图表来源
- [edgeSlice.ts](file://src/stores/flow/slices/edgeSlice.ts#L172-L268)
- [edgeUtils.ts](file://src/stores/flow/utils/edgeUtils.ts#L17-L45)
- [constants.ts](file://src/components/flow/nodes/constants.ts#L1-L6)

章节来源
- [edgeSlice.ts](file://src/stores/flow/slices/edgeSlice.ts#L1-L275)
- [edgeUtils.ts](file://src/stores/flow/utils/edgeUtils.ts#L1-L45)
- [constants.ts](file://src/components/flow/nodes/constants.ts#L1-L14)

### 边渲染与样式（edges.tsx）
- 根据源句柄类型渲染不同样式类
- 选中状态与标签显示受配置影响
- 使用贝塞尔曲线绘制路径与标签定位

```mermaid
classDiagram
class MarkedEdge {
+props : EdgeProps
+render()
}
class EdgeTypes {
+marked : MarkedEdge
}
MarkedEdge --> EdgeTypes : "导出"
```

图表来源
- [edges.tsx](file://src/components/flow/edges.tsx#L1-L70)

章节来源
- [edges.tsx](file://src/components/flow/edges.tsx#L1-L70)

### 节点引用与外部节点（edgeLinker）
- 支持字符串与对象两种节点引用格式
- 字符串前缀支持 [Anchor] 与 [JumpBack]
- 不存在的目标节点会创建外部节点或锚点节点，并返回新增边与节点

```mermaid
flowchart TD
Parse["解析节点引用"] --> HasPrefix{"是否包含前缀？"}
HasPrefix --> |是| Apply["提取属性并去除前缀"]
HasPrefix --> |否| Keep["保留名称"]
Apply --> TargetFound{"目标节点存在？"}
Keep --> TargetFound
TargetFound --> |是| BuildEdge["创建边"]
TargetFound --> |否| CreateNode["创建外部/锚点节点"] --> BuildEdge
BuildEdge --> Return["返回新增边/节点/映射"]
```

图表来源
- [edgeLinker.ts](file://src/core/parser/edgeLinker.ts#L37-L159)

章节来源
- [edgeLinker.ts](file://src/core/parser/edgeLinker.ts#L1-L159)

### 连接设置面板（EdgePanel）
- 展示当前选中边的源/目标节点名称、连接类型与顺序
- 提供顺序输入框与最大值限制
- 支持删除当前边

```mermaid
sequenceDiagram
participant U as "用户"
participant EP as "连接设置面板<br/>EdgePanel"
participant ES as "边切片<br/>edgeSlice"
U->>EP : 选中一条边
EP->>ES : setEdgeLabel(id, value)
ES-->>EP : 更新成功
U->>EP : 点击删除
EP->>ES : updateEdges(remove)
ES-->>EP : 删除完成
```

图表来源
- [EdgePanel.tsx](file://src/components/panels/EdgePanel.tsx#L90-L209)
- [edgeSlice.ts](file://src/stores/flow/slices/edgeSlice.ts#L112-L171)

章节来源
- [EdgePanel.tsx](file://src/components/panels/EdgePanel.tsx#L1-L209)
- [edgeSlice.ts](file://src/stores/flow/slices/edgeSlice.ts#L112-L171)

## 依赖分析
- 组件与状态
  - ConnectionPanel 依赖 MFWStore 的连接状态、设备列表与错误信息
  - EdgePanel 依赖 FlowStore 的边集合与节点集合
- 协议与服务
  - MFWProtocol 依赖 LocalWebSocketServer 发送/接收消息
  - MFWProtocol 注册设备列表与控制器状态路由，更新 MFWStore
- 流程图与边
  - edgeSlice 依赖 constants 中的句柄类型与 edgeUtils 的顺序计算
  - edges.tsx 依赖配置存储决定标签显示

```mermaid
graph LR
CP["ConnectionPanel"] --> MS["MFWStore"]
CP --> MP["MFWProtocol"]
MP --> WS["LocalWebSocketServer"]
EP["EdgePanel"] --> ES["edgeSlice"]
ES --> EC["SourceHandleTypeEnum"]
ES --> EU["edgeUtils"]
ES --> ED["edges.tsx"]
```

图表来源
- [ConnectionPanel.tsx](file://src/components/panels/ConnectionPanel.tsx#L1-L681)
- [mfwStore.ts](file://src/stores/mfwStore.ts#L1-L134)
- [MFWProtocol.ts](file://src/services/protocols/MFWProtocol.ts#L1-L399)
- [server.ts](file://src/services/server.ts#L1-L298)
- [EdgePanel.tsx](file://src/components/panels/EdgePanel.tsx#L1-L209)
- [edgeSlice.ts](file://src/stores/flow/slices/edgeSlice.ts#L1-L275)
- [constants.ts](file://src/components/flow/nodes/constants.ts#L1-L14)
- [edgeUtils.ts](file://src/stores/flow/utils/edgeUtils.ts#L1-L45)
- [edges.tsx](file://src/components/flow/edges.tsx#L1-L70)

章节来源
- [ConnectionPanel.tsx](file://src/components/panels/ConnectionPanel.tsx#L1-L681)
- [MFWProtocol.ts](file://src/services/protocols/MFWProtocol.ts#L1-L399)
- [server.ts](file://src/services/server.ts#L1-L298)
- [edgeSlice.ts](file://src/stores/flow/slices/edgeSlice.ts#L1-L275)
- [edges.tsx](file://src/components/flow/edges.tsx#L1-L70)
- [constants.ts](file://src/components/flow/nodes/constants.ts#L1-L14)
- [edgeUtils.ts](file://src/stores/flow/utils/edgeUtils.ts#L1-L45)
- [EdgePanel.tsx](file://src/components/panels/EdgePanel.tsx#L1-L209)

## 性能考虑
- 设备列表刷新
  - 刷新操作仅在面板打开且未连接时触发，避免不必要的网络请求
  - 刷新完成后延迟关闭加载状态，减少频繁刷新
- 边顺序计算
  - 顺序计算基于现有边集合遍历，复杂度与边数量线性相关
  - 删除边时仅对同源同类型边进行 label 调整，避免全量重排
- 渲染优化
  - 连接面板使用记忆化与条件渲染，减少不必要的重绘
  - 边渲染根据选中状态与句柄类型动态选择样式类，避免冗余 DOM 操作

[本节为通用建议，无需特定文件引用]

## 故障排查指南
- 无法连接设备
  - 检查本地服务是否运行且端口可用
  - 确认协议版本匹配，不匹配会导致握手失败
  - 查看错误提示，确认设备方法是否有效
- 设备列表为空
  - 点击“刷新”重新获取设备列表
  - 确认设备驱动/权限正确（ADB/Win32）
- 连接后断开
  - 检查控制器状态路由是否收到断开通知
  - 确认控制器 ID 与设备信息一致
- 边顺序异常
  - 删除边后顺序可能发生变化，重新设置顺序
  - 确认 next/jump_back 共享顺序规则

章节来源
- [server.ts](file://src/services/server.ts#L101-L187)
- [MFWProtocol.ts](file://src/services/protocols/MFWProtocol.ts#L122-L186)
- [edgeSlice.ts](file://src/stores/flow/slices/edgeSlice.ts#L24-L71)

## 结论
连接面板通过清晰的设备选择与方法配置，结合协议层与状态层的协作，实现了稳定的设备连接体验；同时，流程图中的边管理与渲染确保了连接语义的直观表达与可控性。整体设计在易用性与扩展性之间取得平衡，便于后续增加新的设备类型与连接能力。