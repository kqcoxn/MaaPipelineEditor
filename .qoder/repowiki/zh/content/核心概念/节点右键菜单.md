...
## 更新摘要
**变更内容**   
- 新增便签内容复制功能：支持右键复制便签节点的内容到剪贴板
- 新增便签颜色选择功能：支持通过右键菜单为便签节点选择不同颜色主题
- 新增分组颜色选择功能：支持为分组节点设置颜色主题
- 更新颜色处理：使用统一的剪贴板助手类进行内容复制
- 完善节点类型支持：增强对 Sticker 和 Group 节点的右键菜单支持
...
## 节点类型支持
节点右键菜单支持多种节点类型，包括 Pipeline 节点、External 节点、Anchor 节点、Sticker 便签节点和 Group 分组节点。不同类型的节点具有不同的菜单选项，例如只有 Pipeline 节点可以保存为模板。系统通过 NodeTypeEnum 枚举来定义和区分这些节点类型，并在菜单配置生成过程中根据节点类型调整可用的菜单项。
...
**更新** 新增颜色选择功能，支持便签节点和分组节点的颜色主题设置

节点右键菜单现在支持为不同类型的节点设置颜色主题，提供更直观的视觉定制体验。

### 便签节点颜色选择
便签节点支持五种颜色主题：
- **黄色**：#fff9c4 背景，#f9e066 边框
- **绿色**：#e8f5e9 背景，#81c784 边框  
- **蓝色**：#e3f2fd 背景，#64b5f6 边框
- **粉色**：#fce4ec 背景，#f06292 边框
- **紫色**：#f3e5f5 背景，#ba68c8 边框

### 分组节点颜色选择
分组节点同样支持五种颜色主题，与便签节点保持一致的颜色方案。

### 颜色选择实现
颜色选择功能通过子菜单实现，用户可以通过右键菜单直接选择所需的颜色主题。颜色设置会立即应用到节点上，并保存到历史记录中以便撤销操作。
...
**更新** 新增内容复制功能，支持便签节点内容和节点名的快速复制

节点右键菜单现在提供多种内容复制选项，提升用户的操作效率。

### 便签内容复制
便签节点支持直接复制其内容到系统剪贴板。用户只需右键点击便签节点，选择"复制便签内容"即可完成复制操作。该功能使用统一的剪贴板助手类，提供更好的错误处理和用户反馈。

### 节点名复制
所有节点类型都支持复制节点名功能。对于 Pipeline 节点，复制的名称会自动包含文件前缀，便于在调试环境中识别。

### Reco JSON 复制
Pipeline 节点支持复制识别配置的 JSON 格式内容。该功能使用编译器解析节点配置，提取识别相关的参数并格式化输出。

### 复制功能实现
所有复制功能都通过统一的剪贴板助手类实现，提供一致的成功提示和错误处理机制。复制操作会自动处理不同类型的数据（字符串、对象、JSON）并进行适当的格式化。
