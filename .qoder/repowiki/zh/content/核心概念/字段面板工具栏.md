...
## 更新摘要
**所做更改**
- 新增字段快捷工具系统重大重构章节，详细介绍基于 QuickToolConfig 接口的分类系统
- 新增六种工具类型（roi、roi_offset、ocr、template、color、delta）的详细说明
- 更新工具箱面板架构，从简单图标映射升级为分类系统
- 新增工具配置接口和处理逻辑的详细分析
- 更新字段面板工具栏与工具箱的交互关系
...
## 简介
字段面板工具栏位于"字段面板"的顶部区域，提供与当前节点相关的便捷操作入口。本次更新重点介绍了字段快捷工具系统的重大重构：从简单的图标映射升级为基于 QuickToolConfig 接口的分类系统，新增了 roi、roi_offset、ocr、template、color、delta 六种工具类型的分类和处理逻辑。系统现在支持更精细的工具分类、统一的配置接口和增强的用户体验。
...
**更新** 本次重大重构引入了基于 QuickToolConfig 接口的字段快捷工具系统，从简单的图标映射升级为分类管理系统，新增了 roi、roi_offset、ocr、template、color、delta 六种工具类型的完整支持，实现了工具的统一配置、分类管理和增强的用户体验。
