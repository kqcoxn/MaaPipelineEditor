...
`JsonViewer` 组件基于 `@microlink/react-json-view` 实现，提供直观的 JSON 数据可视化展示。其核心交互特性包括：
...
- **实时预览模式**：当启用“实时预览”配置时，面板会根据当前选中的节点和边动态编译并展示局部流程的 JSON 结构。

面板顶部提供多个操作按钮，支持从剪贴板导入、文件导入、部分导出、全部导出以及应用到本地等功能，满足不同场景下的使用需求。

```mermaid
flowchart TD
A[JsonViewer 组件] --> B[语法高亮]
A --> C[折叠展开]
A --> D[自动折叠配置项]
A --> E[实时预览]
A --> F[导入导出操作]
...
participant JsonViewer as JsonViewer组件
...
- **`$__mpe_config_`**: 配置节点前缀，用于存储文件元信息。
- **`$__mpe_code`**: 配置标记，其下的 `filename` 和 `version` 字段记录了文件名和编辑器版本。
