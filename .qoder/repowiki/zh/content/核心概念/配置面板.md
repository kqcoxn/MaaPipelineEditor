...
- [LiveScreenPanel.module.less](file://src/styles/LiveScreenPanel.module.less)
</cite>

## 更新摘要
**所做更改**
- 新增实时屏幕预览配置选项章节
- 更新面板配置模块以包含实时屏幕预览相关设置
- 更新配置状态管理部分以包含新的实时屏幕配置项
- 更新配置面板结构图以包含实时屏幕预览功能
...
**更新** 新增 `enableLiveScreen` 和 `liveScreenRefreshRate` 配置项，用于控制实时屏幕预览功能。
...
**新增** 实时屏幕预览功能为用户提供设备屏幕的实时预览能力，支持性能优化和懒加载机制。

### 功能概述

实时屏幕预览功能允许用户在编辑器中实时查看连接设备的屏幕内容，当字段面板、连接面板或 JSON 面板打开时自动隐藏，以避免遮挡工作区域。
...
LiveScreenPanel --> LiveScreenConfig : "使用"
```
...
1. **页面可见性监控**：通过监听 `visibilitychange` 事件，在页面不可见时自动暂停截图请求
2. **懒加载功能**：当面板不可见时停止所有网络请求和定时器
3. **智能定时器管理**：自动清理定时器防止内存泄漏
4. **条件渲染**：仅在满足条件时才显示和更新屏幕内容

### 配置面板集成

实时屏幕预览功能在配置面板中提供两个主要配置项：

1. **实时画面预览**：启用/禁用实时屏幕预览功能
2. **画面刷新间隔(ms)**：设置设备屏幕更新间隔，范围 200-5000ms，默认 1000ms
...
**更新** 新增的实时屏幕预览功能显著提升了用户体验，为开发者提供了直观的设备屏幕监控能力。该功能包含完善的性能优化机制，包括页面可见性监控、懒加载和智能定时器管理，确保在提供实时预览的同时保持应用的流畅运行。通过配置面板中的两个主要设置项，用户可以灵活控制实时屏幕预览的启用状态和刷新频率，平衡性能和体验需求。
