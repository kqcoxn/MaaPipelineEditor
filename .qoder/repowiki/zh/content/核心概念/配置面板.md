...
Pipeline 配置模块提供对 Pipeline 相关设置的控制，主要涉及节点属性的导出形式。
...
配置面板是 MaaPipelineEditor 的核心设置界面，通过模块化设计提供了全面的配置选项。面板采用 Zustand 进行状态管理，确保配置的集中管理和响应式更新。通过 WebSocket 与后端服务通信，实现了本地服务配置的实时查看和修改。配置的导出/导入功能支持用户在不同环境间迁移设置，提高了使用灵活性。整体设计注重用户体验，通过清晰的分组和提示信息帮助用户快速完成配置。
