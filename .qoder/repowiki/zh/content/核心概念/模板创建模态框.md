# 模板创建模态框

<cite>
**本文引用的文件**
- [TemplateModal.tsx](file://src/components/modals/TemplateModal.tsx)
- [ScreenshotModalBase.tsx](file://src/components/modals/ScreenshotModalBase.tsx)
- [ParamFieldListElem.tsx](file://src/components/panels/field/items/ParamFieldListElem.tsx)
- [FieldPanelToolbar.tsx](file://src/components/panels/field/tools/FieldPanelToolbar.tsx)
- [customTemplateStore.ts](file://src/stores/customTemplateStore.ts)
- [nodeTemplates.ts](file://src/data/nodeTemplates.ts)
- [NodeAddPanel.tsx](file://src/components/panels/main/NodeAddPanel.tsx)
- [index.ts](file://src/components/modals/index.ts)
- [iconfonts/index.tsx](file://src/components/iconfonts/index.tsx)
- [roiNegativeCoord.ts](file://src/utils/roiNegativeCoord.ts)
</cite>

## 更新摘要
**变更内容**
- 新增ROI坐标转换支持，提供更直观的模板创建体验
- 增强负数坐标处理能力，支持从右边缘和下边缘计算坐标
- 添加ROI分割显示功能，支持超出边界的区域处理
- 优化坐标输入提示和可视化反馈

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [ROI坐标转换支持](#roi坐标转换支持)
7. [依赖关系分析](#依赖关系分析)
8. [性能考量](#性能考量)
9. [故障排查指南](#故障排查指南)
10. [结论](#结论)

## 简介
本文件围绕"模板创建模态框"的实现进行系统化文档化，重点说明如何通过截图采集、ROI 区域框选与遮罩绘制，最终导出模板图片并回填到字段参数中；同时梳理该模态框与字段面板、节点模板系统、自定义模板存储之间的协作关系。文档面向不同技术背景的读者，既提供高层架构说明，也给出关键流程的可视化图示与定位路径。

**更新** 本次更新重点介绍了新增的ROI坐标转换支持功能，该功能显著提升了模板创建的直观性和易用性，特别是对于负数坐标的处理和超出边界的区域管理。

## 项目结构
与"模板创建模态框"直接相关的模块分布如下：
- 模态框组件：TemplateModal.tsx（基于 ScreenshotModalBase.tsx 的通用截图容器）
- ROI坐标转换工具：roiNegativeCoord.ts（提供负数坐标解析和分割处理）
- 字段面板与参数编辑：ParamFieldListElem.tsx（在字段参数中触发模板创建）
- 节点模板与自定义模板：nodeTemplates.ts、customTemplateStore.ts
- 节点添加面板：NodeAddPanel.tsx（展示与管理模板列表）
- 图标系统：iconfonts/index.tsx（提供模板相关图标）

```mermaid
graph TB
subgraph "模态框层"
SM["TemplateModal.tsx"]
SMB["ScreenshotModalBase.tsx"]
RNC["roiNegativeCoord.ts"]
end
subgraph "字段面板层"
PFLE["ParamFieldListElem.tsx"]
end
subgraph "模板系统层"
NT["nodeTemplates.ts"]
CTS["customTemplateStore.ts"]
NAP["NodeAddPanel.tsx"]
end
subgraph "图标层"
IF["iconfonts/index.tsx"]
end
PFLE --> SM
SM --> SMB
SM --> RNC
NAP --> NT
NAP --> CTS
SM --> IF
```

**图表来源**
- [TemplateModal.tsx](file://src/components/modals/TemplateModal.tsx#L1-L991)
- [ScreenshotModalBase.tsx](file://src/components/modals/ScreenshotModalBase.tsx#L1-L405)
- [roiNegativeCoord.ts](file://src/utils/roiNegativeCoord.ts#L1-L313)
- [ParamFieldListElem.tsx](file://src/components/panels/field/items/ParamFieldListElem.tsx#L1-L775)
- [nodeTemplates.ts](file://src/data/nodeTemplates.ts#L1-L108)
- [customTemplateStore.ts](file://src/stores/customTemplateStore.ts#L1-L310)
- [NodeAddPanel.tsx](file://src/components/panels/main/NodeAddPanel.tsx#L1-L583)
- [iconfonts/index.tsx](file://src/components/iconfonts/index.tsx#L1-L200)

**章节来源**
- [TemplateModal.tsx](file://src/components/modals/TemplateModal.tsx#L1-L991)
- [ScreenshotModalBase.tsx](file://src/components/modals/ScreenshotModalBase.tsx#L1-L405)
- [roiNegativeCoord.ts](file://src/utils/roiNegativeCoord.ts#L1-L313)
- [ParamFieldListElem.tsx](file://src/components/panels/field/items/ParamFieldListElem.tsx#L1-L775)
- [NodeAddPanel.tsx](file://src/components/panels/main/NodeAddPanel.tsx#L1-L583)
- [customTemplateStore.ts](file://src/stores/customTemplateStore.ts#L1-L310)
- [nodeTemplates.ts](file://src/data/nodeTemplates.ts#L1-L108)
- [iconfonts/index.tsx](file://src/components/iconfonts/index.tsx#L1-L200)

## 核心组件
- TemplateModal：提供截图采集、ROI 框选、遮罩绘制、画笔/橡皮擦、导出模板图片、回填字段参数的完整交互。
- ScreenshotModalBase：封装截图请求、缩放/平移、工具栏与确认按钮等通用能力。
- roiNegativeCoord：提供ROI负数坐标解析、分割处理和边界检测功能。
- ParamFieldListElem：在字段参数编辑界面触发 TemplateModal，并接收确认回调，将模板路径、遮罩状态与 ROI 写回字段数据。
- customTemplateStore：负责自定义模板的持久化、增删改查与版本迁移。
- nodeTemplates：内置模板集合，NodeAddPanel 展示时会合并自定义模板与预设模板。
- NodeAddPanel：模板列表展示、搜索、键盘导航、删除自定义模板。
- iconfonts：提供模板相关图标资源。

**章节来源**
- [TemplateModal.tsx](file://src/components/modals/TemplateModal.tsx#L1-L991)
- [ScreenshotModalBase.tsx](file://src/components/modals/ScreenshotModalBase.tsx#L1-L405)
- [roiNegativeCoord.ts](file://src/utils/roiNegativeCoord.ts#L1-L313)
- [ParamFieldListElem.tsx](file://src/components/panels/field/items/ParamFieldListElem.tsx#L1-L775)
- [customTemplateStore.ts](file://src/stores/customTemplateStore.ts#L1-L310)
- [nodeTemplates.ts](file://src/data/nodeTemplates.ts#L1-L108)
- [NodeAddPanel.tsx](file://src/components/panels/main/NodeAddPanel.tsx#L1-L583)
- [iconfonts/index.tsx](file://src/components/iconfonts/index.tsx#L1-L200)

## 架构总览
TemplateModal 作为"模板创建模态框"，其工作流由以下步骤构成：
1. 通过 ScreenshotModalBase 请求设备截图并渲染到画布。
2. 用户使用"框选/画笔/橡皮擦"工具完成 ROI 选择与遮罩绘制。
3. 新增的ROI坐标转换功能自动解析负数坐标，支持从右边缘和下边缘计算。
4. 点击"保存模板"，将裁剪后的截图与遮罩叠加导出为 PNG 文件，同时回传模板文件名、遮罩状态与 ROI。
5. 字段面板接收回传数据，写入对应字段参数（如模板路径、ROI、是否使用绿色遮罩）。

```mermaid
sequenceDiagram
participant U as "用户"
participant PFLE as "ParamFieldListElem"
participant TM as "TemplateModal"
participant SMB as "ScreenshotModalBase"
participant RNC as "roiNegativeCoord"
participant MFW as "mfw 协议"
participant FS as "文件系统/下载"
U->>PFLE : 触发模板创建
PFLE->>TM : 打开模态框(open=true)
TM->>SMB : 请求截图(requestScreenshot)
SMB->>MFW : 发送截图请求
MFW-->>SMB : 返回截图数据
SMB-->>TM : 渲染截图到画布
U->>TM : 框选ROI/绘制遮罩
TM->>RNC : 解析负数坐标
RNC-->>TM : 返回解析结果
U->>TM : 点击"保存模板"
TM->>FS : 导出PNG(含遮罩叠加)
FS-->>TM : 成功/失败
TM-->>PFLE : onConfirm(模板名, 遮罩状态, ROI)
PFLE-->>PFLE : 更新字段参数
```

**图表来源**
- [ParamFieldListElem.tsx](file://src/components/panels/field/items/ParamFieldListElem.tsx#L142-L154)
- [TemplateModal.tsx](file://src/components/modals/TemplateModal.tsx#L1-L991)
- [ScreenshotModalBase.tsx](file://src/components/modals/ScreenshotModalBase.tsx#L1-L405)
- [roiNegativeCoord.ts](file://src/utils/roiNegativeCoord.ts#L55-L178)

## 详细组件分析

### TemplateModal 组件
- 截图与画布渲染
  - 基于 ScreenshotModalBase 的截图请求与渲染，内部维护两层 Canvas：底图 Canvas 与遮罩 Canvas。
  - 通过 redrawCanvas 将底图与遮罩层叠加绘制，并在 ROI 选择状态下绘制矩形框与预览圆。
- 工具与状态
  - 支持"框选/画笔/橡皮擦"三种工具；画笔大小可调；支持清除遮罩。
  - 维护 isDrawing、startPoint、mousePos 等状态，保证拖拽过程的连续性。
- ROI 与遮罩
  - rectangle 表示当前 ROI；hasGreenMask 标记是否使用绿色遮罩。
  - drawMask 在遮罩 Canvas 上绘制圆形区域，brush/eraser 分别填充或擦除。
- 导出与回填
  - handleSave 将 ROI 区域从底图裁剪并叠加遮罩，导出为 PNG；优先使用 File System Access API，否则回退到传统下载。
  - onConfirm 回传模板文件名、遮罩状态与 ROI，供字段面板写入参数。

```mermaid
flowchart TD
Start(["进入 TemplateModal"]) --> Load["加载截图<br/>初始化画布尺寸"]
Load --> Tools{"选择工具"}
Tools --> |框选| Select["鼠标按下/移动<br/>更新 rectangle"]
Tools --> |画笔| Brush["鼠标按下/移动<br/>drawMask(绿色遮罩)"]
Tools --> |橡皮擦| Eraser["鼠标按下/移动<br/>drawMask(擦除遮罩)"]
Select --> CheckCoord{"检查负数坐标"}
CheckCoord --> |有负数| Resolve["调用 resolveNegativeROI<br/>解析实际坐标"]
CheckCoord --> |无负数| Preview["redrawCanvas<br/>绘制矩形框/预览圆"]
Resolve --> Preview
Preview --> Save{"点击保存模板"}
Save --> Export["裁剪底图+叠加遮罩<br/>toBlob 导出 PNG"]
Export --> Confirm["onConfirm(模板名, 遮罩状态, ROI)"]
Confirm --> End(["关闭模态框"])
```

**图表来源**
- [TemplateModal.tsx](file://src/components/modals/TemplateModal.tsx#L1-L991)
- [roiNegativeCoord.ts](file://src/utils/roiNegativeCoord.ts#L55-L178)

**章节来源**
- [TemplateModal.tsx](file://src/components/modals/TemplateModal.tsx#L1-L991)

### ScreenshotModalBase 组件
- 截图请求与监听
  - 打开时自动请求截图；监听 mfw 协议的截图结果事件，成功后设置截图并回调 onScreenshotChange。
- 视口控制
  - 提供缩放、平移、重置等能力，统一传递给子组件渲染。
- 通用 UI
  - 提供工具栏、缩放控制、确认/取消按钮、重新截图等通用交互。

**章节来源**
- [ScreenshotModalBase.tsx](file://src/components/modals/ScreenshotModalBase.tsx#L1-L405)

### ROI坐标转换支持
**新增功能** TemplateModal 现在集成了强大的ROI坐标转换功能，显著提升了用户体验：

- 负数坐标解析
  - x为负数：从右边缘计算距离
  - y为负数：从下边缘计算距离
  - w为0：延伸至右边缘
  - h为0：延伸至下边缘
  - w/h为负数：取绝对值，(x,y)视为右下角
- ROI分割处理
  - 当ROI超出图像边界时，自动分割为可见区域和超出区域
  - 提供左右上下的分割显示，帮助用户理解坐标含义
- 实时坐标转换
  - 在用户输入坐标时实时解析并显示实际坐标
  - 提供视觉反馈，包括颜色变化和工具提示
- 边界检测
  - 检测坐标是否超出图像边界
  - 计算需要的扩展边距大小
  - 提供坐标有效性验证

**章节来源**
- [TemplateModal.tsx](file://src/components/modals/TemplateModal.tsx#L112-L184)
- [TemplateModal.tsx](file://src/components/modals/TemplateModal.tsx#L887-L943)
- [roiNegativeCoord.ts](file://src/utils/roiNegativeCoord.ts#L55-L178)

### 字段面板与参数编辑（ParamFieldListElem）
- 触发与回填
  - 在字段参数编辑界面，当需要选择模板时打开 TemplateModal；onConfirm 回调中将模板文件名、遮罩状态与 ROI 写回到字段数据结构。
- 初始 ROI
  - 若字段参数中已有 ROI，可通过 initialROI 传入，便于复用或编辑。

**章节来源**
- [ParamFieldListElem.tsx](file://src/components/panels/field/items/ParamFieldListElem.tsx#L142-L154)

### 自定义模板存储（customTemplateStore）
- 数据结构
  - 以 localStorage 为持久化介质，版本号为 1.0；每条模板包含 label、nodeType、data、createTime。
- 能力
  - loadTemplates：加载并校验版本，转换为 NodeTemplateType 列表；异常时清空并提示。
  - addTemplate/updateTemplate/removeTemplate：增删改查与本地持久化。
  - getAllTemplates：合并预设模板与自定义模板，空节点模板优先。
  - hasTemplate：判断名称是否存在。

**章节来源**
- [customTemplateStore.ts](file://src/stores/customTemplateStore.ts#L1-L310)

### 节点模板与节点添加面板（NodeAddPanel）
- 模板展示
  - NodeAddPanel 从 customTemplateStore 获取模板列表，结合 nodeTemplates 合并展示；支持搜索、键盘导航、删除自定义模板。
- 图标与描述
  - 使用 iconfonts 提供的图标；描述根据识别/动作类型生成。

**章节来源**
- [NodeAddPanel.tsx](file://src/components/panels/main/NodeAddPanel.tsx#L1-L583)
- [nodeTemplates.ts](file://src/data/nodeTemplates.ts#L1-L108)
- [iconfonts/index.tsx](file://src/components/iconfonts/index.tsx#L1-L200)

### 模板保存入口（FieldPanelToolbar）
- 保存为模板
  - 在字段面板工具栏中，针对 Pipeline 节点提供"保存为模板"入口；弹窗输入模板名，调用 customTemplateStore 完成保存或覆盖。
- 与模板列表联动
  - 保存后可在 NodeAddPanel 中查看与删除。

**章节来源**
- [FieldPanelToolbar.tsx](file://src/components/panels/field/tools/FieldPanelToolbar.tsx#L88-L93)
- [customTemplateStore.ts](file://src/stores/customTemplateStore.ts#L96-L170)

## 依赖关系分析
- 组件耦合
  - TemplateModal 依赖 ScreenshotModalBase 提供截图与视口控制；依赖 roiNegativeCoord 提供坐标转换功能；依赖字段面板 ParamFieldListElem 作为调用方与回填目标。
  - NodeAddPanel 依赖 customTemplateStore 与 nodeTemplates，用于展示与管理模板。
  - FieldPanelToolbar 依赖 customTemplateStore，用于保存节点为模板。
- 数据流
  - 字段参数 -> TemplateModal -> ROI坐标转换 -> 导出模板 -> 字段参数
  - 节点数据 -> FieldPanelToolbar -> customTemplateStore -> NodeAddPanel
- 外部依赖
  - mfw 协议用于截图请求与结果监听。
  - File System Access API 用于现代浏览器的文件保存体验。

```mermaid
graph LR
PFLE["ParamFieldListElem.tsx"] --> TM["TemplateModal.tsx"]
TM --> SMB["ScreenshotModalBase.tsx"]
TM --> RNC["roiNegativeCoord.ts"]
TM --> FS["文件系统/下载"]
PFLE --> FS
NAP["NodeAddPanel.tsx"] --> CTS["customTemplateStore.ts"]
NAP --> NT["nodeTemplates.ts"]
FPT["FieldPanelToolbar.tsx"] --> CTS
TM --> IF["iconfonts/index.tsx"]
```

**图表来源**
- [ParamFieldListElem.tsx](file://src/components/panels/field/items/ParamFieldListElem.tsx#L142-L154)
- [TemplateModal.tsx](file://src/components/modals/TemplateModal.tsx#L1-L991)
- [ScreenshotModalBase.tsx](file://src/components/modals/ScreenshotModalBase.tsx#L1-L405)
- [roiNegativeCoord.ts](file://src/utils/roiNegativeCoord.ts#L1-L313)
- [NodeAddPanel.tsx](file://src/components/panels/main/NodeAddPanel.tsx#L1-L583)
- [customTemplateStore.ts](file://src/stores/customTemplateStore.ts#L1-L310)
- [nodeTemplates.ts](file://src/data/nodeTemplates.ts#L1-L108)
- [FieldPanelToolbar.tsx](file://src/components/panels/field/tools/FieldPanelToolbar.tsx#L88-L93)
- [iconfonts/index.tsx](file://src/components/iconfonts/index.tsx#L1-L200)

## 性能考量
- Canvas 绘制
  - redrawCanvas 与 drawMask 均在内存 Canvas 上进行，避免频繁 DOM 操作；建议在高分辨率截图场景下注意内存占用。
- ROI坐标转换
  - resolveNegativeROI 函数在每次坐标变化时调用，建议优化计算频率，避免频繁重绘。
- 导出策略
  - 优先使用 File System Access API，减少中间 URL 对象的创建与释放；若回退到传统下载，注意及时 revokeObjectURL。
- 截图与渲染
  - 截图请求与渲染采用异步监听，避免阻塞 UI；在多次打开模态框时应重置视口与遮罩状态，防止残留影响。

## 故障排查指南
- 无法获取截图
  - 检查连接状态与控制器 ID；确认 mfw 协议已注册截图结果监听。
  - 参考路径：[ScreenshotModalBase.tsx](file://src/components/modals/ScreenshotModalBase.tsx#L124-L169)
- ROI坐标转换异常
  - 检查图像尺寸与坐标值的匹配关系；确认负数坐标的计算逻辑。
  - 参考路径：[roiNegativeCoord.ts](file://src/utils/roiNegativeCoord.ts#L55-L178)
- 导出失败
  - 检查 toBlob 是否返回 Blob；确认浏览器支持 File System Access API 或传统下载回退路径。
  - 参考路径：[TemplateModal.tsx](file://src/components/modals/TemplateModal.tsx#L380-L496)
- 遮罩未生效
  - 确认 hasGreenMask 状态与 drawMask 的绘制逻辑；检查遮罩 Canvas 是否初始化。
  - 参考路径：[TemplateModal.tsx](file://src/components/modals/TemplateModal.tsx#L202-L243)
- 模板保存/加载异常
  - 检查 localStorage 权限与容量；确认版本号匹配；异常时会清空并提示。
  - 参考路径：[customTemplateStore.ts](file://src/stores/customTemplateStore.ts#L50-L94)
- 模板列表为空
  - 确认 getAllTemplates 合并顺序与过滤条件；检查自定义模板数量上限。
  - 参考路径：[customTemplateStore.ts](file://src/stores/customTemplateStore.ts#L212-L248)

**章节来源**
- [ScreenshotModalBase.tsx](file://src/components/modals/ScreenshotModalBase.tsx#L124-L169)
- [roiNegativeCoord.ts](file://src/utils/roiNegativeCoord.ts#L55-L178)
- [TemplateModal.tsx](file://src/components/modals/TemplateModal.tsx#L380-L496)
- [TemplateModal.tsx](file://src/components/modals/TemplateModal.tsx#L202-L243)
- [customTemplateStore.ts](file://src/stores/customTemplateStore.ts#L50-L94)
- [customTemplateStore.ts](file://src/stores/customTemplateStore.ts#L212-L248)

## 结论
"模板创建模态框"通过 ScreenshotModalBase 提供稳定的截图与视口控制能力，结合 TemplateModal 的 ROI 框选与遮罩绘制，实现了从截图到模板导出的完整闭环。**新增的ROI坐标转换支持**进一步提升了用户体验，特别是对于负数坐标的直观处理和超出边界的区域管理。配合字段面板的回填与自定义模板存储，用户可以便捷地创建、保存、复用模板，并在节点添加面板中统一管理。整体设计具备良好的扩展性与可维护性，适合在复杂节点参数场景中推广使用。