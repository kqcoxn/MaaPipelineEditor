...
- [src/stores/configStore.ts:100-173](file://src/stores/configStore.ts#L100-L173)
...
| 字段类型 | 默认顺序项 | 数量 |
|---------|-----------|------|
| 主任务字段 | desc, doc, enabled, max_hit, sub_name, recognition, inverse, pre_wait_freezes, pre_delay, action, anchor, repeat, repeat_wait_freezes, repeat_delay, post_wait_freezes, post_delay, timeout, rate_limit, next, on_error, focus, attach | 20项 |
...
字段排序系统通过模块化的设计和清晰的职责分工，成功实现了灵活而强大的字段排序功能。系统不仅提供了完善的类型定义和默认配置，还通过直观的用户界面让用户能够轻松定制排序规则。
...
通过持续的优化和改进，字段排序系统将继续为用户提供优秀的开发体验。