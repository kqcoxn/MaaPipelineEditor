...
## 更新摘要
**变更内容**
- 新增动态资源路径管理功能，支持多路径配置和自动填充
- 新增 Agent 标识符配置选项，支持可选的 Agent 连接
- 增强连接面板功能，提供更直观的状态指示和操作反馈
- 优化调试状态显示，改进节点执行状态的可视化效果
- 完善错误处理机制，提供更详细的资源加载错误提示
...
- 前后端协同的调试状态机与 UI 响应
- **新增**：动态资源路径管理、Agent 标识符配置、增强的连接面板功能
...
- [DebugProtocol.ts](file://src/services/protocols/DebugProtocol.ts#L1-L835)
- [debugStore.ts](file://src/stores/debugStore.ts#L1-L773)
...
- **新增**：动态资源路径管理，支持多路径配置和自动填充
- **新增**：Agent 标识符配置，支持可选的 Agent 连接
- 断点管理：设置/移除/清空断点
- 调试控制：开始/停止（V2 使用会话 ID）
- 日志导出：文本/JSON 下载
- 与 mfwStore 协作，确保控制器连接状态与调试按钮可用性
- **新增**：增强的连接面板功能，提供更直观的状态指示
...
- [handler_v2.go](file://LocalBridge/internal/protocol/debug/handler_v2.go#L1-L491)
- [debug_service_v2.go](file://LocalBridge/internal/mfw/debug_service_v2.go#L1-L409)
- [Agent客户端.md](file://instructions/maafw-golang-binding/高级功能/Agent架构/Agent客户端.md#L1-L296)
...
- **新增**：资源路径问题
  - 确认资源路径指向包含 pipeline 文件的目录
  - 检查各 pipeline 内容是否符合格式要求，有无重名等问题
- **新增**：Agent 连接问题
  - 检查 Agent 标识符是否正确配置
  - 确认 Agent 服务器正常运行且可访问
...
**主要增强功能**：
- 动态资源路径管理：支持多路径配置和自动填充，提升资源加载的灵活性
- Agent 标识符配置：提供可选的 Agent 连接支持，增强调试能力
- 连接面板增强：改进状态指示和操作反馈，提升用户体验
- 错误处理优化：提供更详细的资源加载错误提示和解决方案
...
- 在回调中避免阻塞操作，必要时异步处理
- 使用日志导出辅助问题定位与回归分析
- **新增**：合理配置资源路径和 Agent 标识符，确保调试环境稳定可靠
