...
**更新** 新增Anchor引用高亮支持，增强了节点相关性的可视化展示能力。
...
- 锚点引用：anchorRefSlice.ts提供锚点引用索引与高亮功能
- 样式与主题：nodes.module.less统一管理节点CSS类，包含Anchor引用高亮样式
...
## 核心组件
- PipelineNode：节点渲染入口，负责根据配置选择内容组件、计算焦点透明度、应用调试样式、挂载右键菜单，**新增锚点引用高亮功能**。
...
- **锚点引用高亮**：通过anchorRefSlice管理锚点引用索引，支持根据选中的锚点名称高亮相关节点。
...
UI->>ARS : 获取锚点引用高亮状态
UI->>UI : 计算 isRelated 与 isAnchorRefHighlighted
...
- **锚点引用高亮**：isAnchorRefHighlighted基于anchorRefHighlightedNodeIds计算，当节点ID在高亮集合中时应用特殊样式。
- 样式类：根据selected、nodeStyle、调试状态（执行中/已执行/识别中/失败）和**锚点引用高亮**动态拼装CSS类。
...
**图表来源**
- [PipelineNode/index.tsx:22-194](file://src/components/flow/nodes/PipelineNode/index.tsx#L22-L194)
- [PipelineNode/index.tsx:137-141](file://src/components/flow/nodes/PipelineNode/index.tsx#L137-L141)
...
- [PipelineNode/index.tsx:150-151](file://src/components/flow/nodes/PipelineNode/index.tsx#L150-L151)
- [nodes.module.less:266-280](file://src/styles/nodes.module.less#L266-L280)
- [debugStore.ts:143-800](file://src/stores/debugStore.ts#L143-L800)
...
- 组件耦合：
  ...
  - **新增** anchorRefSlice提供锚点引用高亮功能，被PipelineNode通过flowStore访问。
  - 内容组件依赖utils（图标/颜色）、KVElem、NodeHandles、NodeTemplateImages（现代风格）。
- 样式解耦：
  - 节点样式集中在nodes.module.less，通过类名组合实现不同风格，**新增锚点引用高亮样式**。
- 状态解耦：
...
## 结论
Pipeline节点通过配置驱动与状态联动实现了灵活多样的显示风格与交互体验。ClassicContent强调信息密度，ModernContent突出层次与可视化，MinimalContent追求简洁高效。**新增的锚点引用高亮功能**进一步增强了节点相关性的可视化展示，配合句柄系统、模板图片与调试状态，节点在复杂工作流中提供了清晰的视觉反馈与良好的可维护性。
...
- **锚点引用高亮集成**
  - 在nodes.module.less中定义新的高亮样式类，如anchor-ref-highlighted。
  - 使用@keyframes定义脉冲动画效果，增强视觉反馈。
  - 通过anchorRefSlice管理锚点引用索引，提供setSelectedAnchorName方法。
- 图标与颜色
