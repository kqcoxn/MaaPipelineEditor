...
## 更新摘要
**已做更改**
- 在“布局工具”部分新增了节点间距缩放工具的详细说明，包括 `createShiftTool` 函数和 `shiftNodes` 方法的实现原理。
- 更新了“布局工具”章节的流程图，新增对间距调整功能的描述。
- 更新了“依赖关系分析”部分，补充 `graphSlice.ts` 的依赖关系。
- 新增“节点间距缩放工具”子章节，详细阐述其使用方法与底层机制。
...
## 结论
工具栏通过“通用工具 + 节点模板 + 布局工具”的组合，覆盖了复制粘贴、撤销重做、节点对齐与自动布局、节点搜索与 AI 辅助定位、全局快捷键与截图导出等高频场景。本次更新新增的节点间距缩放工具，通过 `createShiftTool` 和 `shiftNodes` 方法，实现了基于距离比例的智能间距调整，进一步提升了布局的灵活性。历史状态采用差异检测与节流策略，兼顾性能与可靠性；布局与截图分别由独立模块提供，职责清晰、扩展性强。配合搜索面板与快捷键，能够显著提升复杂流程图的编辑效率与准确性。
