...
## 更新摘要
**所做更改**
- 更新了节点识别显示逻辑的说明，反映了重复识别单个节点时显示所有识别尝试的修复
- 新增了关于识别次数索引（runIndex）功能的详细说明
- 补充了timeout内重复识别行为的展示说明
- 更新了故障排查指南中关于重复识别的相关内容
...
**重要更新**：该功能现已修复了重复识别单个节点时仅显示一条记录的问题。现在会正确显示所有识别尝试，包括timeout内的重复识别。每个识别记录都包含执行次数索引（runIndex），可以清楚地看到同一节点的多次识别尝试。
...
- 数据转换
  ...
  - **新增**：每个目标节点项现在包含runIndex，显示该节点的第几次识别尝试
- 展示与交互
  - 倒序展示（最新的在前）
  - 分页：每页固定数量
...
- 交互：支持关闭并清除选中ID
- **新增**：现在可以查看识别记录的runIndex，了解该节点的识别尝试次数
...
- 事件处理要点：
  ...
  - **新增**：每个识别记录都包含runIndex，表示该节点的第几次识别尝试
...
- **新增**：重复识别问题排查
  - 如果某个节点显示只有一条识别记录，但实际上应该有多次尝试，检查是否启用了timeout内的重复识别
  - 查看识别记录的runIndex，确认是否正确显示了该节点的所有识别尝试
  - 检查RateLimit和Timeout配置，确认是否符合预期的行为模式
...
## 结论
节点识别卡片列表通过"发起节点 -> 目标节点"的轮次化展示，将复杂的识别过程以直观的卡片形式呈现，结合分页与命中高亮，显著提升了调试效率。

**重要更新**：该功能现已修复了重复识别单个节点时仅显示一条记录的问题。现在会正确显示所有识别尝试，包括timeout内的重复识别。每个识别记录都包含runIndex，可以清楚地看到同一节点的多次识别尝试，这大大增强了调试的准确性。

配合识别详情模态框与全局识别记录面板，形成了从宏观到微观的完整识别诊断链路。建议在大规模流水线下优先使用"目标节点记录"模式聚焦特定节点，再通过"查看详情"深入定位问题根因。新的runIndex功能使得开发者能够更好地理解节点的识别行为模式，特别是在处理timeout和重复识别场景时。