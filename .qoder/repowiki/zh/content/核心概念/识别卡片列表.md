...
## 更新摘要
**变更内容**
- 更新了特征检测算法选项的说明，反映了识别算法优化后的简化列表
- 新增了算法性能对比表格，展示了各算法的特点和适用场景
- 更新了识别系统配置文档中的算法支持信息
...
**更新** 系统现已支持简化的特征检测算法列表，移除了重复的 ORB 算法选项，优化了算法选择体验。

#### 算法支持列表

当前支持的特征检测算法包括：

| 算法 | 速度 | 尺度不变性 | 旋转不变性 | 精度 | 适用场景 |
|------|------|------------|------------|------|----------|
| SIFT | 慢 | ✅ | ✅ | 最高 | **推荐**，对精度要求高的场景 |
| KAZE | 较慢 | ✅ | ✅ | 高 | 2D/3D 图像，边缘保持好 |
| AKAZE | 中等 | ✅ | ✅ | 较高 | 速度与精度平衡 |
| BRISK | 快 | ✅ | ✅ | 中等 | 实时性要求高的场景 |
| ORB | 最快 | ❌ | ✅ | 较低 | 模板与目标尺寸一致时使用 |

#### 算法选择建议

- **优先选择**：SIFT（最高精度）或 AKAZE（速度与精度平衡）
- **实时场景**：BRISK 或 ORB（快速但精度略低）
- **特殊需求**：KAZE（边缘保持好）

**章节来源**
- [schema.ts](file://src/core/fields/recognition/schema.ts#L101-L106)
- [aiPredictor.ts](file://src/utils/aiPredictor.ts#L309-L311)