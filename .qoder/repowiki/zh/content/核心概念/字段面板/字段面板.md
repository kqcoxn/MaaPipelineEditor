...
## 更新摘要
**已做更改**  
- 新增了相邻信息标签页功能，集成 AdjacentInfoPanel 组件，提供节点关系可视化
- 更新了字段面板的标签页结构，新增"邻接信息"标签页
- 增强了节点关系展示能力，支持前驱和后继节点的分组显示
- 添加了节点跳转和聚焦功能，提升用户体验
- 更新了依赖分析，增加了相邻信息面板的依赖关系
...
AdjacentInfoPanel[AdjacentInfoPanel.tsx]
...
SourceHandles[SourceHandleTypeEnum]
TargetHandles[TargetHandleTypeEnum]
EdgeSystem[边连接系统]
end
subgraph "状态管理"
Store[flow/types.ts]
...
FieldPanel --> Other
FieldPanel --> Store
FieldPanel --> AdjacentInfoPanel
FieldPanel --> UpdateLog
...
AdjacentInfoPanel --> NodeTypes
AdjacentInfoPanel --> SourceHandles
AdjacentInfoPanel --> TargetHandles
AdjacentInfoPanel --> EdgeSystem
AdjacentInfoPanel --> Store
...
**更新** 新增了相邻信息标签页功能，通过 AdjacentInfoPanel 组件提供节点关系可视化。该组件支持前驱和后继节点的分组显示，包括 next 和 on_error 两种连接类型，并提供 jumpback 标识。
...
**更新** 新增了相邻信息面板的架构层，该组件通过 `useFlowStore` 订阅节点和边的关系数据，实时计算并展示节点的前驱和后继关系。
...
C --> |AdjacentInfoPanel.tsx| C3[相邻信息]
C --> |UpdateLog.tsx| C4[更新日志]
D --> |nodeSlice.ts| D1[Zustand Store]
D --> |updateLogs.ts| D2[更新日志数据]
C3 --> |节点关系计算| C5[前驱/后继节点]
C3 --> |节点跳转| C6[视图聚焦]
```
...
## 详细组件分析
### FieldPanel 组件分析
`FieldPanel` 组件是传统字段面板的入口点。它首先通过 `useFlowStore` 获取当前选中的节点，然后根据节点类型决定渲染哪个子组件。对于 `Pipeline` 节点，它会渲染一个包含识别、动作、其它参数和相邻信息的完整表单。表单的每一项都由 `ParamFieldListElem` 组件生成，该组件根据字段的 `type` 属性选择合适的输入控件。例如，`Int` 类型的字段会使用 `InputNumber`，`StringList` 类型的字段会使用 `TextArea` 并支持添加/删除多个值。

**更新** 新增了相邻信息标签页，通过 `Tabs` 组件提供三个标签页：字段配置、邻接信息和调试记录。邻接信息标签页集成了 `AdjacentInfoPanel` 组件，提供节点关系可视化功能。
...
participant AdjacentInfoPanel as "AdjacentInfoPanel"
...
FieldPanel->>AdjacentInfoPanel : 渲染相邻信息
AdjacentInfoPanel->>Store : 计算节点关系
Store-->>AdjacentInfoPanel : 返回边关系数据
User->>FieldPanel : 切换到邻接信息标签页
FieldPanel->>AdjacentInfoPanel : 显示节点关系
User->>AdjacentInfoPanel : 点击节点标签
AdjacentInfoPanel->>Store : 跳转到目标节点
Store->>Store : 更新节点选中状态
...
RenderRecoForm --> AddAdjacentTab["添加邻接信息标签页"]
RenderActionForm --> AddAdjacentTab
RenderOtherForm --> AddAdjacentTab
AddAdjacentTab --> RenderAdjacentPanel["渲染相邻信息面板"]
RenderAdjacentPanel --> End([结束])
ShowEmpty --> End
```
...
### AdjacentInfoPanel 组件分析
`AdjacentInfoPanel` 组件是新增的相邻信息面板，专门用于展示当前选中节点的前驱和后继节点关系。该组件通过 `useFlowStore` 订阅节点和边的状态，实时计算并展示节点的连接关系。组件支持两种连接类型：`next`（正常流程）和 `on_error`（错误处理），并能识别 `jumpback`（回跳）连接。

组件的核心功能包括：
- **前驱节点计算**：遍历所有边，找到目标节点为当前节点的边，提取源节点信息
- **后继节点计算**：遍历所有边，找到源节点为当前节点的边，提取目标节点信息
- **分组排序**：按连接类型（next/error）和顺序（order）进行分组和排序
- **节点标签渲染**：为每个节点生成带颜色标识的标签，支持点击跳转
- **交互功能**：点击节点标签可跳转到对应节点并聚焦视图

```mermaid
flowchart TD
Start([开始]) --> GetEdges["获取边关系数据"]
GetEdges --> CalcPredecessors["计算前驱节点"]
CalcPredecessors --> CalcSuccessors["计算后继节点"]
CalcSuccessors --> GroupByType["按连接类型分组"]
GroupByType --> SortByOrder["按顺序排序"]
SortByOrder --> RenderTags["渲染节点标签"]
RenderTags --> HandleClick["处理节点点击"]
HandleClick --> UpdateSelection["更新节点选中状态"]
UpdateSelection --> FocusView["聚焦视图到目标节点"]
FocusView --> End([结束])
CalcPredecessors --> FilterPreds["过滤 next 和 on_error"]
CalcSuccessors --> FilterSuccs["过滤 next 和 on_error"]
FilterPreds --> GroupPreds["分组前驱节点"]
FilterSuccs --> GroupSuccs["分组后继节点"]
GroupPreds --> RenderPreds["渲染前驱列表"]
GroupSuccs --> RenderSuccs["渲染后继列表"]
RenderPreds --> CombineUI["组合UI界面"]
RenderSuccs --> CombineUI
CombineUI --> End
```
...
## 依赖分析
字段面板的依赖关系清晰且层次分明。`FieldPanel.tsx` 和 `InlineFieldPanel.tsx` 直接依赖于 `fieldFactory.ts`、`fieldTypes.ts` 和 `types.ts` 来获取字段定义和类型信息。它们还依赖于 `src/stores/flow/` 中的 Zustand store 来管理应用状态。`recognition/schema.ts`、`action/schema.ts` 和 `other/schema.ts` 文件定义了具体的字段参数，并通过 `index.ts` 统一导出。`AdjacentInfoPanel.tsx` 组件依赖于 `constants.ts` 中的节点类型枚举和边连接类型定义。`UpdateLog.tsx` 组件依赖于 `updateLogs.ts` 提供的更新日志数据。

**更新** 新增了相邻信息面板的依赖关系，包括节点类型枚举、边连接类型定义和边系统。
...
A --> D[types.ts]
A --> E[nodeSlice.ts]
A --> F[AdjacentInfoPanel.tsx]
B --> G[createField]
C --> H[FieldTypeEnum]
D --> I[FieldType]
E --> J[setNodeData]
A --> K[recognition/schema.ts]
A --> L[action/schema.ts]
A --> M[other/schema.ts]
K --> N[识别参数]
L --> O[动作参数]
M --> P[其他参数]
Q[InlineFieldPanel.tsx] --> B
Q --> C
Q --> D
Q --> E
Q --> K
Q --> L
Q --> M
R[AdjacentInfoPanel.tsx] --> S[NodeTypeEnum]
R --> T[SourceHandleTypeEnum]
R --> U[TargetHandleTypeEnum]
R --> V[EdgeSystem]
R --> W[useFlowStore]
X[UpdateLog.tsx] --> Y[updateLogs.ts]
Y --> Z[PinnedNotice]
Y --> AA[UpdateCategories]
AB[PanelConfigSection.tsx] --> AC[configStore.ts]
AD[constants.ts] --> S
AD --> T
AD --> U
```
...
**更新** 相邻信息面板同样采用了性能优化策略，使用 `useMemo` 缓存前驱和后继节点的计算结果，避免在节点关系频繁变化时产生性能开销。组件只在相关依赖（edges、nodes、currentNodeId）发生变化时重新计算节点关系。
...
**更新** 如果相邻信息标签页没有正确显示节点关系，首先检查 `AdjacentInfoPanel.tsx` 中的 `currentNodeId` 和 `currentNodeLabel` 参数是否正确传递。确认 `useFlowStore` 是否能正确获取到节点和边的关系数据。如果节点标签点击无效，检查 `handleNodeClick` 函数中的节点查找逻辑。如果节点跳转功能异常，确认 `instance` 对象是否存在并且支持 `setCenter` 方法。
...
**更新** 新增的相邻信息标签页功能进一步增强了字段面板的能力，通过 AdjacentInfoPanel 组件提供了节点关系可视化功能。该功能支持前驱和后继节点的分组显示，包括 next 和 on_error 两种连接类型，并提供 jumpback 标识。用户可以通过点击节点标签直接跳转到对应节点并聚焦视图，大大提升了流程图的导航体验。
