...
## 更新摘要
**变更内容**
- 新增OCR错误专用Modal弹窗功能，提供结构化错误信息展示
- 增强错误协议服务，区分普通错误与OCR错误的处理策略
- 优化OCR资源加载失败的错误提示，包含详细的原因分析和排查建议
- 扩展错误处理机制，支持更丰富的错误上下文信息
...
## 简介
本文件围绕"错误协议"展开，系统性阐述错误处理机制与实现原理，覆盖错误分类、错误码定义、错误信息格式化、错误传播与恢复、错误上报、全局错误处理器、错误状态管理、错误日志记录与查询、错误通知与用户提示策略，以及错误统计分析与故障预警的实现思路。特别关注新增的OCR错误处理功能，提供更结构化的错误信息展示和用户友好的错误提示。

## 项目结构
错误协议位于前端协议层，负责统一接收来自本地服务的错误消息，并进行用户可见的提示与状态联动；同时配合日志协议与本地桥接错误模型，形成完整的错误闭环。新增的OCR错误处理功能通过专门的Modal弹窗提供更详细的错误信息展示。
...
- [OCRModal.tsx:1-1105](file://src/components/modals/OCRModal.tsx#L1-L1105)
...
- 错误协议(ErrorProtocol)
  ...
  - **新增**：区分普通错误与OCR错误，OCR错误通过专门的Modal弹窗展示详细信息。
- 协议基类(BaseProtocol)
...
- **新增**：OCR错误弹窗(OCRModal)
  - 提供结构化的OCR错误信息展示，包含原因分析、资源目录和排查建议。
  - 支持前端和原生两种OCR识别模式的错误处理。

**章节来源**
- [ErrorProtocol.ts:10-119](file://src/services/protocols/ErrorProtocol.ts#L10-L119)
...
## 架构总览
错误协议的端到端流程如下：本地服务通过WebSocket向前端广播错误消息；前端错误协议解析并根据错误码生成用户提示；对部分控制器错误自动清理连接状态；**新增**：OCR错误通过专门的Modal弹窗提供详细的信息展示；同时日志协议接收并持久化日志，形成可观测性闭环。
...
participant OM as "OCR错误弹窗(OCRModal.tsx)"
...
- [ErrorProtocol.ts:26-119](file://src/services/protocols/ErrorProtocol.ts#L26-L119)
...
- 错误分类与提示映射
  ...
  - **新增**：区分普通错误与OCR错误，OCR错误通过专门的Modal弹窗处理。
- 用户提示与状态联动
  - 使用UI库的消息组件弹出错误提示；对控制器相关错误触发mfwStore.clearConnection，清空连接状态。
  - **新增**：OCR错误使用Modal.error显示详细信息，包含原因、资源目录和排查建议。
- 错误传播与恢复
...
Parse --> CheckOCR{"是否OCR错误?"}
CheckOCR --> |是| BuildDetail["构建详细错误信息"]
BuildDetail --> ShowModal["显示OCR错误弹窗"]
ShowModal --> IsControllerErr{"是否控制器错误?"}
CheckOCR --> |否| ShowMsg["弹出错误提示(message.error)"]
ShowMsg --> IsControllerErr
...
**章节来源**
- [ErrorProtocol.ts:19-119](file://src/services/protocols/ErrorProtocol.ts#L19-L119)

### OCR错误弹窗(OCRModal)
- **新增**：专门处理OCR错误的Modal弹窗组件
- 错误信息结构化展示
  - 支持MFW_OCR_RESOURCE_NOT_CONFIGURED和MFW_RESOURCE_LOAD_FAILED等错误码
  - 展示详细的原因分析、资源目录路径和具体的排查建议
- 用户界面设计
  - 使用React.createElement动态构建pre格式的内容，保持换行和缩进
  - 设置合适的弹窗宽度(520px)确保信息完整显示
  - 提供清晰的标题区分不同类型的OCR错误
...
- 与前端联动
  ...
  - **新增**：支持OCR相关的错误码，如MFW_OCR_RESOURCE_NOT_CONFIGURED和MFW_RESOURCE_LOAD_FAILED。
...
- 错误消息处理
  ...
  - **新增**：OCR错误弹窗使用React.createElement动态构建内容，避免不必要的组件重渲染。
- 日志存储
  - loggerStore采用固定容量队列，超出上限丢弃最旧条目，降低内存压力。
- WebSocket分发
  - server.ts按path分发，未命中路由仅告警不阻塞，保障整体吞吐。
- **新增**：OCR错误弹窗的Modal组件按需渲染，只有在出现OCR错误时才触发弹窗显示。
...
- 提示内容不符合预期
  ...
  - **新增**：检查OCR错误是否正确触发showOCRErrorModal方法。
- 控制器错误后仍显示连接
  - 确认错误协议是否正确调用mfwStore.clearConnection；检查MFW协议是否在状态异常时清理。
- 日志未显示
  - 检查"/lte/logger"路由是否被注册；确认LoggerProtocol是否正确解析level与时间戳。
- **新增**：OCR错误弹窗不显示
  - 检查错误数据是否包含detail对象和必要的字段
  - 确认Modal组件的渲染条件是否满足

**章节来源**
- [server.ts:348-373](file://src/services/server.ts#L348-L373)
- [ErrorProtocol.ts:57-119](file://src/services/protocols/ErrorProtocol.ts#L57-L119)
- [LoggerProtocol.ts:32-56](file://src/services/protocols/LoggerProtocol.ts#L32-L56)
- [OCRModal.tsx:315-351](file://src/components/modals/OCRModal.tsx#L315-L351)

## 结论
错误协议通过统一的错误码与消息格式，实现了从前端到后端的一致性错误体验；结合日志协议与状态管理，形成了可观测、可恢复、可提示的完整闭环。**新增的OCR错误弹窗功能**进一步提升了用户体验，通过结构化的错误信息展示帮助用户快速定位和解决问题。未来可在现有基础上扩展错误统计与趋势分析能力，以支撑更高级别的故障预警与运维决策。
...
- MFW错误码(本地服务)
  ...
  - 资源类：MFW_OCR_RESOURCE_NOT_CONFIGURED、MFW_RESOURCE_LOAD_FAILED、MFW_TASK_SUBMIT_FAILED
  - 其他：MFW_NOT_INITIALIZED、MFW_DEVICE_NOT_FOUND、MFW_CONNECTION_FAILED、MFW_SCREENCAP_FAILED、MFW_OPERATION_FAILED/FAIL、MFW_INVALID_PARAMETER
...
- 前端映射
  ...
  - **新增**：OCR错误使用Modal弹窗展示详细信息，包含原因、资源目录和排查建议

**章节来源**
- [errors.go:44-50](file://LocalBridge/internal/errors/errors.go#L44-L50)
- [error.go:34-52](file://LocalBridge/internal/mfw/error.go#L34-L52)
- [ErrorProtocol.ts:29-119](file://src/services/protocols/ErrorProtocol.ts#L29-L119)

### 错误传播与恢复
- 传播路径
  - 本地服务 -> WebSocket -> server.ts -> 错误协议 -> 用户提示/状态联动
  - **新增**：OCR错误 -> 错误协议 -> OCR错误弹窗 -> 用户交互
- 恢复策略
  - 控制器错误触发状态清理，引导用户重新连接
  - 日志持久化便于事后分析与定位
  - **新增**：OCR错误弹窗提供具体的排查建议，帮助用户快速解决问题
...
- 状态管理
  ...
  - **新增**：OCR错误弹窗通过专门的状态管理，提供结构化的错误信息展示
...
- 通知方式
  ...
  - **新增**：OCR错误使用Modal弹窗提供详细信息展示
- 提示策略
  - 优先使用错误码映射的中文提示；对MFW错误优先展示detail中的字符串
  - **新增**：OCR错误弹窗提供结构化的错误信息，包含原因分析和具体建议

**章节来源**
- [ErrorProtocol.ts:54-119](file://src/services/protocols/ErrorProtocol.ts#L54-L119)
...
- 统计分析
  ...
  - **新增**：OCR错误统计分析，包括资源加载失败、任务提交失败等细分类型
- 趋势预测
  - 建议按小时/天粒度统计错误频次，观察峰值与波动趋势
  - **新增**：OCR错误趋势分析，监控OCR资源配置问题的发生频率
- 故障预警
  - 当某类错误出现异常增长或持续高发时，触发阈值告警并联动通知
  - **新增**：OCR错误预警，当资源加载失败率超过阈值时及时提醒用户检查OCR配置