...
## 更新摘要
**所做更改**
- 修正了MFW协议中WlRoots相关方法的注释和方法签名，确保文档与实际实现保持一致
- 更新了WlRoots控制器的创建、连接和管理机制说明
- 完善了WlRoots设备发现和管理的详细流程
...
**更新** 本次更新重点关注WlRoots相关方法的注释修正和方法签名更新，确保文档与实际实现保持一致，特别是控制器管理器中WlRoots控制器的创建和连接方法。
...
**更新** WlRoots控制器现已正式纳入控制器管理器，支持Linux Wayland合成器的连接和管理。
...
FE->>WS : "/etl/mfw/create_wlroots_controller"
WS->>Proto : 路由分发
Proto->>Svc : 调用 ControllerManager.CreateWlRootsController
Svc->>Ctl : 创建并连接WlRoots控制器
Ctl->>FW : NewWlRootsController + PostConnect
FW-->>Ctl : 连接结果
Ctl-->>Svc : 返回控制器ID
Svc-->>Proto : 返回控制器创建结果
...
**更新** WlRoots设备发现现在明确标注为"需手动输入socket路径"，因为WlRoots仅在Linux上支持。
...
### 控制器管理与设备控制
- 支持类型：ADB、Win32、PlayCover、Gamepad、**WlRoots**。
...
**更新** 新增WlRoots控制器支持，包括创建、连接、断开和管理功能。
...
+CreateWlRootsController(socketPath)
...
### WlRoots控制器支持
**新增功能** 完整的WlRoots控制器支持：

- **创建WlRoots控制器**：通过CreateWlRootsController方法创建WlRoots控制器实例
- **连接管理**：自动连接，支持超时控制和UUID获取
- **设备发现**：RefreshWlRootsSockets方法返回WlRoots合成器列表（当前为空，需手动输入socket路径）
- **类型标识**：控制器类型标记为"WlRoots"，设备信息记录socket路径
- **适配器集成**：MaaFWAdapter支持ConnectWlRoots方法进行控制器连接

```mermaid
flowchart TD
Start(["WlRoots控制器创建"]) --> Create["CreateWlRootsController<br/>创建控制器实例"]
Create --> Connect["ConnectController<br/>自动连接"]
Connect --> PostConnect["PostConnect<br/>异步连接"]
PostConnect --> Wait["等待连接完成<br/>10秒超时"]
Wait --> Success{"连接成功?"}
Success --> |是| GetUUID["获取UUID"]
Success --> |否| Error["返回连接失败错误"]
GetUUID --> SetInfo["设置控制器信息<br/>类型=WlRoots, UUID=socket路径"]
SetInfo --> Done(["控制器就绪"])
Error --> End(["结束"])
```
...
**更新** MaaFWAdapter现在包含ConnectWlRoots方法，支持WlRoots控制器的连接管理。
...
- **WlRoots支持**：createWlRootsController方法支持WlRoots控制器创建。

**更新** 前端协议现在包含完整的WlRoots控制器创建支持，包括设备信息记录和连接状态管理。
...
- **新增** WlRoots控制器支持完整的依赖链路，从设备发现到控制器管理再到MaaFramework集成。
...
- **新增** WlRoots控制器优化：WlRoots控制器连接采用异步模式，支持超时控制和UUID获取。

**更新** WlRoots控制器连接现在具有完整的超时控制和错误处理机制，确保连接过程的稳定性。
...
- **新增** WlRoots设备发现失败：确认WlRoots仅在Linux上支持，当前实现需要手动输入socket路径。

**更新** 新增WlRoots相关的故障排查指导，包括连接失败、设备发现失败等问题的解决方案。
...
**更新** 最新更新完成了WlRoots控制器的完整支持，包括设备发现、控制器创建、连接管理等全套功能，确保文档与实际实现保持一致。新增的注释修正和方法签名更新进一步提升了代码的可读性和维护性。
...
**更新** 新增WlRoots设备信息的数据结构支持，包括socket_path字段的定义和使用。更新了直接动作执行的路由和数据结构支持。
