...
## 更新摘要
**变更内容**
- 更新了全局配置中的mfwVersion从5.10.5升级到5.11.1，反映MaaFramework版本的最新升级
- 更新了全局配置中的protocolVersion从1.0.5升级到1.0.6，体现协议版本的改进
- 更新了版本号生成逻辑，现在使用beta迭代号1进行开发版本命名
- 更新了GitHub Actions工作流中对beta迭代号的监控逻辑
...
- 全局配置结构
  - dev: 开发模式开关
  - version: 主版本号（1.7.0）
  - betaIteration: Beta迭代号（已从3更新为1）
  - mfwVersion: MaaFramework版本（已从5.10.5升级到5.11.1）
  - protocolVersion: 协议版本（已从1.0.5升级到1.0.6）
- 版本号生成逻辑
  - 在开发模式下，版本号格式为 `主版本号_beta_迭代号`
  - 当betaIteration为1时，开发版本号为 `1.7.0_beta_1`
- GitHub Actions集成
  - 工作流监控betaIteration变化，当值大于0时触发部署
  - 通过正则表达式提取src/stores/configStore.ts中的betaIteration值

```mermaid
flowchart TD
Start(["启动应用"]) --> CheckDev{"开发模式？"}
CheckDev --> |是| GenerateVersion["生成开发版本号"]
CheckDev --> |否| UseMainVersion["使用主版本号"]
GenerateVersion --> Concat["拼接格式: 主版本号_beta_迭代号"]
Concat --> Display["显示版本信息"]
UseMainVersion --> Display
```

**图表来源**
- [configStore.ts:6-17](file://src/stores/configStore.ts#L6-L17)
- [preview.yaml:36-62](file://.github/workflows/preview.yaml#L36-L62)
...
- 版本号显示异常
  - 检查 configStore.ts 中的 betaIteration 值是否正确（现已更新为1）
  - 确认 GitHub Actions 工作流是否正确识别 betaIteration 变更
  - 验证 mfwVersion 和 protocolVersion 是否正确反映最新的版本升级
...
**更新** 本次更新反映了前端配置版本的重要升级：mfwVersion从5.10.5升级到5.11.1，protocolVersion从1.0.5升级到1.0.6。这些版本升级体现了MaaFramework和协议系统的持续改进，为用户提供更稳定和功能丰富的配置管理体验。同时，开发周期的变化体现在beta迭代号从3调整为1，体现了项目进入新的开发阶段。GitHub Actions工作流也相应更新了对beta迭代号的监控逻辑，确保开发版本的正确部署。
...
- 版本管理
  ...
  - Beta迭代：1（已更新）
  - 开发版本格式：1.7.0_beta_1
  - GitHub Actions监控：当betaIteration>0时触发部署
  - MaaFramework版本：5.11.1（已升级）
  - 协议版本：1.0.6（已升级）
