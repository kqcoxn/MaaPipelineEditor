...
**图表来源**
- [clipboardStore.ts:1-52](file://src/stores/clipboardStore.ts#L1-L52)
...
**章节来源**
- [clipboardStore.ts:5-52](file://src/stores/clipboardStore.ts#L5-L52)
- [Flow.tsx:53-135](file://src/components/Flow.tsx#L53-L135)
- [graphSlice.ts:64-249](file://src/stores/flow/slices/graphSlice.ts#L64-L249)
- [selectionContextMenu.tsx:129-152](file://src/components/flow/selectionContextMenu.tsx#L129-L152)
- [GlobalPanel.tsx:160-163](file://src/components/panels/tools/GlobalPanel.tsx#L160-L163)
...
- **设计要点**
  ...
  - **新增**：使用 lodash 的 cloneDeep 确保深拷贝，避免对象引用问题
...
## 性能考虑
- **克隆与去重**
  - **新增**：paste 对节点进行深拷贝，使用 lodash 的 cloneDeep 确保完全独立的对象副本
...
- **数据完整性保障**
  - **新增**：使用 lodash 深拷贝避免对象引用问题，确保复制的数据不会相互影响
  - **新增**：修复粘贴功能可能产生虚节点的问题，通过深拷贝确保数据一致性
...
- **新增**：虚节点问题
  - **新增**：如果遇到粘贴后出现虚节点或数据不一致问题，可能是由于浅拷贝导致的引用共享
  - **新增**：系统已通过 lodash 深拷贝修复此问题，确保每个节点都是独立的副本
...
**新增**：通过引入 lodash 深拷贝机制，系统显著提升了数据安全性，修复了粘贴功能可能产生的虚节点问题，确保了复制数据的完整性和独立性。
...
### 技术实现细节
- **新增**：lodash 深拷贝集成
  - 在 clipboardStore 中使用 `cloneDeep(nodes)` 和 `cloneDeep(edges || [])` 确保复制的数据完全独立
  - 在 graphSlice 中同样使用 `cloneDeep(nodes)` 和 `cloneDeep(edges)` 处理粘贴数据
  - 通过深拷贝避免对象引用问题，防止数据共享导致的副作用
- **新增**：数据完整性保障
  - 深拷贝确保每个复制的节点和边都是独立的对象实例
  - 修复了粘贴功能可能产生虚节点的问题，因为不再共享原始数据的引用
  - 提升了系统的稳定性和可靠性

**章节来源**
- [clipboardStore.ts:25-28](file://src/stores/clipboardStore.ts#L25-L28)
- [graphSlice.ts:102](file://src/stores/flow/slices/graphSlice.ts#L102)
- [graphSlice.ts:251](file://src/stores/flow/slices/graphSlice.ts#L251)
