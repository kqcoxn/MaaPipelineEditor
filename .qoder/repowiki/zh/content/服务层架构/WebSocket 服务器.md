...
## 结论
本文档系统梳理了 LocalWebSocketServer 与后端 WebSocketServer 的实现细节，覆盖连接管理、握手协议、消息路由、状态监听、错误恢复与性能优化等方面。通过清晰的前后端职责划分与事件驱动的路由分发，系统实现了稳定可靠的本地服务通信能力。建议在扩展新协议时遵循现有模式，确保路由命名规范、错误处理一致与状态监听完善。
...
FE->>WS : "发送 {path : '/system/handshake', data : {protocol_version}}"
WS->>RT : "Route(msg)"
RT->>RT : "handleHandshake()"
RT-->>FE : "返回 {path : '/system/handshake/response', data : {success,...}}"
...
图表来源
- [src\services\type.ts:2-5](file://src/services/type.ts#L2-L5)
