...
**更新** 项目现已升级至1.7.0版本，引入了完整的版本管理系统和自动化发布流程，确保版本一致性并提高发布效率。
...
### 版本管理系统
MPE 项目引入了完整的版本管理系统，确保多模块版本一致性：

- **统一版本号管理**：通过交互式迁移脚本 `scripts/migrate-version.mjs` 精确更新所有版本位置
- **多位置版本同步**：自动检测并更新配置存储、Go 主程序、Wails 配置、Windows 清单文件、macOS Info.plist 等多处版本号
- **严格语义化版本**：支持 X.Y.Z 格式，包含可选预发布标识（如 1.7.0-beta.1）

### 自动化发布流程
项目采用 GitHub Actions 实现自动化发布，包含完整的质量控制：

- **多平台构建**：同时构建 Windows、Linux、macOS 平台的 LocalBridge 可执行文件
- **Web 包装构建**：生产环境构建、文档站点构建、着陆页构建
- **质量检查**：跳过测试版本标签，支持手动触发和自动触发
- **发布自动化**：自动生成变更日志，创建 GitHub Release

```mermaid
flowchart TD
Start(["版本发布触发"]) --> CheckTag{"标签检查"}
CheckTag --> |自动触发| BuildLocal["构建 LocalBridge<br/>Windows/Linux/macOS"]
CheckTag --> |手动触发| ManualBuild["手动构建测试版本"]
BuildLocal --> BuildWeb["构建 Web 包装<br/>生产环境"]
ManualBuild --> BuildWeb
BuildWeb --> QualityCheck["质量检查<br/>跳过测试版本"]
QualityCheck --> |通过| GenerateChangelog["生成变更日志"]
QualityCheck --> |失败| Abort["中止发布"]
GenerateChangelog --> CreateRelease["创建 GitHub Release"]
CreateRelease --> Publish["发布到 GitHub Releases"]
```

**图表来源**
- [.github\workflows\release.yaml:13-223](file://.github/workflows/release.yaml#L13-L223)

### 发布流程特点
- **版本一致性**：通过自动化脚本确保所有模块版本号同步
- **质量优先**：跳过测试版本标签，确保发布质量
- **多平台支持**：覆盖主流操作系统平台
- **文档完整**：同时发布 Web 应用、文档站点、着陆页
- **自动化程度高**：从构建到发布的全流程自动化

**章节来源**
- [scripts\migrate-version.mjs:1-307](file://scripts/migrate-version.mjs#L1-L307)
- [.github\workflows\release.yaml:1-223](file://.github/workflows/release.yaml#L1-L223)
...
- 版本管理问题
  - 使用 `yarn migrate` 交互式脚本更新版本号，确保所有位置版本一致。
  - 检查 `scripts/migrate-version.mjs` 的输出，确认版本迁移是否成功。
...
- [scripts\migrate-version.mjs:163-307](file://scripts/migrate-version.mjs#L163-L307)
...
- **版本管理现代化**：1.7.0版本引入完整版本管理系统和自动化发布流程，确保版本一致性并提高发布效率。

**更新** 通过引入交互式版本迁移脚本和 GitHub Actions 自动化发布流程，项目在版本管理方面达到了新的高度，为未来的持续集成和发布奠定了坚实基础。

## 附录
- **版本与发布**：项目采用语义化版本管理，通过 `scripts/migrate-version.mjs` 脚本进行精确版本迁移；GitHub Actions 实现自动化发布，包含质量检查和多平台构建。
- **历史与演进**：从 YaMaaPE 到 MaaPipelineExtremer，再到 MaaPipelineEditor，持续完善协议、UI 与本地能力；1.7.0版本标志着版本管理系统的正式上线。
- **开发与贡献**：提供文档站、脚手架与开发工具链，鼓励社区参与与反馈；自动化发布流程降低了发布门槛，提高了发布频率。
