...
## 更新摘要
**变更内容**   
- 新增对便签节点(sticker_nodes)的配置分割与合并支持
- 扩展了原有的external_nodes和anchor_nodes处理机制
- 更新了配置类型定义以包含sticker_nodes字段
- 增强了节点配置处理的完整性
...
**更新** 新增了对便签节点(sticker nodes)的完整支持，包括位置信息、颜色主题和内容配置的分割与合并。
...
5. 普通节点中的 `configMark` 字段（包含位置信息）被提取到 `config.node_configs` 中，其余部分保留在纯 Pipeline 对象中

**更新** 新增了对便签节点的处理逻辑，便签节点的配置包含标签、内容和颜色主题信息，但不包含位置信息。
...
**Diagram sources**
- [configSplitter.ts](file://src/core/parser/configSplitter.ts#L19-L126)
...
5. 遍历纯 Pipeline 对象，为包含位置信息的节点添加 `configMark` 字段

**更新** 新增了对便签节点的合并处理，便签节点的数据直接包含在配置对象中，不需要额外的位置信息包装。
...
AddStickerNodes --> ProcessPipeline["处理纯 Pipeline 节点"]
...
**Diagram sources**
- [configSplitter.ts](file://src/core/parser/configSplitter.ts#L135-L213)

## 便签节点支持
系统新增了对便签节点(sticker nodes)的完整支持，便签节点是一种特殊的配置节点，用于提供额外的注释和说明信息。

### 便签节点特征
- **标签**：便签的标题，用于标识便签内容
- **内容**：便签的具体文本内容，支持多行编辑
- **颜色主题**：支持多种颜色主题，包括黄色、绿色、蓝色、粉色和紫色
- **位置信息**：便签节点的位置信息存储在配置对象中，但便签节点本身不包含位置配置
...
### 便签节点处理机制
便签节点的处理与其他节点类型类似，但具有以下特点：
1. 便签节点的键名使用 `stickerMarkPrefix` 前缀
2. 便签节点的配置直接存储在 `config.sticker_nodes` 中
3. 便签节点不包含位置信息，因为它们是配置节点而非执行节点
4. 便签节点的颜色主题信息存储在配置中，用于UI渲染
...
**Diagram sources**
- [configSplitter.ts](file://src/core/parser/configSplitter.ts#L220-L234)
...
**更新** 新增了便签节点的处理流程，便签节点作为配置节点参与分割和合并过程，但不参与实际的执行流程。
