...
## 更新摘要
**变更内容**   
- 增强配置分割功能，支持分离模式下节点位置和处理方向信息提取
- 改进非Pipeline节点处理，新增分组节点支持
- 扩展配置类型定义以包含分组节点和便签节点
- 增强节点配置处理的完整性，支持更丰富的节点类型
...
**更新** 增强了对多种节点类型的配置处理能力，包括分组节点和便签节点，支持更完整的节点位置和处理方向信息提取。
...
- `configMark`：`$__mpe_code`，用于标记节点的位置信息和处理方向
...
- `stickerMarkPrefix`：`$__mpe_sticker_`，用于标识便签节点
- `groupMarkPrefix`：`$__mpe_group_`，用于标识分组节点
...
+string stickerMarkPrefix = "$__mpe_sticker_"
+string groupMarkPrefix = "$__mpe_group_"
}
...
+Record~string, any~ sticker_nodes
+Record~string, any~ group_nodes
}
class FileConfig {
+string filename
+string prefix
...
MpeConfigType --> sticker_nodes : "可选"
MpeConfigType --> group_nodes : "可选"
```

**Diagram sources**
- [types.ts](file://src/core/parser/types.ts#L15-L21)
- [types.ts](file://src/core/parser/types.ts#L65-L77)
...
4. **新增**：以 `stickerMarkPrefix` 开头的键被视为便签节点，其内容被提取到 `config.sticker_nodes` 中
5. **新增**：以 `groupMarkPrefix` 开头的键被视为分组节点，其内容被提取到 `config.group_nodes` 中
6. 普通节点中的 `configMark` 字段（包含位置信息和处理方向）被提取到 `config.node_configs` 中，其余部分保留在纯 Pipeline 对象中

**更新** 增强了对分组节点的支持，分组节点包含位置信息、颜色主题和子节点标签列表，但不包含位置配置。
...
CheckKey --> IsSticker{"以 $__mpe_sticker_ 开头?"}
IsSticker --> |是| ExtractSticker["提取到 config.sticker_nodes"]
CheckKey --> IsGroup{"以 $__mpe_group_ 开头?"}
IsGroup --> |是| ExtractGroup["提取到 config.group_nodes"]
CheckKey --> IsNormal{"普通节点?"}
IsNormal --> |是| ExtractPosition["提取位置和处理方向到 config.node_configs"]
ExtractFileConfig --> NextKey
ExtractExternal --> NextKey
ExtractAnchor --> NextKey
ExtractSticker --> NextKey
ExtractGroup --> NextKey
StorePureNode["存储纯净节点到 pipeline"] --> NextKey
NextKey --> CheckKey
...
**Diagram sources**
- [configSplitter.ts](file://src/core/parser/configSplitter.ts#L19-L141)
...
4. **新增**：遍历 `config.sticker_nodes`，为每个便签节点创建以 `stickerMarkPrefix`、节点名和文件名组成的键，并添加相应数据
5. **新增**：遍历 `config.group_nodes`，为每个分组节点创建以 `groupMarkPrefix`、节点名和文件名组成的键，并添加相应数据
6. 遍历纯 Pipeline 对象，为包含位置信息的节点添加 `configMark` 字段，支持新的配置格式

**更新** 增强了对分组节点的合并处理，分组节点的数据包含位置信息、颜色主题和子节点标签列表，不需要额外的位置信息包装。
...
AddAnchorNodes --> AddStickerNodes["添加便签节点"]
AddStickerNodes --> AddGroupNodes["添加分组节点"]
AddGroupNodes --> ProcessPipeline["处理纯 Pipeline 节点"]
ProcessPipeline --> HasPosition{"包含位置信息?"}
HasPosition --> |是| BuildMpeCode["构建 $__mpe_code 字段"]
HasPosition --> |否| KeepAsIs["保持原样"]
BuildMpeCode --> AddToMerged
KeepAsIs --> AddToMerged
AddToMerged --> NextNode
NextNode --> ProcessPipeline
ProcessPipeline --> |完成| ReturnMerged["返回合并后的 Pipeline"]
ReturnMerged --> End([结束])
```

**Diagram sources**
- [configSplitter.ts](file://src/core/parser/configSplitter.ts#L151-L448)

## 节点类型支持
系统支持多种节点类型的配置处理，每种节点类型都有其特定的配置结构和处理机制。

### 普通Pipeline节点
普通节点包含识别算法、动作类型和各种参数，支持位置信息和处理方向的配置。

### 外部节点（External Nodes）
外部节点用于引用其他文件中的节点，包含位置信息和处理方向，但不包含实际的识别和动作配置。

### 锚点节点（Anchor Nodes）
锚点节点用于流程控制，支持跳转回退功能，包含位置信息和处理方向。

### **新增** 便签节点（Sticker Nodes）
便签节点是一种特殊的配置节点，用于提供额外的注释和说明信息。

#### 便签节点特征
- **标签**：便签的标题，用于标识便签内容
- **内容**：便签的具体文本内容，支持多行编辑
- **颜色主题**：支持多种颜色主题，包括黄色、绿色、蓝色、粉色和紫色
- **尺寸**：支持自定义宽度和高度
- **位置信息**：便签节点的位置信息存储在配置对象中

#### 便签节点配置结构
便签节点的配置数据结构如下：
```typescript
{
  position: Position;      // 便签位置
  content: string;         // 便签内容
  color: StickerColorTheme; // 颜色主题
  width?: number;          // 宽度
  height?: number;         // 高度
}
```
...
- **子节点**：包含子节点的标签列表
- **尺寸**：支持自定义宽度和高度
- **位置信息**：分组节点的位置信息存储在配置对象中

#### 分组节点配置结构
分组节点的配置数据结构如下：
```typescript
{
  position: Position;      // 分组位置
  color: GroupColorTheme;  // 颜色主题
  childrenLabels: string[]; // 子节点标签列表
  width?: number;          // 宽度
  height?: number;         // 高度
}
```
...
- [StickerNode.tsx](file://src/components/flow/nodes/StickerNode.tsx#L13-L48)
...
**Diagram sources**
- [configSplitter.ts](file://src/core/parser/configSplitter.ts#L471-L485)
...
2. **编辑过程**：在编辑过程中，所有配置信息（如节点位置、视口状态、便签内容等）都存储在内存中的 `fileStore` 和 `configStore` 中，用户界面实时反映这些状态。
...
**更新** 增强了对分组节点和便签节点的处理流程，这些节点作为配置节点参与分割和合并过程，但不参与实际的执行流程。

这一完整的工作流程确保了配置信息的一致性和可靠性，为用户提供灵活而强大的配置管理能力。

**Section sources**
- [importer.ts](file://src/core/parser/importer.ts#L155-L507)
- [exporter.ts](file://src/core/parser/exporter.ts#L42-L243)
