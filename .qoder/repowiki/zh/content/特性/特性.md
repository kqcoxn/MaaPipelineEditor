...
- [IconShell.tsx](file://src/components/iconfonts/IconShell.tsx)
...
5. [关键逻辑深入解析](#关键逻辑深入解析)
6. [Shell 动作新特性](#shell-动作新特性)
7. [旧配置迁移支持能力](#旧配置迁移支持能力)
8. [总结](#总结)
...
本次更新引入了重要的 Shell 动作功能，为 ADB 设备提供了直接的 shell 命令执行能力。这是一个重要的新特性，显著扩展了 MaaPipelineEditor 的自动化能力。

### 动作定义与字段配置

Shell 动作在 `actionFields` 中定义如下：
...
对应的字段配置在 `actionFieldSchema` 中定义：
...
为了支持 Shell 动作，系统新增了相应的 UI 组件：

1. **图标支持**：新增了 `IconShell` 组件，用于在节点中显示 Shell 动作的图标
2. **图标注册**：在 `iconfonts/index.tsx` 中注册了 `icon-shell` 图标名称
3. **节点模板**：Shell 动作已集成到节点模板系统中，用户可以通过模板快速创建 Shell 节点

### 协议兼容性

根据 MaaFramework 协议规范，Shell 动作具有以下特性：

- **平台限制**：仅对 ADB 控制器有效
- **超时控制**：支持设置命令执行超时时间，默认 20000 毫秒
- **输出获取**：命令输出可以通过 `MaaTaskerGetActionDetail` 在动作详情中获取
- **参数支持**：支持标准的运行时参数替换，如 `{ENTRY}`、`{NODE}`、`{IMAGE}` 等

### 使用场景

Shell 动作适用于以下自动化场景：

- **设备配置**：执行系统设置命令，如 `settings put global animator_duration_scale 0`
- **系统信息获取**：查询设备信息，如 `getprop ro.build.version.sdk`
- **环境诊断**：执行诊断命令，检查设备状态和配置
- **动态配置**：根据运行时条件动态调整设备配置

**Section sources**
- [fields.ts](file://src/core/fields/action/fields.ts#L132-L135)
- [schema.ts](file://src/core/fields/action/schema.ts#L229-L242)
- [IconShell.tsx](file://src/components/iconfonts/IconShell.tsx#L1-L33)
- [index.tsx](file://src/components/iconfonts/index.tsx#L316-L317)
- [3.1-任务流水线协议.md](file://instructions/maafw-guide/3.1-任务流水线协议.md#L1152-L1166)
- [API大全.md](file://instructions/maafw-golang-binding/API大全.md#L1877-L1880)
...
## 总结
MaaPipelineEditor 的编译器与解析器特性通过模块化、分层的设计，实现了可视化流程与 JSON 协议之间的高效、可靠双向转换。解析器（importer）能够智能地处理版本迁移和配置元数据，将复杂的 JSON 结构还原为直观的 Flow 数据。编译器（exporter）则通过严格的类型匹配和版本检测，确保生成的 JSON 完全符合 MaaFramework 协议。

**更新** 本次更新新增了 Shell 动作功能，为 ADB 设备提供了直接的 shell 命令执行能力。这一重要特性通过新增的动作定义、字段配置、UI 组件支持和协议兼容性，显著扩展了编辑器的自动化能力。Shell 动作支持超时控制、输出获取和运行时参数替换等功能，适用于设备配置、系统信息获取和环境诊断等多种自动化场景。

这些核心机制不仅保证了功能的正确性，还为用户从旧工具（如 YAMaaPE）迁移提供了坚实的支持，体现了项目在用户体验和兼容性方面的深度考量。