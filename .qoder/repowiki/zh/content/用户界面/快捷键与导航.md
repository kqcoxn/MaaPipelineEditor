...
## 更新摘要
**变更内容**
- 新增模态框检测机制，防止全局撤销/重做快捷键在模态框打开时干扰用户操作
- 更新全局快捷键处理流程，增加模态框状态检查
- 增强快捷键冲突处理，提升用户体验
...
**更新** 新增模态框检测机制，确保在模态框打开时不会干扰用户的输入操作。

## 项目结构
围绕快捷键与导航的关键模块分布如下：
- 全局快捷键：在应用入口订阅全局键盘事件，拦截并处理撤销/重做、Delete键重定向等，新增模态框检测防止冲突。
...
## 核心组件
- 全局快捷键 Hook：统一处理撤销/重做、Delete键重定向，避免在输入框中触发，并向画布传递删除事件。**新增模态框检测功能**。
...
**更新** 全局快捷键现在包含模态框检测机制，确保在模态框打开时不干扰用户操作。
...
**更新** 新增模态框检测流程，确保快捷键在模态框状态下不会干扰用户输入。
...
- 撤销/重做：支持 Ctrl+Z（Mac 使用 Cmd+Z），在输入框中不触发；支持 Ctrl+Y 或 Ctrl+Shift+Z（Mac 使用 Cmd+Shift+Z）。**新增模态框检测**。
- 输入框保护：若当前焦点在可编辑元素（输入框、文本域、内容可编辑元素），则忽略快捷键，避免干扰输入。
- 模态框检测：**新增功能**，检查是否存在可见的模态框，如果存在则忽略快捷键，确保用户能够正常输入。
...
CheckUndo --> |是| CheckModal1{"模态框检测"}
CheckModal1 --> |关闭| Undo["阻止默认/冒泡<br/>调用 FlowStore.undo()<br/>提示成功/警告"]
CheckModal1 --> |开启| End1(["忽略快捷键"])
CheckUndo --> |否| CheckRedo{"是否 Ctrl/Cmd+Y 或 Ctrl/Cmd+Shift+Z?"}
CheckRedo --> |是| CheckModal2{"模态框检测"}
CheckModal2 --> |关闭| Redo["阻止默认/冒泡<br/>调用 FlowStore.redo()<br/>提示成功/警告"]
CheckModal2 --> |开启| End2(["忽略快捷键"])
CheckRedo --> |否| End3(["交由其他处理"])
Redirect --> End4(["结束"])
Undo --> End4
Redo --> End4
End1 --> End4
...
- [useGlobalShortcuts.ts:19-26](file://src/hooks/useGlobalShortcuts.ts#L19-L26)
- [useGlobalShortcuts.ts:87-90](file://src/hooks/useGlobalShortcuts.ts#L87-L90)
...
### 模态框检测机制
**新增功能** 模态框检测机制用于防止全局快捷键在模态框打开时干扰用户操作。

- 检测原理：通过查询 DOM 中具有 `ant-modal-wrap` 类名的元素来判断模态框状态。
- 可见性判断：检查元素的 CSS 样式，确保 `display` 不为 `none` 且 `visibility` 不为 `hidden`。
- 实现细节：使用 `window.getComputedStyle()` 获取元素样式，确保准确判断模态框的可见状态。
...
- **新增模态框检测** 依赖 Ant Design 的模态框样式类名进行状态判断。
...
- **新增模态框检测性能**：DOM 查询操作在每次按键时执行，但只检查可见性状态，性能开销较小。
...
- 快捷键无效
  ...
  - **新增**：检查模态框是否处于打开状态，模态框打开时快捷键会被自动忽略。
- 复制/粘贴异常
  - 确认存在选中节点/边，且剪贴板内容非空。
...
## 结论
本系统通过全局快捷键与画布快捷键的协同、工具面板的布局与对齐能力、以及面板定位与无障碍设计，构建了高效、可访问且可扩展的快捷键与导航体系。**新增的模态框检测机制进一步提升了用户体验，确保在模态框打开时不会干扰用户的输入操作**。配置存储与动态替换机制进一步增强了灵活性。建议在团队协作中统一快捷键约定，并持续优化焦点管理与提示信息，以提升用户体验。
...
**更新** 模态框打开时，撤销/重做快捷键会被自动忽略，确保用户能够正常输入。
...
- **检测目标**：查找具有 `ant-modal-wrap` 类名的元素
- **可见性判断**：检查元素的 CSS 样式属性
  - `display !== "none"`：确保元素不是隐藏状态
  - `visibility !== "hidden"`：确保元素不是透明隐藏状态
- **实现方式**：使用 `window.getComputedStyle()` 获取元素样式
- **应用场景**：在用户打开配置面板、文件对话框、确认对话框等模态框时，自动忽略全局快捷键
