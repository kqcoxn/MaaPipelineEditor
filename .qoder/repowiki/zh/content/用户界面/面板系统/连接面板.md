...
## 更新摘要
**变更内容**
- WlRoots表单新增套接字路径选择和验证功能，支持可用套接字下拉选择和当前连接检测
- 添加了预设套接字路径列表和手动输入路径的双重选择机制
- 增强了套接字路径的验证和冲突处理逻辑
- 改进了用户界面的交互体验和错误提示
...
**更新** WlRoots表单经过重大增强，现在支持套接字路径的自动发现、预设选择和手动输入验证功能，显著提升了Linux Wayland环境下的连接便利性和可靠性。
...
**图表来源**
- [ConnectionPanel.tsx:1-816](file://src/components/panels/main/ConnectionPanel.tsx#L1-L816)
...
- **WlRootsForm**：WlRoots连接配置界面（**新增套接字路径选择功能**）
...
**图表来源**
- [ConnectionPanel.tsx:49-816](file://src/components/panels/main/ConnectionPanel.tsx#L49-L816)
- [index.ts:1-9](file://src/components/panels/main/connection/index.ts#L1-L9)
- [utils.ts:1-26](file://src/components/panels/main/connection/utils.ts#L1-L26)
...
**更新** WlRoots表单经过重大增强，现在支持套接字路径的自动发现、预设选择和手动输入验证功能：

#### 套接字路径选择机制

```mermaid
flowchart TD
WlRootsForm[WlRootsForm组件] --> PresetList[预设套接字路径列表]
WlRootsForm --> ManualInput[手动输入套接字路径]
WlRootsForm --> Validation[路径验证和冲突处理]
PresetList --> AutoDiscover[自动发现可用套接字]
AutoDiscover --> DataSource[数据源: sockets数组]
DataSource --> RenderList[渲染列表项]
RenderList --> SelectPreset[选择预设路径]
SelectPreset --> ClearManual[清空手动输入]
SelectPreset --> UpdateSelection[更新选择状态]
ManualInput --> TextInput[文本输入框]
TextInput --> ValidateInput{验证输入有效?}
ValidateInput --> |有效| EnableConnect[启用连接按钮]
ValidateInput --> |无效| ShowWarning[显示警告信息]
Validation --> ConflictCheck{检查路径冲突?}
ConflictCheck --> |冲突| ShowWarning
ConflictCheck --> |无冲突| ProceedConnect[继续连接流程]
ShowWarning --> End([结束])
ProceedConnect --> End
```

**图表来源**
- [WlRootsForm.tsx:26-36](file://src/components/panels/main/connection/WlRootsForm.tsx#L26-L36)
- [WlRootsForm.tsx:128-150](file://src/components/panels/main/connection/WlRootsForm.tsx#L128-L150)
...
**章节来源**
- [WlRootsForm.tsx:1-174](file://src/components/panels/main/connection/WlRootsForm.tsx#L1-L174)
...
- [MFWProtocol.ts:1-939](file://src/services/protocols/MFWProtocol.ts#L1-L939)
...
- **WlRootsForm**：WlRoots连接表单组件（**新增套接字路径选择功能**）
- **MethodConfig**：方法配置组件
- **utils**：平台检测和工具函数
...
**更新** WlRoots连接现在支持手动输入socket路径，简化了连接流程：
...
CheckPlatform --> |包含"mac" or "darwin"| MacOS[macOS平台]
...
**更新** WlRoots表单的套接字路径选择和验证功能是本次更新的重点，它提供了两种连接方式：自动发现的预设路径选择和手动输入的精确控制，大大提升了Linux Wayland环境下的连接便利性和可靠性。用户可以根据自己的需求选择最适合的连接方式，同时系统提供了完善的验证和冲突处理机制，确保连接的稳定性和安全性。
