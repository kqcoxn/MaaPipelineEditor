...
- UI 控件：位于工具面板，提供一键自动布局、对齐、间距调整、连接线路径还原等入口
...
graph TB
subgraph "工具面板"
LP["LayoutPanel.tsx<br/>布局工具按钮"]
end
subgraph "布局逻辑"
LH["LayoutHelper<br/>auto()/align()"]
ELK["ELKJS 分层布局"]
end
...
## 核心组件
- 布局工具面板：提供“居中对齐/顶部对齐/底部对齐”、“水平/垂直间距调整”、“连接线路径还原”、“自动布局”、“导出布局为图片”等入口
...
U->>LP : 点击“自动布局”
LP->>LH : 调用 auto()
LH->>ELK : 提交节点/边结构与布局选项
ELK-->>LH : 返回布局后节点坐标
LH->>GS : replace(布局节点, 边)
GS->>NS : 写入节点位置变更
NS-->>RF : 触发节点渲染
RF-->>U : 显示优化后的布局
```
...
Choose --> |导出图片| Export["调用 saveNodesToImage()"]
...
## 结论
布局面板通过“自动布局 + 对齐工具 + 磁吸对齐 + 视口管理”的组合，提供了从宏观到微观的完整布局优化能力。其与 React Flow 的深度集成确保了布局结果的即时可视化与交互体验，适合在复杂工作流场景下提升可读性与一致性。
...
### 使用指南
- 一键优化布局
  - 在工具面板点击“自动布局”，系统将基于 ELKJS 的分层算法重新排列节点
- 手动调整节点位置
