...
## 架构总览
配置面板通过 Zustand 管理全局配置状态，各配置区域通过受控组件读写状态；本地服务通过 WebSocket 与后端通信，AI 配置通过 OpenAIChat 调用远端 API；文件配置与文件存储协同实现本地持久化与后端同步。
...
participant LS as "LocalServiceSection"
participant WS as "LocalWebSocketServer"
participant CFG as "ConfigProtocol"
U->>CP : 打开配置面板
CP->>CS : 读取 configs 状态
PC->>CS : setConfig(key, value)
CS-->>PC : 状态更新
LS->>CS : 读取 wsPort
LS->>WS : setPort(wsPort)
U->>LS : 修改端口
LS->>CS : setConfig("wsPort", port)
CS-->>WS : 触发 setPort
U->>LS : 打开后端配置
LS->>CFG : requestGetConfig()
CFG-->>U : 显示后端配置弹窗
U->>CFG : 保存配置
CFG-->>U : 重载配置并提示
```
...
- [LocalServiceSection.tsx:17-86](file://src/components/panels/config/LocalServiceSection.tsx#L17-L86)
...
## 结论
配置面板通过清晰的区域划分、完善的联动与迁移逻辑、稳健的本地服务与 AI 交互，提供了易用且可扩展的配置管理体验。其模块化设计便于新增配置项与协议扩展，同时具备良好的错误处理与用户体验反馈。
