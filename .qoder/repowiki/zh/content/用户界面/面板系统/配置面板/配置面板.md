...
**更新** 新增边走线模式配置项，支持曲线和直角两种连接线样式；新增"连接空白处时创建"配置开关，允许用户控制智能连接到空白处功能的启用/禁用。
...
- 连接处理：Flow.tsx 负责连接事件处理与空白处节点创建逻辑
- 节点添加：NodeAddPanel.tsx 负责空白处节点添加面板的显示与交互
...
- 连接处理器：负责连接事件的捕获、验证与空白处节点创建逻辑
- 节点添加面板：负责空白处节点创建的交互界面与模板选择
...
## 架构总览
配置面板通过 Zustand 管理全局配置状态，各配置区域通过受控组件读写状态；本地服务通过 WebSocket 与后端通信，AI 配置通过 OpenAIChat 调用远端 API；文件配置与文件存储协同实现本地持久化与后端同步。新增的边走线模式配置项通过 edges.tsx 的边渲染引擎实现不同的连接线样式；新增的"连接空白处时创建"配置项通过 Flow.tsx 的连接处理器实现智能连接到空白处的功能。
...
PC->>CS : setConfig("quickCreateNodeOnConnectBlank", enabled)
CS-->>PC : 状态更新
CS-->>FLOW : 触发连接处理更新
FLOW->>FLOW : 监听连接事件
FLOW->>NAP : 根据配置决定是否显示节点面板
NAP-->>U : 显示节点添加面板
```
...
- 功能职责
  ...
  - **新增** 连接空白处时创建：从节点拖出连接线，如果终点落在画布空白处，则在落点直接弹出节点添加面板，方便继续选择要创建的节点类型
- 交互增强
  - 一键应用到所有节点的端点方向
  - **新增** 智能连接到空白处功能，通过 Switch 组件控制
...
- 实现细节
  ...
  - endedOnBlank：通过 connectionState 判断连接是否有效且目标为空白
  - suppressNextPaneClickRef：防止面板闪烁的防抖机制
  - setNodeAddPanelPos/setNodeAddPanelVisible：控制节点添加面板的显示位置和可见性

**章节来源**
- [Flow.tsx:270-323](file://src/components/Flow.tsx#L270-L323)
- [Flow.tsx:331-361](file://src/components/Flow.tsx#L331-L361)

### 节点添加面板（NodeAddPanel.tsx）
- 功能职责
  - **新增** 空白处节点创建的交互界面
  - 模板搜索与筛选
  - 节点预览与选择
...
- **新增依赖**：连接处理器依赖 quickCreateNodeOnConnectBlank 配置控制智能连接功能
- **新增依赖**：节点添加面板依赖 Flow.tsx 的连接事件处理逻辑
...
+setConfig("quickCreateNodeOnConnectBlank", v)
+setConfig("configHandlingMode", v)
}
...
FlowHandler --> ConfigStore : "quickCreateNodeOnConnectBlank"
NodeAddPanel --> FlowHandler : "连接事件处理"
```
...
- **新增性能考量**：连接处理器使用防抖机制（suppressNextPaneClickRef）避免面板闪烁，提升用户体验
- **新增性能考量**：节点添加面板使用虚拟滚动和模板缓存，减少渲染开销
...
- [edges.tsx:206-233](file://src/components/flow/edges.tsx#L206-L233)
- [Flow.tsx:316-318](file://src/components/Flow.tsx#L316-L318)
- [NodeAddPanel.tsx:415-440](file://src/components/panels/main/NodeAddPanel.tsx#L415-L440)
...
- **新增故障排查**：连接空白处时创建功能异常
  - 确认 quickCreateNodeOnConnectBlank 配置为 true
  - 检查连接事件是否正常触发，连接状态是否正确
  - 验证节点添加面板的显示逻辑和位置计算
  - 确认防抖机制（suppressNextPaneClickRef）不会阻止正常交互
...
- [Flow.tsx:286-323](file://src/components/Flow.tsx#L286-L323)

## 结论
配置面板通过清晰的区域划分、完善的联动与迁移逻辑、稳健的本地服务与 AI 交互，提供了易用且可扩展的配置管理体验。其模块化设计便于新增配置项与协议扩展，同时具备良好的错误处理与用户体验反馈。新增的边走线模式配置项进一步增强了用户对连接线样式的控制能力，提供了更丰富的可视化表达方式。新增的"连接空白处时创建"功能显著提升了工作流创建效率，通过智能连接到空白处的便捷交互，让用户能够快速添加新的节点类型，无需手动定位和选择模板。
...
- **新增同步**：连接空白处配置通过 Flow.tsx 的连接处理器实时响应配置变化
...
- **新增** Pipeline 配置区域：节点属性导出形式、默认端点方向、一键应用端点方向、导出默认识别/动作、导出版本、忽略字段校验、JSON 缩进、配置处理方案、**连接空白处时创建**
...
- [PipelineConfigSection.tsx:139-163](file://src/components/panels/config/PipelineConfigSection.tsx#L139-L163)
...
- **新增验证策略**：连接空白处配置值验证，确保布尔值类型正确
- **新增验证策略**：连接处理器状态验证，确保连接事件正确处理
...
- 步骤
  ...
  - **新增步骤**：如涉及事件处理，需要在 Flow.tsx 或相关处理器中添加事件监听
  - **新增步骤**：如涉及 UI 交互，需要在对应的组件中实现交互逻辑
- 注意事项
  - 若存在联动关系（如 isExportConfig 与 configHandlingMode），在 setConfig 中处理同步
  - 为新字段提供合理的默认值与范围约束
  - 在 UI 中提供清晰的提示与帮助信息
  - **新增注意事项**：渲染层配置需要考虑性能影响和兼容性
  - **新增注意事项**：事件处理配置需要考虑用户体验和交互一致性
  - **新增注意事项**：UI 交互配置需要考虑可访问性和键盘导航支持
...
### 连接空白处时创建功能实现详解
- 配置类型定义：quickCreateNodeOnConnectBlank 布尔值，默认为 true
- 连接事件处理：Flow.tsx 监听连接开始和结束事件
- 状态验证：endedOnBlank 通过 connectionState 判断连接是否结束在空白处
- 防抖机制：suppressNextPaneClickRef 防止面板闪烁
- 面板显示：setNodeAddPanelPos 和 setNodeAddPanelVisible 控制节点添加面板
- 用户交互：支持键盘导航和鼠标选择节点模板

**章节来源**
- [configStore.ts:114](file://src/stores/configStore.ts#L114)
