...
- 工具栏状态管理：统一维护右侧面板显示状态、默认导入/导出操作等
...
- [JsonPreviewButton.tsx:1-33](file://src/components/panels/toolbar/JsonPreviewButton.tsx#L1-L33)
...
- JSON 预览按钮：控制右侧 JSON 浮动面板的显示/隐藏
- 工具栏状态管理：集中管理右侧面板显示状态、默认导入/导出操作、当前右侧面板类型等
...
+currentRightPanel : "json"|"field"|"edge"|null
+defaultImportAction
+defaultExportAction
+setJsonPanelVisible()
+toggleJsonPanel()
...
ImportButton --> ToolbarStore : "读写状态"
ExportButton --> ToolbarStore : "读写状态"
JsonPreviewButton --> ToolbarStore : "读写状态"
```
...
- [JsonPreviewButton.tsx:11-33](file://src/components/panels/toolbar/JsonPreviewButton.tsx#L11-L33)
- [toolbarStore.ts:30-146](file://src/stores/toolbarStore.ts#L30-L146)
...
**章节来源**
- [ToolbarPanel.tsx:11-21](file://src/components/panels/main/ToolbarPanel.tsx#L11-L21)
- [ToolbarPanel.module.less:1-71](file://src/styles/ToolbarPanel.module.less#L1-L71)
...
participant TS as "ToolbarStore"
...
**章节来源**
- [ImportButton.tsx:19-235](file://src/components/panels/toolbar/ImportButton.tsx#L19-L235)
- [toolbarStore.ts:52-67](file://src/stores/toolbarStore.ts#L52-L67)
...
**章节来源**
- [ExportButton.tsx:24-316](file://src/components/panels/toolbar/ExportButton.tsx#L24-L316)
- [toolbarStore.ts:61-93](file://src/stores/toolbarStore.ts#L61-L93)

### JSON 预览按钮组件
- 功能：切换右侧 JSON 浮动面板的显示/隐藏
- 状态：根据面板显示状态应用激活态样式类

**章节来源**
- [JsonPreviewButton.tsx:11-33](file://src/components/panels/toolbar/JsonPreviewButton.tsx#L11-L33)
- [toolbarStore.ts:102-118](file://src/stores/toolbarStore.ts#L102-L118)
...
## 依赖关系分析
- 工具栏按钮与状态管理：
  - 导入/导出按钮通过状态管理读取默认操作并更新默认值
  - JSON 预览按钮通过状态管理切换面板显示
- 工具栏与面板联动：
  - 设置 JSON 面板显示时，同步设置当前右侧面板类型为 "json"
  - 切换非 JSON 面板时，自动关闭 JSON 面板
- 工具栏与节点添加面板：
...
```mermaid
graph LR
IB["ImportButton"] --> TS["ToolbarStore"]
EB["ExportButton"] --> TS
JP["JsonPreviewButton"] --> TS
TS --> RP["右侧面板(当前类型)"]
TS --> JPVis["JSON面板可见性"]
...
- [ImportButton.tsx:20-23](file://src/components/panels/toolbar/ImportButton.tsx#L20-L23)
- [ExportButton.tsx:25-28](file://src/components/panels/toolbar/ExportButton.tsx#L25-L28)
- [JsonPreviewButton.tsx:12-17](file://src/components/panels/toolbar/JsonPreviewButton.tsx#L12-L17)
...
- 面板位置计算与约束在节点添加面板中进行，避免超出视口导致的重排
...
- JSON 面板无法显示：
  - 检查工具栏状态管理中 JSON 面板可见性标志位
  - 确认当前右侧面板类型是否被设置为 "json"
- 快捷键无效：
  - 确认全局快捷键钩子已启用
...
- [toolbarStore.ts:102-135](file://src/stores/toolbarStore.ts#L102-L135)
- [useGlobalShortcuts.ts:134-146](file://src/hooks/useGlobalShortcuts.ts#L134-L146)

## 结论
工具栏面板通过清晰的职责划分与状态管理，实现了导入导出、工作流操作与视图控制的统一入口。节点添加面板提供了高效的节点创建体验，配合快捷键与键盘导航进一步提升了操作效率。通过合理的条件启用与联动机制，工具栏在不同场景下都能保持良好的可用性与一致性。