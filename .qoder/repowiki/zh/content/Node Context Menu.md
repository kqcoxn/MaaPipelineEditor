...
## 更新摘要
**变更内容**
- 工具模块重构：UI工具从 src/utils/ 移动到 src/utils/ui/，包含剪贴板、截图和面板定位功能
- 剪贴板功能深度集成：在NodeAddPanel中新增useClipboardStore钩子，实现右键面板的粘贴功能
- 智能定位功能增强：panelPosition模块提供更精确的面板位置计算和边界约束
- 截图功能集成：snapper模块提供节点截图保存功能
...
**更新** 工具模块重构已完成，UI工具现位于 src/utils/ui/ 目录下，包括剪贴板操作、截图保存和面板定位等功能，提升了代码组织的清晰度和可维护性。
...
subgraph "工具模块重构"
L[clipboard.ts<br/>UI剪贴板工具]
M[snapper.ts<br/>截图保存工具]
N[panelPosition.ts<br/>面板定位工具]
end
subgraph "节点类型定义"
O[constants.ts<br/>节点类型枚举]
P[types.ts<br/>数据类型定义]
end
subgraph "存储层"
Q[flow store<br/>状态管理]
R[debugStore.ts<br/>调试状态]
S[selectionSlice.ts<br/>选区状态]
T[clipboardStore.ts<br/>剪贴板状态]
end
A --> B
A --> C
A --> D
...
**更新** 工具模块已完成重构，UI相关工具从 src/utils/ 移动到 src/utils/ui/ 目录下，提升了代码组织的清晰度和可维护性：

```mermaid
graph TB
subgraph "重构前结构"
A[src/utils/<br/>剪贴板工具]
B[src/utils/<br/>截图工具]
C[src/utils/<br/>定位工具]
end
subgraph "重构后结构"
D[src/utils/ui/<br/>clipboard.ts<br/>UI剪贴板操作]
E[src/utils/ui/<br/>snapper.ts<br/>截图保存功能]
F[src/utils/ui/<br/>panelPosition.ts<br/>面板定位计算]
end
A --> D
B --> E
C --> F
...
clipboard.ts 提供了完整的剪贴板操作工具类：

```mermaid
classDiagram
class ClipboardHelper {
+write(content, options) Promise~boolean~
+writeString(content, options) Promise~boolean~
...
ClipboardHelper --> ClipboardOptions : "使用"
```

**图表来源**
- [clipboard.ts:3-63](file://src/utils/ui/clipboard.ts#L3-L63)

### 截图工具函数

snapper.ts 提供了节点截图保存功能：

```mermaid
flowchart TD
Start([开始截图保存]) --> CheckNodes{检查节点数量}
CheckNodes --> |有选中节点| UseSelected[使用选中节点]
CheckNodes --> |无选中节点| UseAll[使用所有节点]
UseSelected --> CalcBounds[计算节点边界]
UseAll --> CalcBounds
CalcBounds --> GetViewport[获取视口元素]
GetViewport --> CalcViewport[计算视口变换]
CalcViewport --> ConvertToImage[转换为PNG图像]
ConvertToImage --> DownloadImage[下载图片文件]
DownloadImage --> ShowSuccess[显示成功消息]
```
...
panelPosition.ts 提供了精确的面板位置计算和边界约束：

```mermaid
graph LR
subgraph "坐标转换"
A[画布坐标<br/>canvasToScreen]
B[屏幕坐标<br/>screenToCanvas]
end
subgraph "位置约束"
C[边界限制<br/>constrainPosition]
end
subgraph "嵌入位置"
D[节点嵌入<br/>calculateEmbeddedPosition]
E[连线嵌入<br/>calculateEdgeEmbeddedPosition]
end
A --> D
B --> E
C --> D
...
**图表来源**
- [panelPosition.ts:15-262](file://src/utils/ui/panelPosition.ts#L15-L262)
...
I[graphSlice 粘贴逻辑]
J[clipboard.ts 剪贴板工具]
K[snapper.ts 截图工具]
L[panelPosition.ts 定位工具]
end
subgraph "状态管理层"
M[Flow Store]
N[Debug Store]
...
### 工具模块性能优化
- **更新** UI工具模块重构后，工具函数按需加载，减少初始包体积
- **更新** panelPosition.ts 提供节流函数 throttling，避免频繁的定位计算
- **更新** snapper.ts 优化截图生成流程，减少DOM操作次数
- **更新** clipboard.ts 使用浏览器原生API，提升剪贴板操作性能
...
| 工具模块加载失败 | UI工具无法使用 | 模块路径错误 | 检查 src/utils/ui/ 路径 |
| 截图保存失败 | 节点截图无法保存 | DOM元素缺失 | 检查画布元素存在性 |
| 面板定位异常 | 面板位置计算错误 | 视口状态异常 | 检查视口参数 |
...
**更新** 当使用重构后的工具模块时，需要检查：

1. **模块路径验证**：确认 src/utils/ui/ 目录存在且包含所需文件
2. **剪贴板工具验证**：检查 clipboard.ts 是否正常加载
3. **截图工具验证**：确认 snapper.ts 的截图功能可用
4. **定位工具验证**：检查 panelPosition.ts 的定位算法
...
**更新** 工具模块重构已完成，UI工具现位于 src/utils/ui/ 目录下，包括剪贴板操作、截图保存和面板定位等功能，提升了代码组织的清晰度和可维护性。重构后的工具模块提供了更好的性能和更清晰的职责分离。
...
11. **工具模块重构**：UI工具分类管理，提升代码组织清晰度
12. **截图功能**：支持节点截图保存，便于分享和记录
13. **面板定位**：精确的面板位置计算和边界约束
14. **性能优化**：重构后的工具模块提供更好的性能表现
