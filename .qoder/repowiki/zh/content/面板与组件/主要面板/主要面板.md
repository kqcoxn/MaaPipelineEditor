# 主要面板

<cite>
**本文引用的文件**
- [ConnectionPanel.tsx](file://src/components/panels/main/ConnectionPanel.tsx)
- [NodeListPanel.tsx](file://src/components/panels/main/node-list/NodeListPanel.tsx)
- [FieldPanel.tsx](file://src/components/panels/main/FieldPanel.tsx)
- [FilePanel.tsx](file://src/components/panels/main/FilePanel.tsx)
- [ToolbarPanel.tsx](file://src/components/panels/main/ToolbarPanel.tsx)
- [GlobalPanel.tsx](file://src/components/panels/tools/GlobalPanel.tsx)
- [DraggablePanel.tsx](file://src/components/panels/common/DraggablePanel.tsx)
- [index.ts（连接面板入口）](file://src/components/panels/main/connection/index.ts)
- [types.ts（节点列表类型）](file://src/components/panels/main/node-list/types.ts)
- [index.ts（Flow Store）](file://src/stores/flow/index.ts)
- [mfwStore.ts](file://src/stores/mfwStore.ts)
- [panelOccupancyStore.ts](file://src/stores/panelOccupancyStore.ts)
- [debugSessionStore.ts](file://src/stores/debugSessionStore.ts)
- [DebugProtocolClient.ts](file://src/services/protocols/DebugProtocolClient.ts)
- [types.ts（调试类型）](file://src/features/debug/types.ts)
- [ImportButton.tsx](file://src/components/panels/toolbar/ImportButton.tsx)
- [ExportButton.tsx](file://src/components/panels/toolbar/ExportButton.tsx)
</cite>

## 更新摘要
**所做更改**
- 新增运行状态徽章功能章节，详细介绍GlobalPanel中新增的实时调试会话状态反馈机制
- 更新GlobalPanel组件分析，增加运行状态徽章的实现细节
- 新增调试协议与状态同步机制说明
- 完善调试会话状态管理的架构设计

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖分析](#依赖分析)
7. [性能考虑](#性能考虑)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本文件聚焦"主要面板系统"的技术实现，覆盖以下核心面板：
- 连接面板：设备发现、连接配置与状态管理
- 节点列表面板：节点浏览、筛选、分组与定位
- 字段面板：节点字段编辑、邻接信息、可拖拽/内联布局
- 文件面板：多文件标签页、拖拽排序、本地文件联动
- 工具面板（横向工具栏）：导入、导出、JSON 预览等快捷入口
- **新增** 全局面板：包含运行状态徽章的实时调试会话状态反馈

文档将深入解析各面板的数据流、状态同步机制、布局与交互模式，并提供扩展与优化建议。

## 项目结构
主要面板位于前端 src/components/panels 目录，按"主面板/工具面板/通用组件"分层组织；状态管理由多个 zustand store 提供，包括 Flow Store、MFW Store、面板占位互斥系统、**调试会话状态管理**等。

```mermaid
graph TB
subgraph "主面板"
CP["连接面板<br/>ConnectionPanel.tsx"]
NLP["节点列表面板<br/>NodeListPanel.tsx"]
FP["字段面板<br/>FieldPanel.tsx"]
FIP["文件面板<br/>FilePanel.tsx"]
end
subgraph "工具面板"
TP["横向工具栏<br/>ToolbarPanel.tsx"]
GP["全局面板<br/>GlobalPanel.tsx"]
IMP["导入按钮<br/>ImportButton.tsx"]
EXP["导出按钮<br/>ExportButton.tsx"]
end
subgraph "通用组件"
DP["可拖动面板包装<br/>DraggablePanel.tsx"]
CONNIDX["连接面板入口导出<br/>connection/index.ts"]
end
subgraph "状态管理"
FS["Flow Store<br/>stores/flow/index.ts"]
MFW["MFW Store<br/>stores/mfwStore.ts"]
POS["面板占位互斥 Store<br/>stores/panelOccupancyStore.ts"]
DSS["调试会话状态 Store<br/>stores/debugSessionStore.ts"]
end
TP --> IMP
TP --> EXP
CP --> MFW
FP --> FS
NLP --> FS
FIP --> FS
FP --> DP
CP --> CONNIDX
FP --> POS
NLP --> POS
GP --> DSS
```

**图表来源**
- [ConnectionPanel.tsx:1-954](file://src/components/panels/main/ConnectionPanel.tsx#L1-L954)
- [NodeListPanel.tsx:1-426](file://src/components/panels/main/node-list/NodeListPanel.tsx#L1-L426)
- [FieldPanel.tsx:1-491](file://src/components/panels/main/FieldPanel.tsx#L1-L491)
- [FilePanel.tsx:1-165](file://src/components/panels/main/FilePanel.tsx#L1-L165)
- [ToolbarPanel.tsx:1-22](file://src/components/panels/main/ToolbarPanel.tsx#L1-L22)
- [GlobalPanel.tsx:1-363](file://src/components/panels/tools/GlobalPanel.tsx#L1-L363)
- [DraggablePanel.tsx:1-178](file://src/components/panels/common/DraggablePanel.tsx#L1-L178)
- [index.ts（连接面板入口）:1-10](file://src/components/panels/main/connection/index.ts#L1-L10)
- [index.ts（Flow Store）:1-124](file://src/stores/flow/index.ts#L1-L124)
- [mfwStore.ts:1-195](file://src/stores/mfwStore.ts#L1-L195)
- [panelOccupancyStore.ts:1-136](file://src/stores/panelOccupancyStore.ts#L1-L136)
- [debugSessionStore.ts:1-273](file://src/stores/debugSessionStore.ts#L1-L273)

**章节来源**
- [ConnectionPanel.tsx:1-954](file://src/components/panels/main/ConnectionPanel.tsx#L1-L954)
- [NodeListPanel.tsx:1-426](file://src/components/panels/main/node-list/NodeListPanel.tsx#L1-L426)
- [FieldPanel.tsx:1-491](file://src/components/panels/main/FieldPanel.tsx#L1-L491)
- [FilePanel.tsx:1-165](file://src/components/panels/main/FilePanel.tsx#L1-L165)
- [ToolbarPanel.tsx:1-22](file://src/components/panels/main/ToolbarPanel.tsx#L1-L22)
- [GlobalPanel.tsx:1-363](file://src/components/panels/tools/GlobalPanel.tsx#L1-L363)
- [DraggablePanel.tsx:1-178](file://src/components/panels/common/DraggablePanel.tsx#L1-L178)
- [index.ts（连接面板入口）:1-10](file://src/components/panels/main/connection/index.ts#L1-L10)
- [index.ts（Flow Store）:1-124](file://src/stores/flow/index.ts#L1-L124)
- [mfwStore.ts:1-195](file://src/stores/mfwStore.ts#L1-L195)
- [panelOccupancyStore.ts:1-136](file://src/stores/panelOccupancyStore.ts#L1-L136)
- [debugSessionStore.ts:1-273](file://src/stores/debugSessionStore.ts#L1-L273)

## 核心组件
- 连接面板：跨平台设备连接（ADB、Win32、PlayCover、Gamepad、WlRoots、macOS），设备列表刷新、连接/断开控制、方法选择与持久化
- 节点列表面板：节点搜索、类型筛选、分组折叠、高亮与定位、点击外部关闭、键盘 ESC 关闭
- 字段面板：节点字段编辑器、邻接信息、错误边界与修复、可拖拽/内联布局、进度遮罩
- 文件面板：文件名校验、标签页可拖拽排序、新增/删除、与本地文件面板联动
- 工具面板：导入/导出/JSON 预览，支持默认动作记忆与条件菜单项
- **新增** 全局面板：包含运行状态徽章的调试会话状态反馈，提供实时的运行状态指示

## 架构总览
面板系统采用"组件 + Store + 互斥系统"的分层架构：
- 组件负责 UI 行为与交互
- Store 提供全局状态（Flow、MFW、面板占位、**调试会话状态**）
- 互斥系统保障右侧区域面板的互斥占用与被动面板的协同
- **新增** 调试协议客户端负责与后端调试服务的实时通信

```mermaid
graph LR
UI["面板组件"] --> STORE["Zustand Store"]
STORE --> CORE["核心逻辑<br/>parser/fields/layout"]
UI --> UTIL["通用工具<br/>hooks/dnd-kit"]
UI --> THEME["主题/样式"]
subgraph "右侧区域互斥"
PO["面板占位互斥系统"]
end
UI --> PO
subgraph "调试协议层"
DPC["调试协议客户端"]
DSS["调试会话状态管理"]
end
DPC --> DSS
```

**图表来源**
- [index.ts（Flow Store）:1-124](file://src/stores/flow/index.ts#L1-L124)
- [mfwStore.ts:1-195](file://src/stores/mfwStore.ts#L1-L195)
- [panelOccupancyStore.ts:1-136](file://src/stores/panelOccupancyStore.ts#L1-L136)
- [debugSessionStore.ts:1-273](file://src/stores/debugSessionStore.ts#L1-L273)
- [DebugProtocolClient.ts:1-353](file://src/services/protocols/DebugProtocolClient.ts#L1-L353)

## 详细组件分析

### 连接面板（ConnectionPanel）
- 功能要点
  - 平台检测与可用标签页动态生成
  - 设备列表刷新与持久化参数（ADB 路径/地址、PlayCover 地址/UUID、Gamepad 类型/句柄、WlRoots Socket、macOS PID/方法）
  - 方法选择（截图/输入）默认策略与手动模式差异
  - 连接/断开控制、连接新设备的切换流程
  - 连接状态徽章与错误提示
- 数据流与状态同步
  - 读取 MFW Store 的连接状态、设备列表与错误信息
  - 通过协议桥接发起设备创建/断开请求
  - 首次打开时根据已连接设备自动定位 Tab 与选中项
- 交互模式
  - 切换标签页时懒加载并刷新设备列表
  - 手动模式禁用部分方法（如 RawByNetcat）
  - 支持"连接新设备"与"断开连接"的条件按钮

```mermaid
sequenceDiagram
participant U as "用户"
participant CP as "连接面板"
participant M as "MFW Store"
participant P as "协议桥接"
U->>CP : 打开面板
CP->>M : 读取连接状态/设备列表
alt 已连接
CP->>CP : 自动定位 Tab 与设备
else 未连接
CP->>CP : 切换标签页时刷新设备
end
U->>CP : 选择设备/方法并点击连接
CP->>P : 创建控制器请求
P-->>M : 更新连接状态/设备信息
M-->>CP : 触发 UI 状态变更
U->>CP : 点击断开/连接新设备
CP->>P : 断开/新建控制器
```

**图表来源**
- [ConnectionPanel.tsx:1-954](file://src/components/panels/main/ConnectionPanel.tsx#L1-L954)
- [mfwStore.ts:1-195](file://src/stores/mfwStore.ts#L1-L195)

**章节来源**
- [ConnectionPanel.tsx:1-954](file://src/components/panels/main/ConnectionPanel.tsx#L1-L954)
- [index.ts（连接面板入口）:1-10](file://src/components/panels/main/connection/index.ts#L1-L10)
- [mfwStore.ts:1-195](file://src/stores/mfwStore.ts#L1-L195)

### 节点列表面板（NodeListPanel）
- 功能要点
  - 节点关键词与类型筛选
  - 按节点类型分组与展开/折叠
  - 边连接数统计（入/出）
  - 点击节点选中并聚焦视图
  - 点击外部区域与 ESC 关闭
  - 响应式定位（锚点元素 + 窗口尺寸监听）
- 数据流与状态同步
  - 从 Flow Store 读取 nodes/edges 实时计算边连接数
  - 通过 Flow Store 更新节点选中状态与视图中心
- 交互模式
  - 悬停高亮、点击定位、键盘 ESC 关闭
  - 外部点击关闭（排除 Ant Design 弹出层）

```mermaid
flowchart TD
Start(["面板可见"]) --> Calc["计算边连接数"]
Calc --> Build["构建节点列表数据"]
Build --> Filter["关键词/类型过滤"]
Filter --> Group["按类型分组"]
Group --> Render["渲染列表与统计"]
Render --> Hover["悬停高亮"]
Render --> Click["点击节点选中并聚焦"]
Click --> Close["外部点击/ESC 关闭"]
```

**图表来源**
- [NodeListPanel.tsx:1-426](file://src/components/panels/main/node-list/NodeListPanel.tsx#L1-L426)
- [types.ts（节点列表类型）:1-64](file://src/components/panels/main/node-list/types.ts#L1-L64)
- [index.ts（Flow Store）:1-124](file://src/stores/flow/index.ts#L1-L124)

**章节来源**
- [NodeListPanel.tsx:1-426](file://src/components/panels/main/node-list/NodeListPanel.tsx#L1-L426)
- [types.ts（节点列表类型）:1-64](file://src/components/panels/main/node-list/types.ts#L1-L64)
- [index.ts（Flow Store）:1-124](file://src/stores/flow/index.ts#L1-L124)

### 字段面板（FieldPanel）
- 功能要点
  - 节点字段编辑器（Pipeline/External/Anchor/Sticker/Group）
  - 邻接信息 Tab 展示
  - 错误边界与节点修复（验证/修复/历史记录）
  - 布局模式：内联/可拖拽/固定
  - 进度遮罩与加载态
- 数据流与状态同步
  - 从 Flow Store 读取目标节点与更新节点
  - 通过面板占位互斥系统协调右侧区域占用
  - JSON 编辑器 Modal 保存节点数据并记录历史
- 交互模式
  - 拖拽面板标题栏移动位置并持久化
  - 被其他面板"挤出"时自动取消选中

```mermaid
sequenceDiagram
participant U as "用户"
participant FP as "字段面板"
participant FS as "Flow Store"
participant POS as "面板占位互斥"
participant DP as "可拖动面板包装"
U->>FP : 选中节点
FP->>FS : 读取目标节点
FP->>POS : 激活面板占用
FP-->>U : 渲染字段编辑器
U->>FP : 拖动标题栏
FP->>DP : 更新面板位置
U->>FP : 打开邻接信息/编辑 JSON
FP->>FS : 更新节点数据并保存历史
FP->>POS : 若被挤出则取消选中
```

**图表来源**
- [FieldPanel.tsx:1-491](file://src/components/panels/main/FieldPanel.tsx#L1-L491)
- [DraggablePanel.tsx:1-178](file://src/components/panels/common/DraggablePanel.tsx#L1-L178)
- [panelOccupancyStore.ts:1-136](file://src/stores/panelOccupancyStore.ts#L1-L136)
- [index.ts（Flow Store）:1-124](file://src/stores/flow/index.ts#L1-L124)

**章节来源**
- [FieldPanel.tsx:1-491](file://src/components/panels/main/FieldPanel.tsx#L1-L491)
- [DraggablePanel.tsx:1-178](file://src/components/panels/common/DraggablePanel.tsx#L1-L178)
- [panelOccupancyStore.ts:1-136](file://src/stores/panelOccupancyStore.ts#L1-L136)
- [index.ts（Flow Store）:1-124](file://src/stores/flow/index.ts#L1-L124)

### 文件面板（FilePanel）
- 功能要点
  - 文件名输入与校验（状态提示）
  - 多文件标签页、可拖拽排序
  - 新增/删除文件、切换当前文件
  - 与本地文件面板联动（显示/隐藏）
- 数据流与状态同步
  - 读取/写入文件列表与当前文件名
  - 通过配置 Store 控制本地文件面板显隐
  - DnD Kit 实现标签页拖拽排序

```mermaid
flowchart TD
Start(["打开文件面板"]) --> Name["输入文件名并校验"]
Name --> Tabs["渲染标签页列表"]
Tabs --> Drag["拖拽排序"]
Drag --> Edit["新增/删除文件"]
Edit --> Switch["切换当前文件"]
Switch --> Local["联动本地文件面板"]
```

**图表来源**
- [FilePanel.tsx:1-165](file://src/components/panels/main/FilePanel.tsx#L1-L165)

**章节来源**
- [FilePanel.tsx:1-165](file://src/components/panels/main/FilePanel.tsx#L1-L165)

### 工具面板（横向工具栏）
- 功能要点
  - 导入：粘贴板/文件，支持 Pipeline 与配置（分离模式）
  - 导出：粘贴板/文件/本地保存/部分导出/分离导出
  - JSON 预览：快捷查看
  - 默认动作记忆与条件菜单项
- 数据流与状态同步
  - 通过工具栏 Store 记忆默认导入/导出动作
  - 与配置 Store、文件 Store、WebSocket Store 协同决定菜单项可用性

```mermaid
sequenceDiagram
participant U as "用户"
participant TP as "工具栏"
participant IB as "导入按钮"
participant EB as "导出按钮"
participant CS as "配置 Store"
participant WS as "WS Store"
participant FS as "Flow Store"
U->>TP : 点击导入/导出
TP->>IB : 执行默认导入动作
TP->>EB : 执行默认导出动作
IB->>CS : 读取配置处理模式
EB->>WS : 检查连接状态
IB-->>FS : 解析并注入 Pipeline
EB-->>FS : 导出 Pipeline/配置
```

**图表来源**
- [ToolbarPanel.tsx:1-22](file://src/components/panels/main/ToolbarPanel.tsx#L1-L22)
- [ImportButton.tsx:1-276](file://src/components/panels/toolbar/ImportButton.tsx#L1-L276)
- [ExportButton.tsx:1-362](file://src/components/panels/toolbar/ExportButton.tsx#L1-L362)

**章节来源**
- [ToolbarPanel.tsx:1-22](file://src/components/panels/main/ToolbarPanel.tsx#L1-L22)
- [ImportButton.tsx:1-276](file://src/components/panels/toolbar/ImportButton.tsx#L1-L276)
- [ExportButton.tsx:1-362](file://src/components/panels/toolbar/ExportButton.tsx#L1-L362)

### 全局面板（GlobalPanel）- 新增
- 功能要点
  - 系统配置、文件配置、AI对话历史面板入口
  - 聚焦透明度、复制、粘贴、撤销、重做等编辑工具
  - **新增** 运行状态徽章：实时显示调试会话运行状态
  - 节点路径模式切换、工具箱弹窗、调试入口、文档站链接
- 数据流与状态同步
  - 从 Flow Store 读取剪贴板节点、选中节点、历史状态
  - 从调试会话状态 Store 获取运行状态徽章状态
  - 通过状态管理协调全局工具的启用/禁用状态
- 交互模式
  - 悬停提示、点击执行、右上角徽章显示
  - 徽章状态映射：running→processing、completed→success、failed→error、stopped→default

```mermaid
sequenceDiagram
participant U as "用户"
participant GP as "全局面板"
participant DSS as "调试会话状态"
participant DS as "调试协议客户端"
U->>GP : 点击调试按钮
GP->>DSS : openModal()
DSS-->>GP : 设置 runBadgeStatus
GP->>GP : 显示运行状态徽章
DS-->>DSS : handleRunStarted()
DSS-->>GP : 更新徽章状态
GP->>GP : 根据状态映射显示不同颜色
```

**图表来源**
- [GlobalPanel.tsx:318-340](file://src/components/panels/tools/GlobalPanel.tsx#L318-L340)
- [debugSessionStore.ts:101-104](file://src/stores/debugSessionStore.ts#L101-L104)
- [DebugProtocolClient.ts:308-311](file://src/services/protocols/DebugProtocolClient.ts#L308-L311)

**章节来源**
- [GlobalPanel.tsx:1-363](file://src/components/panels/tools/GlobalPanel.tsx#L1-L363)
- [debugSessionStore.ts:1-273](file://src/stores/debugSessionStore.ts#L1-L273)
- [DebugProtocolClient.ts:1-353](file://src/services/protocols/DebugProtocolClient.ts#L1-L353)

## 依赖分析
- 组件耦合
  - 连接面板强依赖 MFW Store 与协议桥接
  - 字段面板依赖 Flow Store 与面板占位互斥系统
  - 节点列表面板依赖 Flow Store 的节点/边数据
  - 文件面板依赖文件 Store 与配置 Store
  - 工具面板依赖工具栏 Store、配置 Store、文件 Store、WS Store
  - **新增** 全局面板依赖调试会话状态 Store 和调试协议客户端
- 外部依赖
  - DnD Kit：文件面板标签页拖拽排序
  - Ant Design：UI 组件与图标（含 Badge 组件用于运行状态徽章）
  - Zustand：轻量状态管理

```mermaid
graph TB
CP["连接面板"] --> MFW["MFW Store"]
FP["字段面板"] --> FS["Flow Store"]
NLP["节点列表面板"] --> FS
FIP["文件面板"] --> FILE["文件 Store"]
TP["工具栏"] --> TOOL["工具栏 Store"]
TP --> CFG["配置 Store"]
TP --> WS["WS Store"]
FIP --> CFG
FP --> POS["面板占位互斥"]
NLP --> POS
GP["全局面板"] --> DSS["调试会话状态 Store"]
GP --> DPC["调试协议客户端"]
DSS --> TYPES["调试类型定义"]
```

**图表来源**
- [mfwStore.ts:1-195](file://src/stores/mfwStore.ts#L1-L195)
- [index.ts（Flow Store）:1-124](file://src/stores/flow/index.ts#L1-L124)
- [panelOccupancyStore.ts:1-136](file://src/stores/panelOccupancyStore.ts#L1-L136)
- [ImportButton.tsx:1-276](file://src/components/panels/toolbar/ImportButton.tsx#L1-L276)
- [ExportButton.tsx:1-362](file://src/components/panels/toolbar/ExportButton.tsx#L1-L362)
- [debugSessionStore.ts:1-273](file://src/stores/debugSessionStore.ts#L1-L273)
- [DebugProtocolClient.ts:1-353](file://src/services/protocols/DebugProtocolClient.ts#L1-L353)
- [types.ts（调试类型）:1-476](file://src/features/debug/types.ts#L1-L476)

**章节来源**
- [mfwStore.ts:1-195](file://src/stores/mfwStore.ts#L1-L195)
- [index.ts（Flow Store）:1-124](file://src/stores/flow/index.ts#L1-L124)
- [panelOccupancyStore.ts:1-136](file://src/stores/panelOccupancyStore.ts#L1-L136)
- [ImportButton.tsx:1-276](file://src/components/panels/toolbar/ImportButton.tsx#L1-L276)
- [ExportButton.tsx:1-362](file://src/components/panels/toolbar/ExportButton.tsx#L1-L362)
- [debugSessionStore.ts:1-273](file://src/stores/debugSessionStore.ts#L1-L273)
- [DebugProtocolClient.ts:1-353](file://src/services/protocols/DebugProtocolClient.ts#L1-L353)
- [types.ts（调试类型）:1-476](file://src/features/debug/types.ts#L1-L476)

## 性能考虑
- 渲染优化
  - 使用 memo 包装面板组件，避免不必要的重渲染
  - 节点列表面板对过滤/分组结果进行 useMemo 缓存
  - 字段面板在编辑器渲染失败时使用错误边界减少崩溃影响
  - **新增** 全局面板使用 useMemo 缓存工具列表，避免重复渲染
- 交互优化
  - 文件面板标签页拖拽使用 DnD Kit 的排序策略，降低 DOM 操作成本
  - 连接面板按需刷新设备列表（首次打开或切换标签页）
  - **新增** 运行状态徽章使用受控组件，仅在状态变化时重新渲染
- 状态优化
  - 面板占位互斥系统避免右侧区域同时渲染多个主动面板
  - Flow Store 将视图、选择、历史、节点、边、图等切片组合，减少无关订阅
  - **新增** 调试会话状态 Store 使用原子状态更新，避免深层嵌套对象的性能问题

## 故障排查指南
- 连接面板
  - 现象：连接按钮不可用
  - 排查：确认已选择设备/方法，检查连接状态与错误信息
  - 参考：连接面板的状态判断与可用性检查
- 字段面板
  - 现象：编辑器渲染失败
  - 排查：使用错误边界提供的"尝试修复节点"功能
  - 参考：字段面板的错误边界与节点修复流程
- 节点列表面板
  - 现象：点击外部不关闭
  - 排查：确认未在 Ant Design 弹出层内点击；检查事件监听与锚点元素
- 文件面板
  - 现象：文件名校验失败
  - 排查：检查文件名合法性与状态提示
- **新增** 全局面板运行状态徽章
  - 现象：运行状态徽章不显示或显示异常
  - 排查：检查调试会话状态 Store 的 runBadgeStatus 值，确认调试协议客户端是否正确接收 run_started 事件
  - 参考：全局面板的状态映射逻辑与调试协议事件处理

**章节来源**
- [ConnectionPanel.tsx:1-954](file://src/components/panels/main/ConnectionPanel.tsx#L1-L954)
- [FieldPanel.tsx:1-491](file://src/components/panels/main/FieldPanel.tsx#L1-L491)
- [NodeListPanel.tsx:1-426](file://src/components/panels/main/node-list/NodeListPanel.tsx#L1-L426)
- [FilePanel.tsx:1-165](file://src/components/panels/main/FilePanel.tsx#L1-L165)
- [GlobalPanel.tsx:332-338](file://src/components/panels/tools/GlobalPanel.tsx#L332-L338)
- [debugSessionStore.ts:101-104](file://src/stores/debugSessionStore.ts#L101-L104)
- [DebugProtocolClient.ts:308-311](file://src/services/protocols/DebugProtocolClient.ts#L308-L311)

## 结论
主要面板系统通过清晰的职责划分与状态解耦，实现了设备连接、节点管理、字段编辑与文件操作的高效协作。面板占位互斥系统保证了右侧区域的稳定体验，而工具面板提供了便捷的导入/导出能力。**新增的全局面板运行状态徽章功能**进一步增强了调试体验，为用户提供实时的调试会话状态反馈。建议在扩展新面板时遵循现有模式：明确 Store 依赖、使用互斥系统、提供错误边界与性能优化。

## 附录
- 开发与扩展建议
  - 新增面板：在 panelOccupancyStore 中注册面板描述符，选择合适的反应形态（close/hide/offset）
  - 状态管理：优先使用现有 Store，避免重复造轮子；必要时拆分 slice 或新增 slice
  - 交互优化：对高频计算使用 useMemo/useCallback，对复杂列表使用虚拟化（如后续需求）
  - 可访问性：为按钮与表单控件提供 aria-label 与键盘导航支持
  - **新增** 调试状态徽章：遵循 runBadgeStatus 的状态枚举，使用相应的 Ant Design Badge 颜色方案
- 用户体验最佳实践
  - 保持操作反馈即时（加载遮罩、进度提示）
  - 提供撤销/重做与历史记录（现有 Flow Store 已具备历史切片）
  - 在面板切换时保留必要的上下文（如上次选中的节点）
  - **新增** 调试状态徽章应提供清晰的状态指示，避免与界面其他元素产生视觉冲突