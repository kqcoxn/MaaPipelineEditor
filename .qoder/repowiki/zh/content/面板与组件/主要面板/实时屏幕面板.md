...
## 更新摘要
**变更内容**
- 新增面板折叠功能，支持用户手动折叠/展开实时画面面板
- 增强性能优化机制，包括页面可见性感知、资源使用优化
- 改进面板占位互斥系统的折叠状态管理
- 优化截图请求的节流和并发控制策略
...
- **新增** 面板折叠功能：支持用户手动折叠/展开面板，优化空间利用率和性能。
- **新增** 性能优化与内存管理：节流、去抖、页面可见性感知、失败阈值与自动断连。
...
UAO["区域占用 Hook<br/>useAreaOccupancy.ts"]
end
subgraph "协议与服务"
...
## 核心组件
- 实时屏幕面板：负责截图请求、结果监听、状态管理与 UI 渲染，**新增**支持手动折叠/展开功能。
...
- 样式与交互：面板布局、可见性切换、**新增**折叠按钮、缩放/平移/标注工具与快捷键。
...
## 架构总览
实时屏幕面板通过 MFW 协议向本地服务发送截图请求，接收 Base64 图像数据并在面板中渲染。面板受连接状态、配置与面板占位互斥系统共同控制，具备失败阈值与自动断连保护。**新增**支持用户手动折叠面板以优化空间利用和性能。
...
UI-->>UI : 渲染图片/加载/错误提示
UI->>UI : 折叠状态检查(shouldRequestScreen = shouldShow && !isCollapsed)
UI->>UI : 条件执行截图请求(仅在展开状态下)
```
...
- 关键职责
  ...
  - **新增** 支持手动折叠/展开功能，折叠时停止截图请求。
  - 失败计数与阈值控制，超过阈值自动断开设备连接。
  - 与面板占位系统协作，确保可见性与互斥。
- 数据流与状态
  - 输入：连接状态、控制器 ID、配置开关与刷新率、**新增**折叠状态。
  - 输出：屏幕图像、加载状态、错误状态、**新增**折叠按钮状态。
  - 内部状态：请求中标志、连续失败计数、**新增**折叠状态标志。
- 性能与可靠性
  - 使用防抖与请求中标志避免并发请求。
  - 页面不可见时暂停请求，降低资源消耗。
  - **新增** 折叠状态下完全停止截图请求，释放 CPU 和内存资源。
  - 失败阈值与自动断连减少无效重试。
- 样式与交互
  - 采用绝对定位与可见性切换动画。
  - **新增** 折叠按钮支持展开/折叠两种状态切换。
  - 支持加载态、错误态与图片态的条件渲染。
...
CheckConn --> |可显示| Init["初始化状态(loading=false,error=false,isCollapsed=false)"]
Init --> Timer["启动定时器(周期=刷新率)"]
Timer --> CheckFold["检查折叠状态"]
CheckFold --> |已折叠| FoldWait["等待展开"]
CheckFold --> |已展开| Request["requestScreenshot()"]
...
LSP --> UPO["usePanelOccupancy.ts"]
LSP --> UAO["useAreaOccupancy.ts"]
...
- 渲染优化
  ...
  - **新增** 折叠状态下的最小高度优化，减少布局计算开销。
- 存储与缓存
  - 截图结果以 Base64 传递，避免额外中间层；建议在后端实现缓存与压缩策略（需后端配合）。

**章节来源**
- [LiveScreenPanel.tsx:35-43](file://src/components/panels/main/LiveScreenPanel.tsx#L35-L43)
- [LiveScreenPanel.tsx:84-93](file://src/components/panels/main/LiveScreenPanel.tsx#L84-L93)
- [LiveScreenPanel.module.less:30-32](file://src/styles/panels/LiveScreenPanel.module.less#L30-L32)
...
- 截图异常
  ...
  - 观察连续失败计数，超过阈值会自动断开。
- **新增** 面板折叠问题
  - 检查折叠状态是否被意外设置，确认 shouldRequestScreen 计算逻辑。
  - 确认折叠按钮是否正常响应，检查 aria-expanded 属性。
- 面板不可见
  - 检查面板占位互斥系统是否被其他面板抢占。
  - 确认配置开关与刷新率设置合理。
- 交互问题
...
## 结论
实时屏幕面板通过清晰的状态管理、可靠的协议封装与稳健的错误处理，实现了稳定的设备画面预览。**新增**的折叠功能进一步提升了用户体验和性能表现。相邻信息面板进一步增强了工作流上下文感知。结合截图与标注工具链，用户可在同一界面完成画面观测、ROI 选取与标注绘制。建议在后续迭代中引入后端缓存与压缩、更细粒度的失败恢复策略以及更丰富的自定义显示模式。
...
- 即时反应
  ...
  - **新增** 折叠/展开操作应有明确的视觉反馈和状态指示。
- 简化交互
  - 将高频操作置于内容中（如缩放/平移按钮），减少用户移动距离。
  - **新增** 折叠按钮应放置在易于触及的位置，提供清晰的展开/折叠语义。
- 意义感
  - 明确目标与路径，为每个操作提供清晰的结果预期。
  - **新增** 折叠状态应有明确的视觉提示，让用户了解面板当前状态。
- 生长性
  - 随用户能力提升逐步开放高级功能，避免一次性堆叠复杂工具。
  - **新增** 折叠功能应支持键盘快捷键操作，提升专业用户的使用效率。