# 版本检测系统

<cite>
**本文档引用的文件**
- [versionDetector.ts](file://src/core/parser/versionDetector.ts)
- [nodeParser.ts](file://src/core/parser/nodeParser.ts)
- [index.ts](file://src/core/fields/index.ts)
- [configStore.ts](file://src/stores/configStore.ts)
- [server.ts](file://src/services/server.ts)
- [Header.tsx](file://src/components/Header.tsx)
- [wailsBridge.ts](file://src/utils/wailsBridge.ts)
- [router.go](file://LocalBridge/internal/router/router.go)
- [main.go](file://LocalBridge/cmd/lb/main.go)
- [updater.go](file://Extremer/internal/updater/updater.go)
- [app.go](file://Extremer/app.go)
- [updateLogs.ts](file://src/data/updateLogs.ts)
- [main.go](file://Extremer/main.go)
- [wails.json](file://Extremer/wails.json)
- [package.json](file://package.json)
</cite>

## 更新摘要
**变更内容**
- 更新了版本检测系统的架构描述，特别关注开发构建版本检测的修复
- 新增了开发构建版本检测问题的专门章节
- 更新了版本检测流程图，反映最新的修复逻辑
- 增强了开发环境与生产环境的版本检测差异说明
- 修复了开发模式下版本检查机制的误报问题
- **版本降级处理**：从1.5.0降级到1.4.3，包括多个文件的版本同步更新
- **版本管理改进**：增强了版本检测的准确性，防止开发版本被错误标记为过时

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构概览](#架构概览)
5. [详细组件分析](#详细组件分析)
6. [开发构建版本检测修复](#开发构建版本检测修复)
7. [版本降级与同步更新](#版本降级与同步更新)
8. [依赖关系分析](#依赖关系分析)
9. [性能考虑](#性能考虑)
10. [故障排除指南](#故障排除指南)
11. [结论](#结论)

## 简介

版本检测系统是 MaaPipelineEditor 项目中的一个关键组件，负责检测和管理不同版本的 Pipeline 节点数据格式。该系统主要处理两种版本：Pipeline 节点的识别算法版本（recognition）和动作版本（action），以及前端和后端之间的协议版本兼容性。

**最新修复**：系统现已修复开发构建版本检测问题，防止开发版本被错误标记为过时，确保开发者能够正常工作而不受误导性更新通知影响。修复的核心在于 `src/components/Header.tsx` 中的版本检查机制，通过 `globalConfig.dev` 标志来区分开发模式和生产模式，避免在开发环境下显示过时版本警告。

**版本降级处理**：系统已成功从1.5.0版本降级到1.4.3版本，这一过程涉及多个文件的版本同步更新，包括 Extremer 主程序、Wails 配置、前端包配置以及更新日志等。版本降级确保了系统的稳定性，同时保留了1.4.x系列的功能特性。

系统的核心功能包括：
- 自动检测 Pipeline 节点的版本信息
- 标准化识别算法和动作类型
- 管理前端与后端的协议版本兼容性
- 支持版本迁移和向后兼容
- 区分开发构建与生产构建的版本检测逻辑
- **增强的版本降级处理机制**

## 项目结构

版本检测系统分布在项目的多个层次中：

```mermaid
graph TB
subgraph "前端层"
A[versionDetector.ts<br/>版本检测核心]
B[nodeParser.ts<br/>节点解析器]
C[configStore.ts<br/>配置存储]
D[wailsBridge.ts<br/>Wails桥接]
E[Header.tsx<br/>版本检测UI]
end
subgraph "协议层"
F[server.ts<br/>WebSocket服务器]
G[router.go<br/>路由处理器]
end
subgraph "后端层"
H[main.go<br/>LocalBridge主程序]
I[updater.go<br/>更新检查]
end
subgraph "扩展层"
J[app.go<br/>Extremer应用]
K[updateLogs.ts<br/>更新日志]
L[main.go<br/>Extremer主程序]
M[wails.json<br/>Wails配置]
N[package.json<br/>前端包配置]
end
A --> B
B --> C
C --> E
E --> D
D --> F
F --> G
G --> H
H --> I
J --> K
J --> L
L --> M
N --> L
```

**图表来源**
- [versionDetector.ts:1-149](file://src/core/parser/versionDetector.ts#L1-L149)
- [server.ts:1-373](file://src/services/server.ts#L1-L373)
- [router.go:1-150](file://LocalBridge/internal/router/router.go#L1-L150)
- [Header.tsx:292-312](file://src/components/Header.tsx#L292-L312)
- [main.go:24](file://Extremer/main.go#L24)
- [wails.json:13](file://Extremer/wails.json#L13)

**章节来源**
- [versionDetector.ts:1-149](file://src/core/parser/versionDetector.ts#L1-L149)
- [nodeParser.ts:1-468](file://src/core/parser/nodeParser.ts#L1-L468)
- [configStore.ts:1-276](file://src/stores/configStore.ts#L1-L276)

## 核心组件

### 版本检测器 (Version Detector)

版本检测器是系统的核心组件，负责检测和标准化 Pipeline 节点的版本信息。

```mermaid
classDiagram
class NodeVersionInfo {
+number recognitionVersion
+number actionVersion
}
class VersionDetector {
+detectNodeVersion(node) NodeVersionInfo
+detectRecognitionVersion(node) number
+detectActionVersion(node) number
+normalizeRecoType(value) string
+normalizeActionType(value) string
}
class FieldDetector {
+recoFieldSchemaKeyList array
+actionFieldSchemaKeyList array
+upperRecoValues object
+upperActionValues object
}
VersionDetector --> NodeVersionInfo : "返回"
VersionDetector --> FieldDetector : "使用"
```

**图表来源**
- [versionDetector.ts:11-149](file://src/core/parser/versionDetector.ts#L11-L149)
- [index.ts:41-45](file://src/core/fields/index.ts#L41-L45)

### 节点解析器 (Node Parser)

节点解析器负责将不同版本的节点数据转换为统一的内部格式。

```mermaid
flowchart TD
A[输入节点数据] --> B{检测节点版本}
B --> |v1版本| C[解析为字符串格式]
B --> |v2版本| D[解析为对象格式]
C --> E[标准化类型]
D --> E
E --> F[迁移兼容字段]
F --> G[输出统一格式]
H[识别字段解析] --> E
I[动作字段解析] --> E
J[其他字段解析] --> E
```

**图表来源**
- [nodeParser.ts:276-320](file://src/core/parser/nodeParser.ts#L276-L320)
- [nodeParser.ts:331-380](file://src/core/parser/nodeParser.ts#L331-L380)

**章节来源**
- [versionDetector.ts:23-110](file://src/core/parser/versionDetector.ts#L23-L110)
- [nodeParser.ts:276-380](file://src/core/parser/nodeParser.ts#L276-L380)

## 架构概览

版本检测系统采用分层架构设计，确保版本兼容性和数据完整性：

```mermaid
sequenceDiagram
participant Frontend as 前端应用
participant Parser as 节点解析器
participant Detector as 版本检测器
participant Protocol as 协议处理器
participant Backend as 后端服务
Frontend->>Parser : 导入节点数据
Parser->>Detector : 检测节点版本
Detector-->>Parser : 返回版本信息
Parser->>Parser : 标准化字段类型
Parser->>Protocol : 发送标准化数据
Protocol->>Backend : 转发处理请求
Backend-->>Frontend : 返回处理结果
```

**图表来源**
- [nodeParser.ts:421-423](file://src/core/parser/nodeParser.ts#L421-L423)
- [server.ts:268-283](file://src/services/server.ts#L268-L283)
- [router.go:107-133](file://LocalBridge/internal/router/router.go#L107-L133)

系统的关键特性包括：

1. **自动版本检测**：根据节点结构自动判断 v1 或 v2 版本
2. **类型标准化**：统一识别算法和动作类型的大小写
3. **协议兼容性**：管理前端和后端的协议版本匹配
4. **向后兼容**：支持旧版本数据格式的迁移
5. **开发构建隔离**：区分开发构建与生产构建的版本检测逻辑
6. **版本降级处理**：支持从高版本到低版本的平滑降级

## 详细组件分析

### 版本检测算法

版本检测算法基于节点的结构特征进行判断：

```mermaid
flowchart TD
A[开始检测] --> B{节点是否存在}
B --> |否| C[返回默认v2版本]
B --> |是| D[检查recognition字段]
D --> E{recognition是对象?}
E --> |是| F{包含type字段?}
F --> |是| G[v2版本]
F --> |否| H{包含v1特征?}
E --> |否| I{recognition是字符串?}
I --> |是| J[v1版本]
I --> |否| H
H --> |是| K[v1版本]
H --> |否| L[v2版本]
```

**图表来源**
- [versionDetector.ts:39-71](file://src/core/parser/versionDetector.ts#L39-L71)
- [versionDetector.ts:78-110](file://src/core/parser/versionDetector.ts#L78-L110)

### 类型标准化机制

系统实现了完整的类型标准化机制，确保识别算法和动作类型的统一：

```mermaid
classDiagram
class TypeNormalizer {
+normalizeRecoType(value) string
+normalizeActionType(value) string
-validateType(value) boolean
-findMatchingType(value) string
}
class RecoTypeValidator {
+upperRecoValues object
+validate(value) boolean
+getError() string
}
class ActionTypeValidator {
+upperActionValues object
+validate(value) boolean
+getError() string
}
TypeNormalizer --> RecoTypeValidator : "使用"
TypeNormalizer --> ActionTypeValidator : "使用"
```

**图表来源**
- [versionDetector.ts:118-148](file://src/core/parser/versionDetector.ts#L118-L148)
- [index.ts:43-44](file://src/core/fields/index.ts#L43-L44)

### 协议版本管理

前端和后端通过握手机制确保协议版本兼容性：

```mermaid
sequenceDiagram
participant Client as 前端客户端
participant Server as 后端服务器
participant Router as 路由处理器
Client->>Server : 建立WebSocket连接
Server->>Client : 发送握手请求
Client->>Router : 发送协议版本
Router->>Router : 验证版本兼容性
alt 版本匹配
Router-->>Client : 返回成功响应
else 版本不匹配
Router-->>Client : 返回错误响应
Router-->>Client : 建议更新方式
end
```

**图表来源**
- [server.ts:162-164](file://src/services/server.ts#L162-L164)
- [router.go:107-133](file://LocalBridge/internal/router/router.go#L107-L133)

**章节来源**
- [versionDetector.ts:118-148](file://src/core/parser/versionDetector.ts#L118-L148)
- [server.ts:18-65](file://src/services/server.ts#L18-L65)
- [router.go:107-133](file://LocalBridge/internal/router/router.go#L107-L133)

## 开发构建版本检测修复

**重要更新**：系统已修复开发构建版本检测问题，防止开发版本被错误标记为过时。

### 问题背景

在开发环境中，版本检测系统存在以下问题：
- 开发构建版本被错误地标记为过时
- 开发者收到误导性的更新通知
- 影响开发体验和工作效率

### 修复机制

系统通过以下机制区分开发构建与生产构建：

```mermaid
flowchart TD
A[版本检测开始] --> B{检查全局配置}
B --> |dev=true| C[开发构建模式]
B --> |dev=false| D[生产构建模式]
C --> E{检查开发版本检测逻辑}
E --> |启用| F[跳过更新检查]
E --> |禁用| G[执行标准版本检查]
D --> H[执行标准版本检查]
F --> I[显示开发版本标签]
G --> J[显示稳定版本标签]
H --> J
I --> K[防止误报更新通知]
J --> K
```

**图表来源**
- [Header.tsx:304-312](file://src/components/Header.tsx#L304-L312)
- [configStore.ts:7-16](file://src/stores/configStore.ts#L7-L16)

### 开发构建检测逻辑

开发构建版本检测通过以下步骤实现：

1. **开发模式检测**：检查 `globalConfig.dev` 标志
2. **版本标识**：开发版本显示 "Preview Version" 标签
3. **更新检查绕过**：开发模式下跳过在线更新检查
4. **版本标签显示**：显示绿色 "Stable Version" 或粉色 "Preview Version" 标签

### 版本标签系统

系统提供清晰的版本标识：

```mermaid
graph LR
subgraph "版本标签"
A[开发版本] --> B[粉色标签<br/>Preview Version]
C[生产版本] --> D[绿色标签<br/>Stable Version]
E[MFW版本] --> F[紫色标签<br/>MFW v{version}]
end
```

**图表来源**
- [Header.tsx:344-352](file://src/components/Header.tsx#L344-L352)

**章节来源**
- [Header.tsx:292-312](file://src/components/Header.tsx#L292-L312)
- [configStore.ts:7-16](file://src/stores/configStore.ts#L7-L16)

## 版本降级与同步更新

**重要变更**：系统已从1.5.0版本降级到1.4.3版本，这一过程涉及多个文件的版本同步更新。

### 版本降级背景

系统经历了从1.5.0到1.4.3的版本降级，主要出于以下考虑：
- **稳定性优先**：1.4.x系列版本经过充分测试，稳定性更好
- **向后兼容性**：确保与现有工作流和配置的兼容
- **用户体验**：避免新版本引入的潜在问题影响用户使用

### 版本同步更新机制

版本降级过程中，系统通过以下机制确保版本的一致性：

```mermaid
flowchart TD
A[版本降级开始] --> B{检查所有相关文件}
B --> C[Extremer/main.go]
B --> D[Extremer/wails.json]
B --> E[package.json]
B --> F[src/data/updateLogs.ts]
B --> G[src/stores/configStore.ts]
C --> H[更新版本号为1.4.3]
D --> H
E --> H
F --> H
G --> H
H --> I[验证版本一致性]
I --> J[测试版本降级功能]
J --> K[发布稳定版本]
```

**图表来源**
- [main.go:24](file://Extremer/main.go#L24)
- [wails.json:13](file://Extremer/wails.json#L13)
- [package.json:4](file://package.json#L4)
- [updateLogs.ts:51](file://src/data/updateLogs.ts#L51)
- [configStore.ts:8](file://src/stores/configStore.ts#L8)

### 版本降级影响分析

版本降级对系统的影响包括：

1. **功能层面**：移除1.5.0引入但不稳定的功能
2. **性能层面**：优化了1.4.x系列的性能表现
3. **兼容性层面**：确保与1.4.x系列的完全兼容
4. **用户体验**：提供更稳定可靠的使用体验

### 版本降级验证

系统通过以下方式进行版本降级验证：

1. **版本一致性检查**：确保所有文件中的版本号同步
2. **功能回归测试**：验证1.4.x系列功能的完整性
3. **兼容性测试**：确保与现有配置和工作流的兼容
4. **性能基准测试**：评估降级后的性能表现

**章节来源**
- [main.go:24](file://Extremer/main.go#L24)
- [wails.json:13](file://Extremer/wails.json#L13)
- [package.json:4](file://package.json#L4)
- [updateLogs.ts:51](file://src/data/updateLogs.ts#L51)
- [configStore.ts:8](file://src/stores/configStore.ts#L8)

## 依赖关系分析

版本检测系统与其他组件的依赖关系如下：

```mermaid
graph TD
subgraph "版本检测核心"
A[versionDetector.ts]
B[nodeParser.ts]
C[index.ts]
end
subgraph "配置管理"
D[configStore.ts]
E[server.ts]
F[Header.tsx]
end
subgraph "后端服务"
G[main.go]
H[router.go]
I[updater.go]
end
subgraph "扩展功能"
J[app.go]
K[wailsBridge.ts]
L[updateLogs.ts]
M[main.go]
N[wails.json]
O[package.json]
end
A --> C
B --> A
B --> D
E --> G
F --> D
G --> H
I --> H
J --> K
K --> L
M --> N
O --> M
```

**图表来源**
- [versionDetector.ts:1-6](file://src/core/parser/versionDetector.ts#L1-L6)
- [nodeParser.ts:1-24](file://src/core/parser/nodeParser.ts#L1-L24)
- [configStore.ts:1-11](file://src/stores/configStore.ts#L1-L11)

系统的主要依赖关系特点：

1. **低耦合设计**：版本检测器独立于其他组件
2. **向上依赖**：解析器依赖检测器，配置依赖解析器
3. **双向通信**：前端和后端通过协议处理器通信
4. **扩展性**：支持新的版本和类型扩展
5. **开发构建隔离**：开发构建与生产构建逻辑分离
6. **版本同步机制**：确保多文件版本的一致性

**章节来源**
- [nodeParser.ts:1-24](file://src/core/parser/nodeParser.ts#L1-L24)
- [configStore.ts:1-11](file://src/stores/configStore.ts#L1-L11)
- [router.go:13-17](file://LocalBridge/internal/router/router.go#L13-L17)

## 性能考虑

版本检测系统在设计时充分考虑了性能优化：

### 时间复杂度优化
- 版本检测：O(n) 时间复杂度，其中 n 是节点字段数量
- 类型标准化：O(1) 平均时间复杂度
- 协议版本检查：O(1) 时间复杂度
- 开发构建检测：O(1) 时间复杂度
- **版本降级检查**：O(m) 时间复杂度，其中 m 是检查的文件数量

### 内存使用优化
- 使用对象池避免频繁内存分配
- 缓存标准化结果减少重复计算
- 按需加载字段定义
- 开发构建版本检测无额外内存开销
- **版本降级缓存机制**：避免重复的版本检查

### 并发处理
- 支持多线程并发处理不同节点
- 使用读写锁保护共享资源
- 异步处理协议版本检查
- 开发构建版本检测异步执行
- **版本同步并发处理**：多文件版本检查的并发优化

## 故障排除指南

### 常见问题及解决方案

#### 版本检测失败
**症状**：节点版本检测返回默认值
**原因**：节点数据格式不正确
**解决**：
1. 检查节点数据结构完整性
2. 验证字段名称和类型
3. 确认数据格式符合预期

#### 类型标准化错误
**症状**：抛出"类型错误"异常
**原因**：识别算法或动作类型不在预定义列表中
**解决**：
1. 检查类型名称的大小写
2. 验证类型是否在支持列表中
3. 使用标准化函数进行转换

#### 协议版本不匹配
**症状**：连接被拒绝或出现版本错误
**原因**：前端和后端协议版本不兼容
**解决**：
1. 检查前端协议版本配置
2. 更新后端服务到兼容版本
3. 遵循版本升级指导

#### 开发构建版本误报
**症状**：开发版本显示过时警告
**原因**：开发构建版本检测逻辑错误
**解决**：
1. 检查 `globalConfig.dev` 配置
2. 确认开发构建版本检测逻辑
3. 验证版本标签显示逻辑

#### 版本降级问题
**症状**：版本降级后功能异常
**原因**：版本同步不完整或配置冲突
**解决**：
1. 检查所有相关文件的版本号
2. 验证版本降级的完整性
3. 重新执行版本同步更新

#### 版本检测性能问题
**症状**：版本检测速度缓慢
**原因**：大量节点或复杂数据结构
**解决**：
1. 优化节点数据结构
2. 实施版本检测缓存
3. 分批处理大型数据集

**章节来源**
- [versionDetector.ts:118-148](file://src/core/parser/versionDetector.ts#L118-L148)
- [server.ts:50-63](file://src/services/server.ts#L50-L63)
- [router.go:120-128](file://LocalBridge/internal/router/router.go#L120-L128)
- [Header.tsx:304-312](file://src/components/Header.tsx#L304-L312)

## 结论

版本检测系统通过精心设计的架构和算法，成功解决了 Pipeline 节点版本兼容性的复杂问题。系统的主要优势包括：

1. **自动化程度高**：能够自动检测和处理不同版本的数据
2. **向后兼容性强**：支持从旧版本到新版本的平滑迁移
3. **扩展性良好**：易于添加新的版本和类型支持
4. **性能优化到位**：在保证功能的同时注重性能表现
5. **开发体验优化**：通过修复开发构建版本检测问题，显著改善了开发者的使用体验
6. **版本管理能力**：具备从高版本到低版本的平滑降级能力

**最新改进**：系统现已修复开发构建版本检测问题，防止开发版本被错误标记为过时，确保开发者能够正常工作而不受误导性更新通知影响。同时，系统成功完成了从1.5.0到1.4.3的版本降级，通过多文件版本同步更新确保了系统的稳定性和兼容性。

**版本降级处理**：系统通过严格的版本同步机制，确保了从1.5.0到1.4.3的平滑降级。这一过程涉及Extremer主程序、Wails配置、前端包配置以及更新日志等多个文件的版本同步，保证了系统的完整性和一致性。

该系统为 MaaPipelineEditor 提供了稳定可靠的数据处理基础，确保用户能够在不同版本间无缝切换，同时保持数据的完整性和一致性。随着项目的不断发展，版本检测系统将继续演进，为用户提供更好的版本管理体验。