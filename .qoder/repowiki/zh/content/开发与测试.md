...
- [LocalBridge/.air.toml](file://LocalBridge/.air.toml)
...
- [LocalBridge/test-json/interface.json](file://LocalBridge/test-json/interface.json)
...
**更新** 新增版本管理与迁移系统的完整文档，包括自动化的版本号同步机制。**新增** LocalBridge 的 Air 热重载功能，显著提升 Go 服务的开发体验。

## 项目结构
MPE 采用前后端分离架构：
- 前端工程位于根目录，基于 Vite + React 19 + TypeScript，使用 Vitest 进行单元测试，ESLint 规范代码风格。
- 本地桥接服务 LocalBridge 为 Go 语言实现，提供文件扫描、资源管理、WebSocket 通信等能力，现已集成 Air 热重载功能。
...
- **新增** LocalBridge 集成了 Air 热重载工具，通过 .air.toml 配置实现自动编译和重启。
...
LB["Go 服务<br/>文件/资源/WS 通信<br/>Air 热重载"]
end
...
- [LocalBridge/.air.toml:1-35](file://LocalBridge/.air.toml#L1-L35)
...
- 本地桥接服务
  - Go 服务，提供文件/资源/事件总线/日志等能力，使用 WebSocket 与前端通信，现已集成 Air 热重载功能。
- 桌面打包层
...
- **新增** Air 热重载系统
  - LocalBridge 通过 .air.toml 配置实现自动编译和重启，支持 Go 源码热重载。
  - 配置包括构建命令、二进制文件路径、排除目录、颜色主题等。
- 文档与站点
...
- [LocalBridge/.air.toml:7-21](file://LocalBridge/.air.toml#L7-L21)
...
- 本地桥接服务
  ...
  - **新增** Air 热重载配置：通过 .air.toml 文件定义构建命令、二进制文件路径、排除目录等。
  - 支持自动编译和重启，提升开发效率。
- 桌面打包层
...
- 本地桥接服务
  - **新增** Air 热重载：使用 `yarn server` 命令启动 Air 热重载，自动监控文件变化并重新编译。
  - 传统方式：通过脚本启动 LocalBridge 服务，便于本地联调。
- **新增** Air 热重载配置
  - 通过 `.air.toml` 文件配置构建命令、二进制文件路径、排除目录等。
  - 支持彩色输出、日志记录、自动清理等功能。
- **新增** 版本管理命令
...
- 调试要点
  ...
  - Air 热重载：配置文件变化时自动触发重新编译，支持 DEBUG 级别日志输出。
  - 版本管理：脚本提供详细的变更预览，支持确认机制，避免意外修改。

**章节来源**
- [package.json:6-23](file://package.json#L6-L23)
- [vite.config.ts:16-19](file://vite.config.ts#L16-L19)
- [LocalBridge/package.json:5-6](file://LocalBridge/package.json#L5-L6)
- [LocalBridge/.air.toml:7-21](file://LocalBridge/.air.toml#L7-L21)
...
- 本地桥接性能
  ...
  - **新增** Air 热重载性能：自动编译和重启过程中的性能影响评估。
- 建议的基准指标
  - 页面首屏时间、首次有效绘制、资源包大小、测试覆盖率阈值、端到端用例执行时长、热重载响应时间。
...
- 质量保障
  ...
  - **新增** Air 热重载质量控制：确保热重载配置的正确性和稳定性。
- **新增** 版本管理质量控制
  - 版本升级前强制预览所有变更，确保准确性。
  - 支持批量版本号同步，避免版本不一致问题。
  - 提供详细的变更日志和回滚机制。
...
- **新增** Air 热重载依赖
  - Air 热重载工具（通过 .air.toml 配置）

```mermaid
graph LR
FE["前端依赖"] --> |WS/HTTP| LB["LocalBridge(Go)<br/>Air 热重载"]
...
- **新增** Air 热重载性能
  - 文件监控延迟：500ms 延迟避免频繁触发，平衡响应速度与性能。
  - 排除目录：自动排除 build、test-json、node_modules 等目录，减少监控开销。
  - 颜色输出：彩色日志提升开发体验，不影响性能。
  - 自动清理：退出时自动清理临时文件，保持环境整洁。
...
- [LocalBridge/.air.toml:17-21](file://LocalBridge/.air.toml#L17-L21)
- [LocalBridge/.air.toml:12-13](file://LocalBridge/.air.toml#L12-13)
...
- 本地桥接服务
  ...
  - **新增** Air 热重载问题：
    - 热重载不生效：检查 .air.toml 配置是否正确，确认构建命令可执行。
    - 编译失败：查看 build-errors.log 文件，检查 Go 代码语法和依赖。
    - 性能问题：调整 delay 参数，适当增加监控间隔。
- 端到端测试
...
- [LocalBridge/.air.toml:19](file://LocalBridge/.air.toml#L19)

## 结论
本指南基于仓库现有配置与脚本，给出了 MPE 的开发与测试实践路线：以 Vite/React/TypeScript 为核心，结合 Vitest 与 Playwright 构建完善的测试体系；通过 ESLint 与 TS 严格模式保障代码质量；借助 LocalBridge 与 Extremer 实现本地能力与桌面打包。**新增的版本管理与迁移系统**进一步提升了项目的自动化水平和质量保证能力，通过精确的版本号同步机制确保所有版本信息的一致性。**新增的 Air 热重载功能**显著改善了 LocalBridge 的开发体验，通过自动编译和重启机制提升开发效率。建议在 CI 中补齐流水线配置，完善覆盖率阈值与端到端用例，持续提升交付质量与稳定性。
...
- 快速命令清单
  ...
  - 启动文档站点：yarn doc
  - **新增** 启动本地桥接服务：yarn server
  - **新增** 启动 Air 热重载：yarn server
...
### 系统概述
LocalBridge 新增的 Air 热重载系统通过 .air.toml 配置文件实现了 Go 服务的自动编译和重启功能，显著提升了开发体验。Air 是一个 Go 语言的热重载工具，能够监控文件变化并自动触发重新编译。

### 配置文件分析
- **根目录配置**：`root = "."` 指定监控根目录为当前目录
- **临时目录**：`tmp_dir = "build"` 指定构建输出目录
- **构建命令**：`cmd = "go mod tidy ; go build -o ./build/mpelb.exe ./cmd/lb"` 自动执行依赖整理和构建
- **二进制文件**：`bin = "./build/mpelb.exe"` 指定生成的可执行文件路径
- **完整二进制**：`full_bin = "./build/mpelb.exe --root ./test-json/resource --log-level DEBUG"` 启动时的完整命令行参数
- **文件监控**：`include_ext = ["go", "toml", "yaml", "yml", "json"]` 监控的文件扩展名
- **排除目录**：`exclude_dir = ["build", "test-json", "node_modules", "vendor", ".git"]` 排除监控的目录
- **延迟设置**：`delay = 500` 500ms 延迟避免频繁触发
- **颜色主题**：支持主色、观察者、构建、运行者的彩色输出
...
- **自动重启**：编译成功后自动重启服务
- **彩色输出**：清晰的日志输出，便于调试
- **性能优化**：合理的延迟设置和目录排除，避免性能问题
- **开发友好**：支持 DEBUG 级别日志输出，便于问题排查

**章节来源**
- [LocalBridge/.air.toml:1-35](file://LocalBridge/.air.toml#L1-L35)
- [LocalBridge/package.json:5](file://LocalBridge/package.json#L5)
- [LocalBridge/cmd/lb/main.go:136-160](file://LocalBridge/cmd/lb/main.go#L136-L160)