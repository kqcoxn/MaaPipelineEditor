...
## 更新摘要
**变更内容**
- 新增了SaveFileWithOrder函数，支持字段顺序保持的文件保存功能
- 更新了文件读写操作的安全性检查、JSONC解析、缩进格式化等功能
- 增强了字段顺序保持机制，确保JSON字段的原始顺序在保存时得到保留
- 更新了协议处理器对字段顺序保持功能的支持
...
## 简介
本文件服务模块提供对本地文件系统的递归扫描、实时监听、索引管理、并发安全、错误处理与日志记录能力，并支持安全的文件读写、JSONC 解析与缩进格式化。模块采用事件总线进行发布订阅，结合防抖机制降低高频变更带来的影响，同时通过路径安全校验与限制策略保障系统稳定性。**新增的字段顺序保持功能**确保在保存文件时能够维持JSON字段的原始顺序，这对于需要保持特定显示顺序的配置文件尤为重要。
...
**图表来源**
- [file_service.go:1-392](file://LocalBridge/internal/service/file/file_service.go#L1-L392)
...
- **字段顺序保持机制**：新增功能，确保JSON字段顺序在保存时得到保持。

**章节来源**
- [file_service.go:19-392](file://LocalBridge/internal/service/file/file_service.go#L19-L392)
...
- [file_handler.go:14-358](file://LocalBridge/internal/protocol/file/file_handler.go#L14-L358)

## 架构总览
文件服务的整体架构围绕"扫描—索引—监听—事件—协议"的闭环展开。启动时先扫描并构建索引，随后启动监听器捕获文件系统变化；所有变更通过事件总线广播，协议层将变更推送给前端；读写操作均进行路径安全校验与 JSONC 解析，保证一致性与安全性。**新增的字段顺序保持功能**在保存文件时确保JSON字段的原始顺序得到维护。
...
B --> J["字段顺序保持机制<br/>SaveFileWithOrder"]
C --> K["文件模型 File/FileNode<br/>索引与节点解析"]
D --> E
E --> F
```
...
- [file_handler.go:22-358](file://LocalBridge/internal/protocol/file/file_handler.go#L22-L358)
...
- 读写操作
  ...
  - **SaveFileWithOrder**：**新增**字段顺序保持的文件保存功能，支持保持JSON字段原始顺序。
  - **SaveFile**：现有文件保存功能，调用SaveFileWithOrder但不保持字段顺序。
...
- [file_service.go:253-392](file://LocalBridge/internal/service/file/file_service.go#L253-L392)
...
### 字段顺序保持机制
**新增功能**：SaveFileWithOrder 函数提供了字段顺序保持的文件保存能力，确保JSON字段的原始顺序在保存时得到维护。

- 核心功能
  - **keepOrder 参数**：控制是否保持字段顺序，默认为 false（保持旧版行为）。
  - **字符串内容处理**：当 content 为字符串且 keepOrder 为 true 时，直接使用原始字符串，不进行重新序列化。
  - **对象内容处理**：当 content 为对象时，始终通过 marshalJSON 进行序列化，确保缩进格式正确。
- 实现细节
  - **字符串直写**：`if jsonStr, ok := content.(string); ok` 检查内容类型，如果是字符串则直接使用。
  - **对象序列化**：否则调用 `marshalJSON` 进行标准的 JSON 序列化。
  - **兼容性**：SaveFile 函数保持原有行为，调用 SaveFileWithOrder 但 keepOrder 设为 false。
- 前端集成
  - 前端通过协议处理器传递 JSON 字符串来启用字段顺序保持功能。
  - 支持分离保存模式，分别处理 Pipeline 和配置文件的字段顺序。

```mermaid
flowchart TD
Start(["SaveFileWithOrder 调用"]) --> Validate["验证路径安全性"]
Validate --> CheckOrder{"keepOrder 为真?"}
CheckOrder --> |是| TypeCheck{"content 是否为字符串?"}
TypeCheck --> |是| DirectWrite["直接使用字符串内容"]
TypeCheck --> |否| Marshal["调用 marshalJSON 序列化"]
CheckOrder --> |否| Marshal
Marshal --> Write["写入文件"]
DirectWrite --> Write
Write --> Debounce["清除防抖事件"]
Debounce --> Log["记录日志"]
Log --> End(["完成"])
```
...
- 保存文件
  - 协议路径：/etl/save_file
  - 流程：解析请求 → 保存文件 → 返回确认。
  - **新增**：支持字段顺序保持的保存，通过传递 JSON 字符串启用。
- 分离保存
...
- **字段顺序保持性能**
  - 字符串直写模式避免了重新序列化的开销，适合大量数据的快速保存。
  - 对于需要严格缩进格式的场景，marshalJSON 提供了稳定的序列化性能。
...
- 处理建议
  ...
  - **字段顺序问题**：检查前端传递的 JSON 字符串格式，确保字段顺序符合预期。
...
## 结论
文件服务模块通过清晰的职责划分与完善的基础设施，实现了对本地文件系统的高效管理。扫描器提供灵活的过滤与限制策略，监听器配合防抖机制降低噪音，事件总线与协议层确保了前端的实时反馈。统一的错误与日志体系提升了可观测性与可维护性。**新增的字段顺序保持功能**进一步增强了文件保存的灵活性，使得开发者可以根据需要选择是否保持JSON字段的原始顺序。建议在生产环境中合理配置扫描限制与日志级别，并定期审查根目录与排除列表以保障性能与安全。
