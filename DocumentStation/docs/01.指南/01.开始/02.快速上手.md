...
开始前，请根据 MFW 文档的 [开发思路](https://maafw.xyz/docs/1.1-QuickStarted.html#%E5%BC%80%E5%8F%91%E6%80%9D%E8%B7%AF) 或 [教程视频](https://www.bilibili.com/video/BV1yr421E7MW) 准备好开发环境，然后选择一种方式打开 Editor：

- **纯 Web 端**：直接访问 [在线编辑器](https://mpe.codax.site/stable/)，无需安装，适合临时查看、审阅或编辑 Pipeline。在线站点仅提供静态服务，编辑内容保存在本地；除浏览量统计外，开发组不会获取您的编辑内容。也可以 [自行部署](https://github.com/kqcoxn/MaaPipelineEditor/releases)。
- **MPE Desktop**：在 Windows x64 或 macOS Apple Silicon 上安装桌面应用，通过启动器准备 Editor 与 LocalBridge 环境、添加项目目录并打开编辑器，适合日常本地项目开发。详见 [MPE Desktop 桌面应用](/guide/start/desktop)。
- **Web + LocalBridge**：按 [本地服务指南](/guide/server/deploy) 安装并启动 LocalBridge，再在浏览器中使用 Editor，即可接入本地文件、设备和 MaaFramework 能力。启动时命令行会显示 Editor 地址。
- **[MSE + iframe](/guide/mse/overview)**：在 VS Code 中使用 Maa Support Extension 打开受支持的 Pipeline，由 MSE 承载 Editor，并将修改保存回当前文件。

如果您习惯在 VS Code、Cursor 等 IDE 中使用网页版，也可以通过命令面板的 `Simple Browser` 打开 [在线编辑器](https://mpe.codax.site/stable/)；这与使用 MSE 集成是两种不同的入口。

下文以 Editor 的基础编辑操作为主，本地文件管理、设备连接与调试功能可在熟悉编辑流程后查阅 [本地服务](/guide/server/deploy)。
...
在此过程中，您可以**直接拖拽节点调整其位置**，按住 `shift` 框选多个节点，**使用 `Ctrl+C/V`（macOS 使用 `Cmd+C/V`）快速复制粘贴节点**，**使用右下角排版工具对多个节点进行统一操作**，使用 `Delete` 或 `Backspace` 键删除节点。
