...
失败时 `success` 为 `false`，并通过 `error` 或 `message` 返回可展示原因。MPE 只在成功结果到达后更新 clean 基线；若保存期间又发生编辑，较新的改动仍保持未保存状态。
...
::: tip
顶部导航栏的本地服务连接按钮在嵌入模式下会始终显示为 **"EmbedBridge"**。点击后会提示用户使用宿主提供的对应能力，或打开完整 MPE 并连接 LocalBridge。
:::
...
`host` 用于显示宿主名称和仓库入口。`id` 为 `mse` 时，MPE 的 Star 提示会同时展示 MPE 与 MSE 仓库；两个项目的提醒状态分别记录。

## 嵌入态界面行为

- 保留 Header、现有导入按钮及其菜单，并额外显示“从宿主同步”。
- 常规导出菜单替换为单一“保存到宿主”，持久化目标始终由宿主决定。
- 跳过使用协议确认、答题/新手引导和更新日志自动弹窗，握手和首次 Pipeline 加载完成后直接可用。
- 小工具、调试和 LocalBridge 入口保留；触发时提示使用宿主能力，或前往完整 MPE 配合 LocalBridge。
- 手动导入只改变当前画布，不改变宿主绑定的写回目标。
...
握手时 MPE 在 `mpe:ready` 中声明自身支持的协议版本（当前为 `1.1.0`）。版本策略遵循语义化版本：
