...
- 保留 Header；MSE 宿主下隐藏手动导入和 JSON 预览，显示“保存到 MSE”与“从 MSE 同步”。
...
- 非 MSE 宿主若保留手动导入，导入只改变当前画布，不改变宿主绑定的写回目标。
...
握手时 MPE 在 `mpe:ready` 中声明自身支持的协议版本（当前为 `1.2.0`）。版本策略遵循语义化版本：
