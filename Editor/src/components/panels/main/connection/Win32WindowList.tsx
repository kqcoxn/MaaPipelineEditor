...
export const Win32WindowList = memo(
  ({ windows, selectedWindow, onSelect, loading }: Win32WindowListProps) => {
    ...
    const filteredWindows = useMemo(() => {
      if (!normalizedSearchQuery) return windows;

      return windows.filter((window) =>
        [window.window_name, window.class_name, window.hwnd].some((value) =>
    ...
    return (
      <>
        ...
        />
        <Input
          aria-label="搜索 Win32 窗口"
          allowClear
          prefix={<SearchOutlined />}
          placeholder="搜索窗口标题、类名或句柄"
          value={searchQuery}
        ...
        <List
          ...
          locale={{
            emptyText: normalizedSearchQuery
              ? "没有匹配的窗口"
              : "暂无窗口,请点击刷新",
          }}
          split={false}
