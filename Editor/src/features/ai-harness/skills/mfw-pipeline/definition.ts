...
const pipelineGuideSections: Readonly<Record<string, string>> = Object.freeze({
  ...
  "属性字段/附加字段": `focus: 任意 JSON，框架通过回调原样透传，由上层客户端处理。attach: 任意 JSON 对象，不影响执行逻辑；与 default_pipeline.json 中的 attach 进行字典合并。`,
  ...
  "算法类型/NeuralNetworkClassify": `固定区域分类。字段：roi、roi_offset；labels: string[]；model: string，必选，路径相对 model/classify；expected: int | string | (int | string)[]，字符串为 labels 中的标签名；order_by: Horizontal | Vertical | Score | Random | Expected；index: int。`,
  "算法类型/NeuralNetworkDetect": `目标检测。字段：roi、roi_offset；labels: string[]；model: string，必选，路径相对 model/detect；expected: int | string | (int | string)[]，字符串为 labels 中的标签名；threshold: number | number[]，默认 0.3；order_by: Horizontal | Vertical | Score | Area | Random | Expected；index: int。`,
  ...
  "动作类型/TouchDown": `按下触点。字段：contact: uint，默认 0；target: true | string | [x,y] | [x,y,w,h]；target_offset；pressure: int；auto_up: bool，默认 false，启用后任务停止、结束或控制器销毁时自动抬起未释放的触点。`,
  "动作类型/TouchMove": `移动已按下的触点，使用 contact、target、target_offset、pressure；auto_up 仅对 TouchDown 生效。`,
  ...
  "动作类型/KeyDown": `按下按键但不松开。key: int，必选；与 KeyUp 配合。auto_up: bool，默认 false，启用后任务停止、结束或控制器销毁时自动松开未释放的按键。`,
  "动作类型/KeyUp": `松开按键。key: int，必选。`,
  "动作类型/InputText": `输入文本。input_text: string，必选；部分控制器仅支持 ASCII。`,
  "动作类型/StartApp": `启动应用。package: string；ADB 填包名或 activity；Win32 填可执行文件命令行（可带参数，含空格的程序路径用双引号包裹）。PlayCover 不支持。`,
  "动作类型/StopApp": `关闭应用。package: string；ADB 填包名；Win32 填进程文件名（如 game.exe，会终止所有同名进程），留空则终止控制器窗口所属进程。`,
  ...
  节点通知: `focus 接受任意 JSON，随框架回调原样交给上层客户端处理。使用 PI 模板字典时，消息值可为字符串，或 {content?: string, display?: string|string[], trace?: boolean}；允许仅设置 trace。启用遥测的客户端默认上传 Node.PipelineNode.Failed，其余阶段默认不上传，trace 可显式覆盖。MPE 保留配置，不执行遥测上传。`,
});
