...
import { ProjectInterfaceOptions } from "@/features/project-interface/ProjectInterfaceOptions";
  ...
  return (
    <Space orientation="vertical" size={14} style={{ width: "100%" }}>
      ...
      <Select
        ...
        style={{ width: "100%" }}
        value={pi.taskName}
        onChange={pi.selectTask}
        options={[{ value: "", label: "仅环境配置（不应用任务选项）" }, ...(pi.snapshot?.document.task ?? []).map(task => ({ value: task.name, label: task.label ?? task.name }))]}
      />
      <ProjectInterfaceOptions plan={pi.context} channel="debug" />
      {controller.overrideEntries.length > 0 && <Alert type="info" showIcon title="本次还会应用调试覆盖，其优先级高于 PI 配置。" />}
      <List
        size="small"
        bordered
