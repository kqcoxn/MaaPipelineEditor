...
import {
  layoutAchievementGraph,
  type AchievementGraphLayout,
  type AchievementNodeSize,
} from "../graphLayout";
import {
...
const AchievementGraphNode = memo(function AchievementGraphNode({
  data,
}: NodeProps<AchievementNode>) {
  return (
    <>
...
function AchievementRegionNode({ data }: NodeProps<RegionNode>) {
  return (
    <div className={style.graphRegion}>
      <span>{ACHIEVEMENT_CATEGORY_LABELS[data.category]}</span>
    </div>
  );
}
const nodeTypes = {
  achievement: AchievementGraphNode,
  region: AchievementRegionNode,
};
...
function GraphLayout({
  ...
  const sizes = useStore((state) =>
    JSON.stringify(
      [...state.nodeLookup.values()]
        .filter((node) => node.type === "achievement")
        .map((node) => ({
          id: node.id,
          category: (node.data as AchievementNode["data"]).item.def.category,
  ...
  useEffect(() => {
    ...
    void layoutAchievementGraph(nodes, achievementConnections)
      .then((positions) => {
...
function AchievementGraphContent({ items }: { items: AchievementItem[] }) {
  ...
  const onNodesChange = useCallback(
    (changes: NodeChange<AchievementNode | RegionNode>[]) => {
  ...
  const [layout, setLayout] = useState<AchievementGraphLayout>({
    positions: {},
    regions: [],
  });
  ...
  const regions = useMemo<RegionNode[]>(
    () =>
      layout.regions.map((region) => ({
        id: `region:${region.category}`,
        type: "region",
        position: { x: region.x, y: region.y },
        data: { category: region.category },
        measured: measurements[`region:${region.category}`],
        style: {
          width: region.width,
  ...
  const nodes = useMemo<AchievementNode[]>(
    () =>
      items.map((item, index) => ({
        id: item.key,
        type: "achievement",
        position: positions[item.key] ?? { x: index * 352, y: 0 },
        data: { item },
        measured: measurements[item.key],
      })),
    [items, positions, measurements],
  );
  const edges = useMemo(() => {
    const keys = new Set(items.map((item) => item.key));
    return achievementConnections
      .filter(([source, target]) => keys.has(source) && keys.has(target))
      .map(([source, target]) => ({
        id: `${source}:${target}`,
        source,
        target,
        ...
        focusable: false,
        style: {
          stroke: "var(--ant-color-text-quaternary, #bfbfbf)",
          strokeDasharray: "5 6",
          strokeWidth: 1.5,
        },
      }));
  ...
  return (
    <div className={style.graph}>
      <ReactFlow<AchievementNode | RegionNode>
        nodes={[...regions, ...nodes]}
        edges={edges}
        nodeTypes={nodeTypes}
      ...
      >
        <Background
          gap={20}
          size={1}
          color="var(--ant-color-border-secondary, #e5e5e5)"
        />
        <Controls
...
export function AchievementGraph(
  props: Parameters<typeof AchievementGraphContent>[0],
) {
  return (
    <ReactFlowProvider>
      <AchievementGraphContent {...props} />
    </ReactFlowProvider>
  );
}
