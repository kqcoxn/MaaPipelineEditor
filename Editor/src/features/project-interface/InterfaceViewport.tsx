    ...
    const capture = async () => {
      ...
      if (document.visibilityState === "visible") {
        try {
          ...
          else if (!/busy|skipped/.test(result.error ?? "")) setError("截图暂不可用，正在重试");
        } catch { if (!abort.signal.aborted) setError("截图请求失败，正在重试"); }
      }
      if (!abort.signal.aborted) timer = setTimeout(() => void capture(), 800);
    };
    void capture();
    return () => { abort.abort(); clearTimeout(timer); };
  }, [controllerId, enabled]);
  return <section className={`${styles.viewport} ${expanded ? styles.expanded : ""}`} aria-label="设备视口">
