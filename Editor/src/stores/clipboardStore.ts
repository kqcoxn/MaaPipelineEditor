...
export const useClipboardStore = create<ClipboardState>()((set, get) => ({
  ...
  copy(nodes, edges) {
    if (!nodes || nodes.length === 0) {
      message.error(i18n.t("stores.clipboard.noSelection", "未选中节点"));
      return;
    }
    ...
    message.success(
      i18n.t("stores.clipboard.copySuccess", "已将选中节点加载至内部粘贴板"),
    );
  },
  ...
  paste() {
    ...
    if (state.clipboardNodes.length === 0) {
      message.error(
        i18n.t("stores.clipboard.pasteEmpty", "粘贴板中无已复制节点"),
      );
      return null;
    }
