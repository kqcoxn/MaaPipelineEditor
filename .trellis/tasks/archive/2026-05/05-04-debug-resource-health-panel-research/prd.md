...
* 用户可见文案中，“资源加载”应作为展示名称；不直接向用户暴露底层判定实现的措辞。
* 资源加载结果是资源体检对“资源是否可用”的结论来源；若资源加载成功，不应再把资源目录异常清单当作额外阻塞项附加在成功结果后。
* 资源体检中的图/文件检查应按“可维护的常见异常清单（Checklist）”组织，但其中“资源目录异常清单”只在 MaaFW 加载失败后展开，用于继续缩小失败原因，而不是平铺成常驻后置检查。
* 首批资源目录异常清单至少覆盖：已解析资源 bundle 目录中，尤其 `pipeline` 目录下 JSON/JSONC 文件的格式错误、重复节点名。
* 上述资源目录异常清单以 MaaFW 实际参与加载的 bundle 根目录为扫描范围，而不是只看已加载至 MPE 的 graphSnapshot 文件。
* 若扫描资源目录中的相关文件/目录时发生读取失败，需要明确提示该条目未成功检查，而不是静默视为通过。
* 本任务采用中等方案：首期资源体检聚焦“资源路径解析 + 资源加载 + 当前图静态调试合法性”，不纳入样例图识别回归测试。
...
* [ ] 体检结果至少区分“资源解析/资源加载”和“图/运行前约束”两类问题。
* [ ] 当资源路径无法解析、存在歧义或 MaaFW 无法加载时，能展示明确原因与对应路径。
* [ ] 当当前图快照缺少运行所需关键信息时，能展示明确的错误或警告，并关联到 fileId / nodeId / sourcePath。
* [ ] 资源加载成功时，不会再额外附加“资源目录异常清单”结果去覆盖成功判定。
* [ ] 当资源加载失败时，会展开可扩展的资源目录异常清单；首批至少能识别已解析资源 bundle 目录中 pipeline JSON/JSONC 文件的格式错误与重复节点名。
* [ ] 当扫描某个资源目录条目失败时，资源体检会明确提示该条目未成功检查，而不是静默忽略。
...
* 静态检查聚焦“当前调试上下文”：
  ...
  * 当前可选 target
* 后端先执行资源路径解析与资源加载检查；只有在资源加载失败时，才追加“资源目录异常清单”诊断，用于继续缩小失败原因。
* 图静态检查继续聚焦 graph snapshot / resolver snapshot / target 的调试合法性，不再承载 MaaFW 成功后的本地文件异常追加规则。
* 本地失败排查清单改为扫描 MaaFW 已解析的 bundle 根目录，重点覆盖 `pipeline/**/*.json` 与 `pipeline/**/*.jsonc` 的 JSON/JSONC 格式错误和重复节点名，仍通过 `DebugDiagnostic` + 修复建议返回。
* 对资源目录扫描过程中出现的条目读取失败，需要把错误也编码成 `loading` 类诊断返回，避免面板把未检查条目误当成已通过。
...
**Decision**: 已选择中等方案，做独立“资源体检”面板，覆盖资源解析、资源加载、图静态合法性三层，不纳入识别样例回归测试；面板文案对用户统一使用“资源加载”等产品语言，资源目录异常清单只在加载失败时展开用于缩小原因。首期允许少量快捷操作与修复建议，但不做自动修复。

**Consequences**:

* 需要新增综合检查协议与面板，但大部分底层能力可复用。
* 产品形态更贴近 MPE 调试工作流，不会把 MSE 的工程化/CI 化能力硬塞进 DebugModal。
* “常见异常 Checklist” 让后续扩展更多加载失败原因时不必改协议，只需继续追加 `DebugDiagnostic` 规则。
