...
func CheckUpdate(currentVersion string) (*UpdateInfo, error) {
	...
	req.Header.Set("User-Agent", "MaaPipelineEditor-Desktop")
...
func findDownloadURL(assets []struct {
	...
	for _, asset := range assets {
		...
		if strings.Contains(name, "maapipelineeditor-desktop") && strings.Contains(name, platformPattern) {
			return asset.BrowserDownloadURL
		}
	}

	// 如果没有找到 Desktop 包，返回 Release 页面
