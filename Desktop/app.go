...
import (
	...
	"github.com/kqcoxn/MaaPipelineEditor/Desktop/internal/bridge"
	"github.com/kqcoxn/MaaPipelineEditor/Desktop/internal/ports"
	"github.com/kqcoxn/MaaPipelineEditor/Desktop/internal/splash"
	"github.com/kqcoxn/MaaPipelineEditor/Desktop/internal/updater"
...
func (a *App) loadConfig() error {
	...
	// 使用默认路径（如果配置中为空）
	if a.config.MaaFW.LibDir == "" && config.Desktop.DefaultMFWPath != "" {
		a.config.MaaFW.LibDir = filepath.Join(a.exeDir, config.Desktop.DefaultMFWPath)
	}
	if a.config.MaaFW.ResourceDir == "" && config.Desktop.DefaultResourcePath != "" {
		a.config.MaaFW.ResourceDir = filepath.Join(a.exeDir, config.Desktop.DefaultResourcePath)
	}
...
func (a *App) useDefaultConfig() error {
	...
	if runtime.GOOS != "windows" {
		config.Desktop.MPELBPath = "resources/mpelb"
	}
...
func (a *App) startup(ctx context.Context) {
	...
	if a.config.Desktop.RootDir != "" {
		// 使用用户配置的根目录
		a.workDir = a.config.Desktop.RootDir
...
func (a *App) validateLBConfig(path string) bool {
	...
	if config.MaaFW.LibDir != "" {
		if _, err := os.Stat(config.MaaFW.LibDir); os.IsNotExist(err) {
			// 路径不存在，检查默认路径是否可用
			defaultLibDir := filepath.Join(a.exeDir, a.config.Desktop.DefaultMFWPath)
	...
	if config.MaaFW.ResourceDir != "" {
		if _, err := os.Stat(config.MaaFW.ResourceDir); os.IsNotExist(err) {
			// 路径不存在，检查默认路径是否可用
			defaultResourceDir := filepath.Join(a.exeDir, a.config.Desktop.DefaultResourcePath)
...
func (a *App) createLBConfig(path string) error {
	...
	if libDir != "" {
		...
		// 未配置，使用默认路径
		libDir = filepath.Join(a.exeDir, a.config.Desktop.DefaultMFWPath)
	}
	...
	if resourceDir != "" {
		...
		// 未配置，使用默认路径
		resourceDir = filepath.Join(a.exeDir, a.config.Desktop.DefaultResourcePath)
	}
...
func (a *App) SetRootDir(rootDir string) error {
	...
	// 更新配置
	a.config.Desktop.RootDir = rootDir
	a.workDir = rootDir
