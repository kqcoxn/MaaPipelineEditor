...
jump_back 字段可以接收一个列表,当 next 字段全部没有命中时,按照列表下标的顺序依次检测,就近命中,**且在执行完后续所有 next 后返回进入 jump_back 的节点,并重置 timeout 时间**。

基于 next 优先级的循环都可以使用基于 jump_back 的循环代替,它拥有更高的可读性:

![](/images/practice/process/jump_back循环1.png)
...
![](/images/practice/process/jump_back循环2.png)
...
on_error 字段可以接收一个列表,当 timeout 后若 next 与 jump_back 字段全部没有命中时,按照列表下标的顺序依次检测,就近命中。
