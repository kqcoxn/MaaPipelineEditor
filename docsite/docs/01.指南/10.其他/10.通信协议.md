...
### 1. 发送 Pipeline(MPE → 本地服务)

#### 路由: `/etc/send_pipeline`

**方向**: MPE → 本地服务

**用途**: 用户在 MPE 中点击「应用到本地文件」按钮时，将编译后的 Pipeline 发送到服务端。

**请求数据**:

```json
{
  "path": "/etc/send_pipeline",
  "data": {
...
1. 验证 `file_path` 是否存在
2. 将 `pipeline` 对象写入到指定文件

::: tip
未来会添加是否成功等状态回传路径，目前无需处理。
:::

### 2. 接收 Pipeline(本地服务 → MPE)
