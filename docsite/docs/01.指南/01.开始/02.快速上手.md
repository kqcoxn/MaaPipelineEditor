...
我们目前设计了两套节点风格，另有一套**极简美观**与一套**直接在节点编辑**的风格正在设计中，您可以按您的喜好在点击左上角设置按钮后自行切换：
...
**部分导出**：框选任意数量的节点时，面板会显示 `部分导出` 功能，仅会导出选中的节点与连接的 Pipeline JSON。

**全部导出**：使用 `全部导出` 功能，此功能无视选中的节点，会直接导出当前文件所有节点与连接的 Pipeline JSON。
