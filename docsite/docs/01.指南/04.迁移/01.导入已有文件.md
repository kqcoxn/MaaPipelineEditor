...
MPE 支持 v1 与 v2 两种 Pipeline 协议，并能自动检测每个节点的协议版本。您可以在同一个文件中混合使用两种协议的节点，MPE 会自动识别并正确解析，但一个节点中若存在两种协议将不会被支持。

导入后，所有节点都会统一以 v2 格式导出。
...
2. 点击 MPE 右侧 `Pipeline JSON` 面板的 `导入` 按钮，即可导入当前文件。MPE 会自动检测并兼容 v1/v2 协议格式。
