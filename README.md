...
- **可视化编辑**：注重编辑功能，更注重阅读体验！使用流程化思路构建与阅读您的 Pipeline，多种节点样式满足不同数据读取偏好，紧凑。
- **完全兼容性**：兼容旧项目，支持节点级 v1 与 v2 协议混合导入，提供自动布局与自动迁移旧字段功能，支持复合类型在内的绝大多数字段类型，提供一键自动排版功能。
- **类原生体验**：单面板增量式分类字段添加，减轻操作负担；IDE 级字段编辑灵活度，提供多种字段语法糖简化类型体操
- **AI Bless**：拥抱 AI，目前已实装智能节点搜索，更多功能正在开发中！
- 更多功能：当前正在开发流程级调试功能！预计 `12.17` 前实装。
