...
## 更新摘要
**所做更改**
- 更新了 CustomRecognitionArg 结构体字段说明，从 TaskDetail 字段改为 TaskID 字段
- 修改了回调代理逻辑说明，强调使用 TaskID 获取任务详情的方式
- 更新了示例代码中的参数说明，反映新的 API 使用方式
- 补充了 TaskID 与 TaskDetail 的关系说明
...
**更新** 本版本重点介绍了 CustomRecognitionArg 结构体中 TaskDetail 字段已被 TaskID 字段替代的变更，简化了回调代理逻辑，开发者现在可以通过 TaskID 从 Tasker 中获取任务详情。
...
Tasker["Tasker 任务详情<br/>tasker.go"]
end
subgraph "原生层"
...
- [internal/buffer/image_buffer.go](file://internal/buffer/image_buffer.go#L48-L140)
- [internal/buffer/rect_buffer.go](file://internal/buffer/rect_buffer.go#L1-L59)
- [resource.go](file://resource.go#L138-L168)
...
- [tasker.go](file://tasker.go#L407-L420)
...
- CustomRecognitionArg：包含任务 ID、当前任务名、自定义识别器名称、参数字符串、图像对象与 ROI 区域。
...
**更新** CustomRecognitionArg 结构体已简化，TaskDetail 字段被 TaskID 字段替代，开发者需要通过 TaskID 从 Tasker 中获取任务详情。
...
participant Tasker as "Tasker 任务详情"
...
- 参数结构 CustomRecognitionArg
  - TaskID：int64 类型的任务 ID，可通过 Tasker.GetTaskDetail(TaskID) 获取任务详情
...
**更新** CustomRecognitionArg 结构体已简化，TaskDetail 字段已被 TaskID 字段替代。TaskID 是一个整数标识符，开发者需要通过 Tasker.GetTaskDetail(TaskID) 来获取完整任务详情。
...
class Tasker {
+GetTaskDetail(taskId int64) *TaskDetail
}
class Rect {
+X() int
+Y() int
...
CustomRecognitionArg --> Context : "使用 TaskID"
CustomRecognitionArg --> Rect : "ROI"
Context --> Tasker : "获取任务详情"
Tasker --> CustomRecognitionArg : "通过 TaskID"
```
...
- 回调代理 _MaaCustomRecognitionCallbackAgent
  ...
  - 从原生句柄构造 Context、TaskID、image.Image 与 Rect
...
**更新** 回调代理逻辑已简化，直接使用 TaskID 而非 TaskDetail，减少了不必要的数据复制。
...
Map-->>Bridge : 返回实现
Bridge->>Bridge : 构造 Context/TaskID/Img/Roi
...
- [resource.go](file://resource.go#L138-L168)
- [resource.go](file://resource.go#L170-L213)
- [resource.go](file://resource.go#L196-L213)
- [agent_server.go](file://agent_server.go#L9-L20)
...
**图表来源**
- [examples/custom-recognition/main.go](file://examples/custom-recognition/main.go#L58-L61)
- [examples/custom-recognition/resource/pipeline/pipeline.json](file://examples/custom-recognition/resource/pipeline/pipeline.json#L1-L12)
- [custom_recognition.go](file://custom_recognition.go#L56-L100)
...
### TaskID 与 TaskDetail 的关系
- TaskID 字段
  - CustomRecognitionArg 中的 TaskID 是一个整数标识符
  - 通过 Tasker.GetTaskDetail(TaskID) 可以获取完整的任务详情
- TaskDetail 获取
  - Context 提供 GetTasker() 方法获取 Tasker 实例
  - Tasker.GetTaskDetail(taskId int64) 返回 *TaskDetail 指针
- 使用场景
  - 在 Run 方法中需要访问任务详情时，通过 TaskID 从 Tasker 获取
  - 避免了在回调中传递完整任务详情的开销

**新增** 详细说明了 TaskID 与 TaskDetail 的关系，这是本次变更的核心内容。
...
- [custom_recognition.go](file://custom_recognition.go#L56-L100)
- [context.go](file://context.go#L1-L200)
- [internal/buffer/image_buffer.go](file://internal/buffer/image_buffer.go#L1-L140)
- [internal/buffer/rect_buffer.go](file://internal/buffer/rect_buffer.go#L1-L59)
- [resource.go](file://resource.go#L138-L168)
- [agent_server.go](file://agent_server.go#L9-L20)
- [internal/native/framework.go](file://internal/native/framework.go#L51-L51)
...
- TaskID 获取
  - 使用 TaskID 替代 TaskDetail 减少了数据传输开销，提升了回调性能

**更新** 新增了 TaskID 获取的性能考量，强调了使用 TaskID 的优势。
...
- TaskID 相关问题
  - 确认 TaskID 有效且未过期
  - 检查 Tasker.GetTaskDetail(TaskID) 是否能正常获取任务详情

**更新** 新增了 TaskID 相关的故障排查指导。
...
**更新** 本次变更简化了 CustomRecognitionArg 结构，使用 TaskID 替代 TaskDetail，提升了性能并简化了回调逻辑。开发者现在可以通过 TaskID 从 Tasker 获取任务详情，避免了不必要的数据复制开销。
...
- ImageBuffer.Get/Set
- RectBuffer.Get/Set
- Tasker.GetTaskDetail(taskId) -> *TaskDetail

**更新** 新增了 Tasker.GetTaskDetail 的 API 说明。