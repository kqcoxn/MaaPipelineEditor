...
- [node_recognition_test.go](file://node_recognition_test.go)
...
## 更新摘要
**变更内容**
- 新增了SubRecognitionItem结构体，支持节点名称引用和内联识别对象的统一表示
- 引入Ref()和Inline()辅助函数，简化AND/OR识别配置的构建
- 增强了AND/OR识别系统的灵活性，支持混合使用节点引用和内联识别
- 新增InlineSubRecognition结构体，用于内联识别配置的序列化和反序列化
- 更新了NodeAndRecognitionParam和NodeOrRecognitionParam的参数结构，支持更复杂的识别组合
...
## 简介
本篇文档聚焦于 Node 结构体中的 Recognition 字段，系统性解析 NodeRecognition 类型与其 Param 参数的结构设计，覆盖 DirectHit、TemplateMatch、FeatureMatch、ColorMatch、OCR、NeuralNetworkClassify、NeuralNetworkDetect、AND、OR、Custom 等识别类型。文档将结合 NodeRecognitionType 常量定义与各识别类型的构造函数，说明如何通过代码配置不同识别算法；并深入解释 UnmarshalJSON 的动态解析机制，最后给出模板匹配、特征匹配、颜色匹配等参数的配置策略与最佳实践。

**更新** 重大增强：AND/OR 识别系统现已支持 SubRecognitionItem，通过 Ref() 和 Inline() 辅助函数实现节点名称引用和内联识别对象的灵活组合。
...
O --> P["InlineSubRecognition<br/>SubName + NodeRecognition"]
Q["Ref() 辅助函数"] --> O
R["Inline() 辅助函数"] --> O
```

**图表来源**
- [node.go](file://node.go#L9-L51)
- [node_recognition.go](file://node_recognition.go#L9-L15)
- [node_recognition.go](file://node_recognition.go#L747-L756)
- [node_recognition.go](file://node_recognition.go#L801-L819)
- [node_recognition.go](file://node_recognition.go#L853-L858)
- [node_recognition.go](file://node_recognition.go#L889-L893)
...
+[]SubRecognitionItem AllOf
+int BoxIndex
}
class NodeOrRecognitionParam {
+[]SubRecognitionItem AnyOf
}
Node --> NodeRecognition : "拥有"
NodeRecognition --> NodeRecognitionParam : "组合"
NodeAndRecognitionParam --> SubRecognitionItem : "包含多个"
NodeOrRecognitionParam --> SubRecognitionItem : "包含多个"
...
- [node_recognition.go](file://node_recognition.go#L9-L15)
- [node_recognition.go](file://node_recognition.go#L747-L756)
- [node_recognition.go](file://node_recognition.go#L821-L826)
- [node_recognition.go](file://node_recognition.go#L853-L858)
- [node_recognition.go](file://node_recognition.go#L889-L893)
...
## 架构总览
下图展示了识别配置在运行时的解析流程：JSON 中的 recognition 节点被反序列化为 NodeRecognition，随后根据 Type 选择对应参数结构体并完成参数解析。对于 AND/OR 识别类型，其参数结构体会进一步解析为 SubRecognitionItem 数组，每个子项都支持节点名称引用或内联识别配置。
...
**图表来源**
- [node_recognition.go](file://node_recognition.go#L18-L66)
- [node_recognition.go](file://node_recognition.go#L758-L783)
- [node_recognition.go](file://node_recognition.go#L828-L842)
...
**章节来源**
- [node_recognition.go](file://node_recognition.go#L96-L101)
...
**章节来源**
- [node_recognition.go](file://node_recognition.go#L196-L210)
- [node_recognition.go](file://node_recognition.go#L147-L194)
...
**章节来源**
- [node_recognition.go](file://node_recognition.go#L317-L332)
- [node_recognition.go](file://node_recognition.go#L261-L315)
...
**章节来源**
- [node_recognition.go](file://node_recognition.go#L430-L445)
- [node_recognition.go](file://node_recognition.go#L381-L428)
...
**章节来源**
- [node_recognition.go](file://node_recognition.go#L548-L560)
- [node_recognition.go](file://node_recognition.go#L485-L546)
...
- 关键参数：
  ...
  - Index：选择第几个匹配结果，默认 0。
- 构造与选项：
  - RecNeuralNetworkClassify(model, opts...)：创建分类识别。
...
**章节来源**
- [node_recognition.go](file://node_recognition.go#L637-L652)
- [node_recognition.go](file://node_recognition.go#L595-L635)
...
- 关键参数：
  ...
  - Index：选择第几个匹配结果，默认 0。
- 构造与选项：
  - RecNeuralNetworkDetect(model, opts...)：创建检测识别。
...
**章节来源**
- [node_recognition.go](file://node_recognition.go#L730-L745)
- [node_recognition.go](file://node_recognition.go#L688-L729)
...
- 构造与选项：
  - RecAnd(items, opts...)：创建 AND 识别。
  - RecAndItems(items...)：构建 RecAnd 的第一个参数。
  - WithAndRecognitionBoxIndex(boxIndex)：设置结果框索引。
...
**更新** 重大增强：AND 识别系统现在支持 SubRecognitionItem，通过 Ref() 和 Inline() 辅助函数实现节点名称引用和内联识别对象的灵活组合。

**章节来源**
- [node_recognition.go](file://node_recognition.go#L853-L881)
- [node_recognition.go](file://node_recognition.go#L883-L887)
- [node_recognition.go](file://node_recognition.go#L796-L819)
- [node_recognition.go](file://node_recognition.go#L844-L851)
- [node_recognition.go](file://node_recognition.go#L758-L783)
- [node_recognition.go](file://node_recognition.go#L828-L842)
- [node_recognition_test.go](file://node_recognition_test.go#L127-L153)
...
**更新** 重大增强：OR 识别系统现在支持 SubRecognitionItem，通过 Ref() 和 Inline() 辅助函数实现节点名称引用和内联识别对象的灵活组合。

**章节来源**
- [node_recognition.go](file://node_recognition.go#L889-L907)
- [node_recognition.go](file://node_recognition.go#L801-L819)
- [node_recognition_test.go](file://node_recognition_test.go#L142-L152)
...
**章节来源**
- [node_recognition.go](file://node_recognition.go#L947-L959)
...
P --> AND["NodeAndRecognitionParam"]
P --> OR["NodeOrRecognitionParam"]
AND --> SRI["SubRecognitionItem"]
OR --> SRI
SRI --> ISR["InlineSubRecognition"]
SRI --> REF["Ref() 辅助函数"]
SRI --> INLINE["Inline() 辅助函数"]
ISR --> NR["NodeRecognition"]
```
...
- [node_recognition.go](file://node_recognition.go#L758-L783)
- [node_recognition.go](file://node_recognition.go#L828-L842)
- [node_recognition_test.go](file://node_recognition_test.go#L10-L31)

## 结论
通过 NodeRecognition 与 NodeRecognitionParam 的接口化设计，框架实现了"按类型注入"的灵活识别配置。配合 UnmarshalJSON 的动态解析机制，开发者既可通过 JSON 快速声明识别策略，也可通过构造函数进行细粒度编程式配置。针对不同识别类型，合理设置参数与 ROI 是提升准确率与性能的关键。

**更新** 重大增强：AND/OR 识别系统现已支持 SubRecognitionItem，通过 Ref() 和 Inline() 辅助函数实现节点名称引用和内联识别对象的灵活组合。这一增强使得识别配置更加灵活和强大，能够满足复杂的业务场景需求。InlineSubRecognition 的引入进一步完善了内联识别的序列化和反序列化机制，为开发者提供了更丰富的配置选项。

对于复杂业务，AND/OR 识别提供了强大的组合能力，而 Custom 识别则提供了无限扩展可能。SubRecognitionItem 的统一表示简化了配置构建过程，提高了代码的可读性和可维护性。
