...
## 更新摘要
**变更内容**
- 更新了 AND 识别类型的结构设计，从直接结构体值重构为指针数组
- 新增了 NodeAndRecognitionItem 的自定义 JSON 反序列化实现
- 修正了 AllOf 字段类型为指针数组的说明
- 增加了 SubName 字段的使用说明和最佳实践
...
- NodeAndRecognitionItem 结构体专门处理 AND 识别类型的复合配置，支持子识别项的命名和独立解析。
...
D --> M["AndRecognitionParam<br/>AllOf: []*NodeAndRecognitionItem"]
D --> N["OrRecognitionParam<br/>AnyOf: []*NodeRecognition"]
M --> O["NodeAndRecognitionItem<br/>SubName + NodeRecognition"]
```
...
- [node_recognition.go](file://node_recognition.go#L83-L86)
- [node_recognition.go](file://node_recognition.go#L746-L780)
- [node_recognition.go](file://node_recognition.go#L808-L824)
...
- NodeAndRecognitionItem 专门处理 AND 识别类型的复合配置，支持子识别项的命名和独立解析。
...
NodeAndRecognitionParam --> NodeAndRecognitionItem : "包含多个"
NodeAndRecognitionItem --> NodeRecognition : "组合"
```
...
## 架构总览
下图展示了识别配置在运行时的解析流程：JSON 中的 recognition 节点被反序列化为 NodeRecognition，随后根据 Type 选择对应参数结构体并完成参数解析。对于 AND 识别类型，其参数结构体会进一步解析为 NodeAndRecognitionItem 数组，每个子项都包含独立的识别配置和可选的子名称。
...
participant AI as "NodeAndRecognitionItem"
JSON->>U : 提供原始字节
U->>T : 解析 type
T->>P : 根据 type 分派参数类型
U->>P : 反序列化 param
...
**图表来源**
- [node_recognition.go](file://node_recognition.go#L17-L65)
- [node_recognition.go](file://node_recognition.go#L751-L765)
...
**章节来源**
- [node_recognition.go](file://node_recognition.go#L88-L100)
...
**章节来源**
- [node_recognition.go](file://node_recognition.go#L121-L139)
- [node_recognition.go](file://node_recognition.go#L195-L209)
...
**章节来源**
- [node_recognition.go](file://node_recognition.go#L233-L253)
- [node_recognition.go](file://node_recognition.go#L316-L331)
...
**章节来源**
- [node_recognition.go](file://node_recognition.go#L353-L373)
- [node_recognition.go](file://node_recognition.go#L429-L444)
...
**章节来源**
- [node_recognition.go](file://node_recognition.go#L457-L477)
- [node_recognition.go](file://node_recognition.go#L547-L559)
...
**章节来源**
- [node_recognition.go](file://node_recognition.go#L571-L587)
- [node_recognition.go](file://node_recognition.go#L636-L651)
...
**章节来源**
- [node_recognition.go](file://node_recognition.go#L664-L680)
- [node_recognition.go](file://node_recognition.go#L729-L744)

### 8) AND 识别组合
- 适用场景：需要多个子识别条件同时满足的复杂识别逻辑，如多步骤验证、条件组合等。
- 关键参数：
  - AllOf：子识别项数组，每个元素都是 NodeAndRecognitionItem 指针。
  - BoxIndex：选择哪个子识别结果的边界框作为最终结果，默认 0。
  - 子识别项 NodeAndRecognitionItem：
    - SubName：子识别项的可选名称，用于标识和调试。
    - NodeRecognition：嵌套的识别配置，支持任意识别类型。
- 构造与选项：
  - RecAnd(allOf, opts...)：创建 AND 识别。
  - AndItem(subName, recognition)：创建子识别项。
  - WithAndRecognitionBoxIndex(boxIndex)：设置结果框索引。
- 自定义 JSON 反序列化：
  - NodeAndRecognitionItem 实现了自定义 UnmarshalJSON 方法，支持 sub_name 字段和嵌套识别配置。
  - 支持在单个 JSON 对象中同时包含子名称和识别配置。
- 最佳实践：
  - 为重要的子识别项设置有意义的 SubName，便于调试和维护。
  - 合理选择 BoxIndex，通常选择最可靠的子识别结果。
  - 子识别项的顺序会影响执行效率，将最可能失败的条件放在前面。

**更新** 重要变更：NodeAndRecognitionItem 现在使用指针数组 AllOf: []*NodeAndRecognitionItem，并实现了自定义 JSON 反序列化逻辑。

**章节来源**
- [node_recognition.go](file://node_recognition.go#L746-L780)
- [node_recognition.go](file://node_recognition.go#L794-L806)
- [node_recognition.go](file://node_recognition.go#L751-L765)
- [node_recognition_test.go](file://node_recognition_test.go#L10-L165)

### 9) OR 识别组合
- 适用场景：需要多个子识别条件中任一满足的识别逻辑，如多模式识别、备用方案等。
- 关键参数：
  - AnyOf：子识别项数组，每个元素都是 NodeRecognition 指针。
- 构造与选项：
  - RecOr(anyOf)：创建 OR 识别。
- 最佳实践：
...
**章节来源**
- [node_recognition.go](file://node_recognition.go#L808-L824)
- [node_recognition.go](file://node_recognition.go#L815-L824)
...
**章节来源**
- [node_recognition.go](file://node_recognition.go#L826-L877)
...
- AND 识别类型通过 NodeAndRecognitionItem 实现复杂的多条件组合。
...
AND --> AI["NodeAndRecognitionItem"]
AI --> NR["NodeRecognition"]
```
...
- AND 识别问题
  - 检查 AllOf 数组中的子识别项是否正确配置。
  - 确认 SubName 字段的使用符合预期。
...
## 结论
通过 NodeRecognition 与 NodeRecognitionParam 的接口化设计，框架实现了"按类型注入"的灵活识别配置。配合 UnmarshalJSON 的动态解析机制，开发者既可通过 JSON 快速声明识别策略，也可通过构造函数进行细粒度编程式配置。针对不同识别类型，合理设置参数与 ROI 是提升准确率与性能的关键。对于复杂业务，AND/OR 识别提供了强大的组合能力，而 Custom 识别则提供了无限扩展可能。
...
- AND：设置 AllOf（指针数组）、BoxIndex；子项支持 SubName 命名。
- OR：设置 AnyOf（指针数组）。
