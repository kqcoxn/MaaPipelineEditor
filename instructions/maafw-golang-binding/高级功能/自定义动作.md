...
## 更新摘要
**变更内容**
- 更新CustomActionArg结构体说明，TaskDetail字段已替换为TaskID字段
- 更新回调代理逻辑说明，简化了任务详情查询流程
- 更新示例代码中的参数使用说明
- 更新架构图和流程说明以反映新的API使用方式
...
- 解释 CustomAction 接口设计与 Run 方法的调用时机、参数含义（CustomActionArg 中的任务ID、识别结果、目标框等）
...
- [resource.go](file://resource.go#L215-L245)
- [tasker.go](file://tasker.go#L415-L467)
- [context.go](file://context.go#L1-L240)
- [job.go](file://job.go#L1-L96)
- [internal/native/framework.go](file://internal/native/framework.go#L194-L393)
...
- CustomActionArg 参数：封装当前任务ID、当前节点名、自定义动作名、自定义参数、识别结果、目标框等
- 注册与回调：
...
- 任务详情与上下文：
  - Tasker.GetTaskDetail：通过任务ID查询任务详情
...
- 接口定义：Run(ctx *Context, arg *CustomActionArg) bool
  ...
  - arg：动作参数，包含任务ID、当前节点名、自定义动作名、自定义参数、识别结果、目标框
- 参数字段说明（来自 CustomActionArg 定义）：
  - TaskID：当前任务的ID（整数类型），可通过 Tasker.GetTaskDetail 查询详细信息
...
**更新** TaskDetail字段已替换为TaskID字段，简化了回调代理逻辑，现在需要通过TaskID手动查询任务详情
...
- _MaaCustomActionCallbackAgent：
  ...
  - 通过 Context.GetTasker 获取 Tasker，使用 TaskID 查询任务详情
...
Lookup -- 是 --> BuildArg["构建CustomActionArg<br/>使用TaskID查询任务详情+Box"]
...
### 运行时数据流与任务详情
- Tasker.GetTaskDetail：通过任务ID查询任务条目、节点列表与状态，组装为 TaskDetail
...
- 动作执行但任务失败
  ...
  - 使用 Tasker.GetTaskDetail 获取详细信息定位问题
- 回调未触发
  - 确认资源已加载且已绑定到 Tasker
...
**章节来源**
- [resource.go](file://resource.go#L247-L290)
- [job.go](file://job.go#L66-L96)
- [tasker.go](file://tasker.go#L415-L467)

## 结论
maa-framework-go 的自定义动作机制通过"接口 + 回调代理 + 资源注册"的方式，将 Go 实现无缝注入到原生任务执行流程中。CustomAction.Run 提供了丰富的上下文与识别信息，使得开发者可以实现复杂的条件判断、状态维护与外部服务集成。新的 TaskID 字段设计简化了回调代理逻辑，通过明确的任务ID查询机制，开发者可以更灵活地获取任务详情。配合严格的线程安全与资源管理策略，可在保证稳定性的同时获得良好的性能表现。

## 附录
- 关键流程参考路径
  - 注册回调：[resource.go](file://resource.go#L215-L245)
  - 回调代理：[custom_action.go](file://custom_action.go#L50-L94)
  - 任务详情查询：[tasker.go](file://tasker.go#L415-L467)
  - 上下文能力：[context.go](file://context.go#L211-L239)
  - 示例程序：[examples/custom-action/main.go](file://examples/custom-action/main.go#L1-L76)
