...
## 更新摘要
**变更内容**
- WaitFreezes 方法现在返回 error 而非布尔值，提供更精确的错误处理
- Context.GetNode 确保正确初始化节点附件（Attach）为非空映射
...
**图表来源**
- [context.go](file://context.go#L1-L487)
...
- [wait_freezes.go](file://wait_freezes.go#L1-L62)
...
- **WaitFreezesParam**：专门用于 WaitFreezes 方法的参数结构，支持时间、阈值、方法等配置
...
- **新增**：状态监控阶段：WaitFreezes 提供屏幕稳定检测功能，返回 error 而非布尔值
...
- **状态监控**
  - **更新**：WaitFreezes：等待屏幕稳定检测，返回 error 类型，支持超时和ROI参数
- **查询与定位**
...
**章节来源**
- [context.go](file://context.go#L75-L487)
- [internal/native/framework.go](file://internal/native/framework.go#L215-L233)
...
+WaitFreezes(duration, box, params...) error
+OverridePipeline(override) error
+OverrideNext(name, nextList) error
+OverrideImage(name, image) error
+GetNodeJSON(name) (string, error)
+GetNode(name) (*Node, error)
+GetTaskJob() *TaskJob
+GetTasker() *Tasker
+Clone() *Context
+SetAnchor(name, node) error
+GetAnchor(name) (string, error)
+GetHitCount(name) (uint64, error)
...
**章节来源**
- [context.go](file://context.go#L407-L487)
- [tasker.go](file://tasker.go#L1-L433)
...
### WaitFreezes 错误处理增强
WaitFreezes 方法现在返回 error 类型而非布尔值，提供更精确的错误处理机制：

- **返回值变化**：从 `bool` 改为 `error`
- **成功条件**：返回 nil 表示屏幕稳定检测成功
- **失败条件**：返回错误表示超时或失败
- **参数支持**：支持 duration、box（可选）和 WaitFreezesParam（可选）

```mermaid
flowchart TD
A[调用 WaitFreezes] --> B{屏幕是否稳定?}
B --> |是| C[返回 nil]
B --> |否| D[返回错误]
E[调用方检查返回值] --> F{err == nil?}
F --> |是| G[继续执行]
F --> |否| H[处理错误并重试/退出]
```
...
### Context.GetNode 节点附件初始化
Context.GetNode 方法现在确保节点附件（Attach）正确初始化：

- **默认值处理**：如果节点的 Attach 为 nil，则初始化为空映射
- **数据完整性**：保证返回的 Node 结构始终包含有效的 Attach 映射
- **向后兼容**：不影响现有代码的正常运行

**章节来源**
- [context.go](file://context.go#L375-L391)
...
- **更新**：**WaitFreezes**：通过 WaitFreezes 等待屏幕稳定，返回 error 类型提供精确的错误处理，支持超时控制和ROI参数，确保状态一致性

**章节来源**
- [context.go](file://context.go#L418-L487)
- [job.go](file://job.go#L1-L96)
...
- **新增**：WaitFreezesParam 为 WaitFreezes 方法提供参数序列化支持
...
- **更新**：**WaitFreezes** 采用非阻塞等待机制，返回 error 类型提供精确的错误处理，支持超时控制，避免长时间阻塞

**章节来源**
- [context.go](file://context.go#L60-L487)
- [tasker.go](file://tasker.go#L1-L433)
- [internal/store/store.go](file://internal/store/store.go#L1-L65)
...
- **更新**：**WaitFreezes 问题**
  - 检查返回的 error 是否为 nil，nil 表示成功，非 nil 表示超时或失败
  - 检查超时参数是否合理设置
  - 确认 ROI 参数是否正确
  - 验证图像缓冲区是否有效
- **节点附件问题**
  - 使用 GetNode 获取节点时，Attach 字段始终为非空映射
  - 如果需要清空附件，可以使用 ClearHitCount 或重新初始化节点
...
**更新的功能增强了 Context 的可靠性**：WaitFreezes 现在返回 error 类型提供精确的错误处理，Context.GetNode 确保节点附件正确初始化，这些改进使得开发者能够构建更加健壮和可靠的自动化流程。
...
- 使用 ctx.WaitFreezes 等待屏幕稳定，检查返回的 error 是否为 nil 来判断成功与否
...
- **WaitFreezes 示例**：等待屏幕稳定，检查返回的 error 是否为 nil
...
### WaitFreezes 错误处理最佳实践
- **检查返回值**：始终检查 WaitFreezes 的返回 error，nil 表示成功
- **错误处理**：对于非 nil 错误，根据具体情况进行重试或终止
- **参数配置**：合理设置 WaitFreezesParam 的 Time、Timeout 和 RateLimit 参数
- **超时处理**：根据业务需求设置合适的超时时间，避免无限等待
