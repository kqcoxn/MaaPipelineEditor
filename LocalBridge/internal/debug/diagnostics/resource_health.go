...
func (s *Service) checkResourceLoadDiagnostics(
	...
	for index, resolution := range checkedResolutions {
		single := s.checkSingleResourceLoadDiagnostics(resolution, checkedResolutions[:index+1])
...
func (s *Service) checkSingleResourceLoadDiagnostics(
	...
	loadSequence []mfw.ResourceBundleResolution,
) resourceHealthLoadCheckResult {
	...
	paths := make([]string, 0, len(loadSequence))
	for _, item := range loadSequence {
		paths = append(paths, item.ResolvedPath)
	}
	hash, checkedResolutions, err := checker(paths)
	if err != nil {
