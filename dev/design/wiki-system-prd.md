...
- **思考入口（Ponder Trigger）**：可复用的帮助触点组件。默认呈现为问号或书本图标，hover 显示简述，按住 `W` 或点击后打开 Wiki。
...
- 鼠标点击、键盘 `Enter`/`Space` 也能打开 Wiki，作为无键盘长按或触屏场景的兜底。
...
- 点击图标直接打开 Wiki，避免用户不知道“长按 W”时卡住。
...
- `WikiPonderTrigger` 支持 focus、`Enter`、`Space` 打开。
