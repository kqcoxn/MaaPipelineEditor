...
## 更新摘要
**变更内容**
- 更新版本信息：从 v5.6.0 更新到 v5.7.0-alpha.2
- 更新包版本引用：从 v3 更新到 v4
- 更新示例代码中的包导入路径
- 更新框架版本标识
...
- [controller.go](file://controller.go#L30-L54)
- [resource.go](file://resource.go#L24-L42)
...
**更新** 包版本从 v3 升级到 v4，对应版本号 v5.7.0-alpha.2。

**章节来源**
- [maa.go](file://maa.go#L122-L171)
- [tasker.go](file://tasker.go#L15-L79)
- [controller.go](file://controller.go#L26-L176)
- [resource.go](file://resource.go#L15-L62)
- [job.go](file://job.go#L161-L169)
...
- [examples/quick-start/main.go](file://examples/quick-start/main.go#L1-L65)
- [maa.go](file://maa.go#L122-L171)
- [internal/native/native.go](file://internal/native/native.go#L5-L26)
...
**章节来源**
- [tasker.go](file://tasker.go#L21-L79)
- [tasker.go](file://tasker.go#L75-L79)
- [tasker.go](file://tasker.go#L75-L79)
...
- [controller.go](file://controller.go#L30-L54)
- [controller.go](file://controller.go#L275-L279)
- [controller.go](file://controller.go#L282-L371)
...
### 逐行解析与作用说明
- 第 1-8 行：导入 fmt、os 与 MAA 包，注意包版本已更新为 v4。
- 第 10-15 行：初始化框架，配置初始化选项（用户路径与默认 JSON），用于工具集初始化。
- 第 16-21 行：创建 Tasker 实例，defer 在退出时销毁 Tasker，释放资源。
- 第 23-28 行：查找第一个可用的 ADB 设备。
- 第 29-36 行：使用设备信息创建 ADB 控制器，参数包括 ADB 路径、设备地址、截图方法、输入方法、配置、代理二进制路径。
- 第 41-43 行：defer 在退出时销毁控制器。
- 第 42 行：异步发起连接，Wait 阻塞直到连接完成。
- 第 43 行：将控制器绑定到 Tasker。
- 第 45-49 行：创建资源对象，defer 在退出时销毁资源。
- 第 50-52 行：异步加载资源包路径，Wait 阻塞直到加载完成。
- 第 52 行：将资源绑定到 Tasker。
- 第 53-56 行：检查 Tasker 初始化状态，若未初始化则输出错误并退出。
- 第 58-63 行：提交名为 Startup 的任务，Wait 等待完成，获取任务详情并打印。
...
**图表来源**
- [tasker.go](file://tasker.go#L57-L174)
- [controller.go](file://controller.go#L275-L404)
- [resource.go](file://resource.go#L292-L427)
- [job.go](file://job.go#L161-L169)
...
**更新** 版本升级至 v5.7.0-alpha.2，包版本 v4 提供了更稳定的 API 和更好的性能表现。
...
**章节来源**
- [examples/quick-start/resource/pipeline/pipeline.json](file://examples/quick-start/resource/pipeline/pipeline.json#L1-L5)
- [version.go](file://version.go#L5-L8)
