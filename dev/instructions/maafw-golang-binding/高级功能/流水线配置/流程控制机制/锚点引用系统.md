...
- [node_test.go](file://node_test.go)
</cite>

## 更新摘要
**变更内容**
- 更新锚点数据结构：从简单的字符串切片([]string)改为映射(map[string]string)
- 更新锚点管理方法：支持锚点的设置、获取、清除和删除操作
- 更新节点锚点定义：Node结构体的Anchor字段现在使用映射格式
- 更新GetNodeData输出格式：与C++ GetNodeData输出格式对齐
- 新增锚点清除功能：支持将锚点设置为空字符串来清除之前的锚点
...
**更新** 锚点数据结构已从简单的字符串切片升级为映射格式，提供更灵活的锚点管理和清除能力。
...
**更新** Node结构体的Anchor字段现在使用映射格式，支持更灵活的锚点管理。
...
- 节点定义阶段：在节点上声明锚点集合（映射格式），并在下一跳列表中以"锚点"属性引用锚点名。
...
**图表来源**
- [context.go](file://context.go#L436-L453)
- [tasker.go](file://tasker.go#L285-L344)
- [internal/native/framework.go](file://internal/native/framework.go#L229-L232)

## 详细组件分析

### 组件A：锚点定义与注册（Pipeline/Node）
- 节点锚点集合：节点结构包含锚点映射（map[string]string），用于标识可被下一跳引用的目标。键为锚点名称，值为目标节点名称。
- 节点下一跳项：下一跳列表项支持"锚点"属性，表示该项应被视为锚点引用而非普通节点名。
- 工具方法：提供添加/移除锚点的方法，保证去重与幂等；提供添加/移除下一跳项的方法，避免重复添加。

**更新** 锚点集合现在使用映射格式，支持清除锚点（将值设为空字符串）。
...
+SetAnchorTarget(anchor, nodeName) *Node
+ClearAnchor(anchor) *Node
...
**图表来源**
- [node.go](file://node.go#L10-L52)
- [node.go](file://node.go#L201-L217)
- [node.go](file://node.go#L373-L392)
- [node.go](file://node.go#L332-L371)
...
- [node.go](file://node.go#L373-L392)
- [node.go](file://node.go#L332-L371)
- [node_test.go](file://node_test.go#L1-L82)
...
**更新** SetAnchor和GetAnchor方法保持原有签名，但底层实现现在使用映射格式的锚点管理。
...
- [node_test.go](file://node_test.go#L1-L53)
...
**更新** 检查锚点映射格式：确认锚点集合使用正确的映射格式，键为锚点名称，值为目标节点名称或空字符串。

**章节来源**
- [context.go](file://context.go#L444-L453)
- [tasker.go](file://tasker.go#L285-L344)
- [context_test.go](file://context_test.go#L1108-L1196)
- [node_test.go](file://node_test.go#L1-L82)
...
**更新** 新的映射格式锚点系统提供了更强大的功能，包括锚点清除、批量管理等特性，进一步增强了锚点系统的灵活性和实用性。
...
- 锚点清除：使用ClearAnchor功能清除不再需要的锚点，避免锚点冲突。
- 最佳实践：
  - 为每个锚点命名清晰、语义明确，避免歧义。
  - 在下一跳列表中显式标注锚点属性，确保解析逻辑清晰。
  - 使用任务详情与锚点查询进行调试，及时发现锚点未生效或解析失败的问题。
  - 合理使用锚点清除功能，保持锚点集合的整洁性。
  - 利用映射格式的优势，支持复杂的锚点管理需求。

**更新** 新增锚点清除和映射格式使用的最佳实践建议。
