...
## 更新摘要
**变更内容**
- 新增全局配置选项章节，详细介绍 SetSaveOnError、SetDrawQuality 和 SetRecoImageCacheLimit 三个新功能
- 更新性能考虑部分，增加关于图像质量与缓存管理的最佳实践
- 添加全局配置选项的使用示例和调试建议
...
- **新增** 全局配置选项：SetSaveOnError、SetDrawQuality、SetRecoImageCacheLimit 等高级配置功能。
...
- 上层 Go API：提供用户友好的类型与方法（如 Pipeline、Node、CustomRecognition、CustomAction、AgentClient/AgentServer、全局配置选项）。
- 内部 native 层：通过 purego 动态加载平台库（MaaAgentClient、MaaAgentServer、MaaGlobalSetOption），桥接 Go 与底层 C/C++ 实现。
...
subgraph "内部绑定"
G["internal/native/agent_client.go"]
H["internal/native/agent_server.go"]
I["internal/native/framework.go"]
end
subgraph "示例"
...
- [internal/native/framework.go](file://internal/native/framework.go#L273-L320)
...
MaaFramework 提供了多个全局配置选项，用于优化框架的行为和性能。以下是最新的全局配置选项：

### SetSaveOnError - 错误时自动截图捕获
- **功能描述**：当任务执行过程中发生错误时，自动保存屏幕截图以便调试分析。
- **使用方式**：`maa.SetSaveOnError(true)` 或 `maa.SetSaveOnError(false)`
- **默认值**：false
- **适用场景**：
  - 开发调试阶段，需要捕获错误时的屏幕状态
  - 自动化测试中记录失败原因
...
- **最佳实践**：
  - 在开发环境中启用此选项以提高调试效率
  - 生产环境中谨慎使用，避免产生大量截图文件
  - 结合日志系统使用，便于问题追踪

### SetDrawQuality - 绘图操作图像质量控制
- **功能描述**：控制绘图操作生成的图像质量，影响识别结果的可视化输出。
- **使用方式**：`maa.SetDrawQuality(85)`（范围：0-100，默认：85）
- **参数范围**：0-100（数值越小质量越差但文件更小）
- **适用场景**：
  - 调试识别算法时查看详细可视化结果
  - 需要平衡文件大小和显示质量的应用
  - 内存受限环境下的性能优化
- **性能影响**：
  - 质量越高，生成的图像文件越大
  - 高质量图像会增加磁盘 I/O 和内存占用
  - 适当降低质量可显著减少存储空间需求

### SetRecoImageCacheLimit - 识别图像缓存管理
- **功能描述**：管理识别过程中使用的图像缓存大小限制。
- **使用方式**：`maa.SetRecoImageCacheLimit(4096)`（默认：4096）
- **单位**：图像数量
- **适用场景**：
  - 内存受限的运行环境
  - 长时间运行的自动化任务
...
func main() {
    ...
    // 配置全局选项
    maa.SetSaveOnError(true)           // 启用错误截图捕获
    maa.SetDrawQuality(90)             // 设置绘图质量为90
    maa.SetRecoImageCacheLimit(8192)   // 设置缓存大小为8192
    
    // 或者使用初始化选项
    maa.Init(
        maa.WithSaveOnError(true),
        maa.WithDrawQuality(90),
        maa.WithRecoImageCacheLimit(8192),
    )
}
```
...
全局配置选项通过 `MaaGlobalOption` 枚举类型进行管理，底层通过 `MaaGlobalSetOption` 函数实现：

```mermaid
flowchart LR
A["Go 层配置函数"] --> B["setGlobalOption"]
B --> C["MaaGlobalOption 枚举"]
C --> D["MaaGlobalSetOption"]
D --> E["底层 C/C++ 实现"]
```

**图表来源**
- [maa.go](file://maa.go#L204-L219)
- [internal/native/framework.go](file://internal/native/framework.go#L273-L315)
...
- 组件耦合
  ...
  - **新增** maa.go 通过 setGlobalOption 函数与 native 层的 MaaGlobalSetOption 进行交互。
- 外部依赖
  - purego：用于动态加载平台库。
...
- **新增** 全局配置性能优化
  - **SetDrawQuality**：在保证调试效果的前提下，适当降低图像质量可显著减少磁盘 I/O 和内存占用。
  - **SetRecoImageCacheLimit**：根据系统内存情况调整缓存大小，避免内存不足导致的性能下降。
  - **SetSaveOnError**：仅在开发调试阶段启用，生产环境谨慎使用以避免额外的磁盘写入开销。
...
- **新增** 全局配置问题排查
  - **SetSaveOnError**：确认错误截图路径权限正常，磁盘空间充足。
  - **SetDrawQuality**：检查生成的图像文件大小是否符合预期，必要时调整质量参数。
  - **SetRecoImageCacheLimit**：监控内存使用情况，避免缓存溢出导致的性能问题。
...
## 结论
通过 CustomRecognition/CustomAction 接口与 Agent 架构，MaaFramework 提供了强大的扩展能力。结合基于 JSON 的流水线协议，开发者可以灵活地定义识别算法与动作逻辑，并通过 AgentClient/AgentServer 实现跨进程协作。**新增的全局配置选项**进一步增强了框架的灵活性和性能优化能力，包括错误截图捕获、图像质量控制和缓存管理等功能。合理配置流水线参数、优化识别与动作性能、完善调试与监控，是构建稳定自动化流程的关键。
...
- 快速参考
  ...
  - **新增** 全局配置：SetSaveOnError、SetDrawQuality、SetRecoImageCacheLimit 等选项用于优化框架行为和性能。